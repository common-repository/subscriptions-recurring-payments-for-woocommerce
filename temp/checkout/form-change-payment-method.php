...
<form id="order_review" method="post">
	...
	<table class="shop_table">
		<thead>
			<tr>
				...
				<th class="product-quantity"><?php echo esc_html_x( 'Quantity', 'table headings in notification email', 'subscriptions-recurring-payments-for-woocommerce' ); ?></th>
				<th class="product-total"><?php echo esc_html_x( 'Totals', 'table headings in notification email', 'subscriptions-recurring-payments-for-woocommerce' ); ?></th>
			</tr>
		</thead>
		<tfoot>
	...
	<div id="payment">
		...
		if ( $subscription->has_payment_gateway() ) {
			$pay_order_button_text = _x( 'Change payment method', 'text on button on checkout page', 'subscriptions-recurring-payments-for-woocommerce' );
		} else {
			$pay_order_button_text = _x( 'Add payment method', 'text on button on checkout page', 'subscriptions-recurring-payments-for-woocommerce' );
		}

		$pay_order_button_text     = apply_filters( 'woocommerce_change_payment_button_text', $pay_order_button_text );
		$customer_subscription_ids = awc_Customer_Store::instance()->get_users_subscription_ids( $subscription->get_customer_id() );
		...
		if ( $available_gateways = WC()->payment_gateways->get_available_payment_gateways() ) :
			...
			<ul class="payment_methods methods">
				...
				foreach ( $available_gateways as $gateway ) :
					$supports_payment_method_changes = AWC_Subscription_Change_Payment_Gateway::can_update_all_subscription_payment_methods( $gateway, $subscription );
					?>
					<li class="wc_payment_method payment_method_<?php echo esc_attr( $gateway->id ); ?>">
						<input id="payment_method_<?php echo esc_attr( $gateway->id ); ?>" type="radio" class="input-radio <?php echo $supports_payment_method_changes ? 'supports-payment-method-changes' : ''; ?>" name="payment_method" value="<?php echo esc_attr( $gateway->id ); ?>" <?php checked( $gateway->chosen, true ); ?> data-order_button_text="<?php echo esc_attr( apply_filters( 'awc_gateway_change_payment_button_text', $pay_order_button_text, $gateway ) ); ?>"/>
		...
		<?php else : ?>
			<div class="woocommerce-error">
				<p> <?php echo esc_html( apply_filters( 'woocommerce_no_available_payment_methods_message', __( 'Sorry, it seems no payment gateways support changing the recurring payment method. Please contact us if you require assistance or to make alternate arrangements.', 'subscriptions-recurring-payments-for-woocommerce' ) ) ); ?></p>
			</div>
		<?php endif; ?>

		<?php if ( $available_gateways ) : ?>
			<?php if ( count( $customer_subscription_ids ) > 1 && AWC_Subscription_Payment_Gateways::one_gateway_supports( 'subscription_payment_method_change_admin' ) ) : ?>
			<span class="update-all-subscriptions-payment-method-wrap">
				<?php
				// translators: $1: opening <strong> tag, $2: closing </strong> tag
				$label = sprintf( esc_html__( 'Update the payment method used for %1$sall%2$s of my current subscriptions', 'subscriptions-recurring-payments-for-woocommerce' ), '<strong>', '</strong>' );
				...
				woocommerce_form_field(
					...
					array(
						...
						'label'   => $label,
						'default' => apply_filters( 'awc_update_all_subscriptions_payment_method_checked', false ),
					)
				);
				?>
			</span>
		...
		<div class="form-row">
			<?php wp_nonce_field( 'awc_change_payment_method', '_asubnonce', true, true ); ?>
