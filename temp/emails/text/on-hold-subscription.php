<?php
/**
 * Cancelled Subscription email with plain text
 *
...
printf( __( 'A subscription belonging to %1$s has been suspended by the user. Their subscription\'s details are as follows:', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription->get_formatted_billing_full_name() );
...
if ( ! empty( $last_order_time_created ) ) {
	...
	echo sprintf( __( 'Last Order: %s', 'subscriptions-recurring-payments-for-woocommerce' ), date_i18n( wc_date_format(), $last_order_time_created ) ) . "\n";
}

// translators: placeholder is localised date string
echo sprintf( __( 'Date Suspended: %s', 'subscriptions-recurring-payments-for-woocommerce' ), date_i18n( wc_date_format(), time() ) ) . "\n";
