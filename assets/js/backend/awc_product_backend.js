jQuery(document).ready(function($){
	$.extend( {
		...
		showHideSubscriptionMeta: function(){
			if ($('select#product-type').val()==asububscriptions.productType) {
				$('.show_if_simple').show();
		...
		setSubscriptionLengths: function(){
			$('[name^="_subscription_length"], [name^="variable_subscription_length"]').each(function(){
				...
				$.each(asububscriptions.subscriptionLengths[ $(periodSelector).val() ], function(length,description) {
		...
		setTrialPeriods: function(){
			$('[name^="_subscription_trial_length"], [name^="variable_subscription_trial_length"]').each(function(){
				...
				if( parseInt(trialLength) == 1 ) {
					periodStrings = asububscriptions.trialPeriodSingular;
				} else {
					periodStrings = asububscriptions.trialPeriodPlurals;
				}
			...
			if ( typeof periodField != 'undefined' ) {
				...
				if('day'==billingPeriod) {
					...
					if('year'==billingPeriod) {
						...
						$.each(asububscriptions.syncOptions[billingPeriod], function(key,description) {
		...
		moveSubscriptionVariationFields: function(){
			$('#variable_product_options .variable_subscription_pricing').not('awc_moved').each(function(){
		...
		getVariationBulkEditValue: function(variation_action){
			...
			switch( variation_action ) {
				...
				case 'variable_subscription_trial_period':
					value = prompt( asububscriptions.bulkEditPeriodMessage );
					break;
				case 'variable_subscription_period_interval':
					value = prompt( asububscriptions.bulkEditIntervalhMessage );
					break;
				case 'variable_subscription_trial_length':
				case 'variable_subscription_length':
					value = prompt( asububscriptions.bulkEditLengthMessage );
					break;
		...
		disableEnableOneTimeShipping: function() {
			...
			if ( 'variable-subscription' == $( 'select#product-type' ).val() ) {
				...
				if ( ( number_of_pages > 1 ||  0 == variations.length ) && false == is_synced_or_has_trial ) {
					...
					var data = {
						action:    'awc_product_has_trial_or_is_synced',
						product_id: woocommerce_admin_meta_boxes_variations.post_id,
						variations_checked: variations_checked,
						nonce:      asububscriptions.oneTimeShippingCheckNonce,
					};

					

					$.ajax({
						url:  asububscriptions.ajaxUrl,
						data: data,
						type: 'POST',
		...
		maybeDisableRemoveLinks: function() {
			$( '#variable_product_options .woocommerce_variation' ).each( function() {
				...
				var can_remove_variation = ( '1' === $( this ).find( 'input.asub-can-remove-variation').val() );
				var $msg                 = $( this ).find( '.asub-can-not-remove-variation-msg' );
	...
	if($.getParameterByName('select_subscription')=='true'){
		$('select#product-type option[value="'+asububscriptions.productType+'"]').attr('selected', 'selected');
		$('select#product-type').select().change();
	}

	// Before saving a subscription product, validate the trial period
	$('#post').submit(function(e){
		if ( asububscriptions.subscriptionLengths !== undefined ){
			var trialLength = $('#_subscription_trial_length').val(),
				selectedTrialPeriod = $('#_subscription_trial_period').val();

			if ( parseInt(trialLength) >= asububscriptions.subscriptionLengths[selectedTrialPeriod].length ) {
				alert(asububscriptions.trialTooLongMessages[selectedTrialPeriod]);
				$('#ajax-loading').hide();
	...
	$('#posts-filter').submit(function(){
		if($('[name="post_type"]').val()=='shop_order'&&($('[name="action"]').val()=='trash'||$('[name="action2"]').val()=='trash')){
			...
			if(containsSubscription){
				return confirm(asububscriptions.bulkTrashWarning);
			}
		}
	});
	...
	$('.order_actions .submitdelete').click(function(){
		if($('[name="contains_subscription"]').val()=='true'){
			return confirm(asububscriptions.trashWarning);
		}
	});

	$( '#variable_product_options' ).on( 'click', '.delete.asub-can-not-remove-variation-msg', function( e ) {
		e.preventDefault();
		e.stopPropagation();
	} );
	...
	$('.users-php .submitdelete').on('click',function(){
		return confirm(asububscriptions.deleteUserWarning);
	});
	...
	$('select.variation_actions').on('variable_subscription_sign_up_fee_ajax_data variable_subscription_period_interval_ajax_data variable_subscription_period_ajax_data variable_subscription_trial_period_ajax_data variable_subscription_trial_length_ajax_data variable_subscription_length_ajax_data', function(event, data) {
		...
		if ( 'variable_subscription_trial_length' == bulk_action ) {
			...
			$( '#_subscription_one_time_shipping' ).addClass( 'awc_ots_needs_update' );
		}
	...
	$('.awc_payment_method_selector').on('change', function() {

		var payment_method = $(this).val();

		$('.awc_payment_method_meta_fields').hide();
		$('#awc_' + payment_method + '_fields').show();
	});
	...
	$( '#woocommerce-product-data' ).on( 'woocommerce_variations_loaded', function() {
		if ( $( '.awc_ots_needs_update' ).length ) {
			$.disableEnableOneTimeShipping();
		}
	});
	...
	$( '#_subscription_one_time_shipping' ).on( 'subscription_one_time_shipping_updated', function( event, is_synced_or_has_trial ) {
		...
		if ( $( '.awc_ots_needs_update' ).length ) {
			var data = {
				action: 'awc_update_one_time_shipping',
				product_id: woocommerce_admin_meta_boxes_variations.post_id,
			...
			console.log('shipiingupdate: ', data);
			...
			$.ajax({
				...
				success : function( response ) {
					...
					$( '#_subscription_one_time_shipping' ).removeClass( 'awc_ots_needs_update' );
	 ...
	 */
	var awc_prevent_variation_removal = {
		init: function() {
		...
		product_type_change: function() {
			...
			if ( 'variable-subscription' === product_type && 'delete_all' === $delete_all.val() ) {
				$delete_all.data( 'awc_original_wc_label', $delete_all.text() )
				           .attr( 'value', 'delete_all_no_subscriptions' )
				           .text( asububscriptions.bulkDeleteOptionLabel );
			} else if ( 'variable-subscription' !== product_type && 'delete_all_no_subscriptions' === $delete_all.val() ) {
				$delete_all.text( $delete_all.data( 'awc_original_wc_label' ) )
		...
		bulk_action_data: function( event, data ) {
			if ( window.confirm( woocommerce_admin_meta_boxes_variations.i18n_delete_all_variations ) ) {
				if ( window.confirm( woocommerce_admin_meta_boxes_variations.i18n_last_warning ) ) {
					...
					$( document ).bind( 'ajaxComplete', awc_prevent_variation_removal.update_qty_after_removal );
				}
			}
		...
		update_qty_after_removal: function( event, jqXHR, ajaxOptions ) {
			...
			$( document ).unbind( 'ajaxComplete', awc_prevent_variation_removal.update_qty_after_removal );
	...
	awc_prevent_variation_removal.init();

	/*
	 * Prevents changing of the product type for subscription products in use by a subscription.
	 */
	var awc_prevent_product_type_change = {
		init: function() {
			if ( 'yes' !== asububscriptions.productHasSubscriptions ) {
				return;
			}
			...
			$select.addClass( 'tips' ).attr( 'data-tip', asububscriptions.productTypeWarning );
		},
	};
	awc_prevent_product_type_change.init();

	/**
	 * Disable subscriptions in product editor 
	 */
	var awc_disable_subscription = {
		init: function(){
			var $select    = $( 'select#product-type' );
			var $options   = $select.find( 'option[value="variable-subscription"]' );
			var $subscription = $select.find( 'option[value="subscription"]' );
			$subscription.text(`${$subscription.text()}  (${window.asububscriptions.subscriptionProductCount}/10)`);
			
			if(window.asububscriptions.subscriptionProductCount >= 10){
				$options = $select.find( 'option[value*="subscription"]' );
			}
	...
	var awc_paypal_standard_settings = {
		...
		paypal_for_subscriptions_enabled: function() {
			if ( $( this ).is( ':checked' ) && ! confirm( asububscriptions.enablePayPalWarning ) ) {
				$( this ).removeAttr( 'checked' );
			}
		}
	};
	awc_paypal_standard_settings.init();
