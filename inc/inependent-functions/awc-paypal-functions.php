 ...
 */
function awc_get_paypal_id( $order ) {

	if ( ! is_object( $order ) ) {
		$order = wc_get_order( $order );
	}

	return awc_get_objects_property( $order, '_paypal_subscription_id' );
}
 ...
 */
function awc_set_paypal_id( $order, $paypal_subscription_id ) {

	if ( ! is_object( $order ) ) {
		$order = wc_get_order( $order );
	}

	if ( awc_is_paypal_profile_a( $paypal_subscription_id, 'billing_agreement' ) ) {
	...
	awc_set_objects_property( $order, 'paypal_subscription_id', $paypal_subscription_id );
}
 ...
 */
function awc_is_paypal_profile_a( $profile_id, $profile_type ) {
 ...
 */
function awc_get_paypal_item_name( $item_name ) {
 ...
 */
function awc_calculate_paypal_trial_periods_until( $future_timestamp ) {
 ...
 * like from AWC_Subscription_Products::is_purchasable() and WC_Product_Subscription_Variation::is_purchasable(),
 * both of which are called within WC_Cart::get_cart_from_session(), which is run before query vars are setup.
 *
 * @return 2.0.13
 * @return bool
 **/
function awc_is_paypal_api_page() {
	return ( false !== strpos( $_SERVER['REQUEST_URI'], 'wc-api/awc_paypal' ) );
}
