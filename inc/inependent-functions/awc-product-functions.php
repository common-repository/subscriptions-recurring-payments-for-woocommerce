 ...
 * @return float
 */
function awc_get_price_including_tax( $product, $args = array() ) {
 ...
 * @return float
 */
function awc_get_price_excluding_tax( $product, $args = array() ) {
 ...
 * @return string
 */
function awc_get_price_html_from_text( $product = '' ) {
 ...
 */
function awc_get_variation_prices( $variation, $variable_product ) {

	return array(
		'price'         => apply_filters( 'woocommerce_variation_prices_price', AWC_Subscription_Products::awc_get_price( $variation ), $variation, $variable_product ),
		'regular_price' => apply_filters( 'woocommerce_variation_prices_regular_price', AWC_Subscription_Products::awc_get_regular_price( $variation, 'edit' ), $variation, $variable_product ),
		'sale_price'    => apply_filters( 'woocommerce_variation_prices_sale_price', AWC_Subscription_Products::awc_get_sale_price( $variation, 'edit' ), $variation, $variable_product ),
		'sign_up_fee'   => apply_filters( 'woocommerce_variation_prices_sign_up_fee', AWC_Subscription_Products::awc_get_sign_up_fee( $variation ), $variation, $variable_product ),
	);
}
 ...
 */
function awc_get_min_max_variation_data( $variable_product, $child_variation_ids = array() ) {
	...
	foreach ( $child_variation_ids as $variation_id ) {
		...
		if ( $variation = wc_get_product( $variation_id ) ) {
			...
			$prices = awc_get_variation_prices( $variation, $variable_product );

			foreach ( $prices as $price_key => $amount ) {
				if ( '' !== $amount ) {
					if ( 'incl' === get_option( 'woocommerce_tax_display_shop' ) ) {
						$prices[ $price_key ] = awc_get_price_including_tax( $variable_product, array( 'price' => $amount ) );
					} else {
						$prices[ $price_key ] = awc_get_price_excluding_tax( $variable_product, array( 'price' => $amount ) );
					}
				}
			}
			...
			$variations_data[ $variation_id ] = array(
				...
				'subscription'  => array(
					...
					'period'       => AWC_Subscription_Products::awc_get_period( $variation ),
					'interval'     => AWC_Subscription_Products::awc_get_interval( $variation ),
					'trial_length' => AWC_Subscription_Products::awc_get_trial_length( $variation ),
					'trial_period' => AWC_Subscription_Products::awc_get_trial_period( $variation ),
					'length'       => AWC_Subscription_Products::awc_get_length( $variation ),
				),
			);
		}
	}

	return awc_calculate_min_max_variations( $variations_data );
}

/**
 * Determine the minimum and maximum values for a set of structured subscription
 * price data in a form created by @see awc_get_min_max_variation_data()
 *
 * @param array $child_variation_ids the IDs of product variation children ids
 * @return array
 
 */
function awc_calculate_min_max_variations( $variations_data ) {
	...
	foreach ( $variations_data as $variation_id => $variation_data ) {
		...
		$is_longest_period   = AWC_Subscriptions::get_longest_period( $variable_subscription_period, $variation_data['subscription']['period'] ) === $variation_data['subscription']['period'];
		...
		if ( $has_free_trial && 0 == $variation_data['subscription']['sign_up_fee'] ) {
			...
			} elseif ( AWC_Subscriptions::get_longest_period( $longest_trial_period, $variation_data['subscription']['trial_period'] ) === $variation_data['subscription']['trial_period'] ) {
			...
			if ( 0 == $lowest_price || '' === $lowest_price ) {
				...
				} elseif ( AWC_Subscriptions::get_shortest_period( $shortest_trial_period, $variation_data['subscription']['trial_period'] ) === $variation_data['subscription']['trial_period'] ) {
			...
			$longest_initial_period  = AWC_Subscriptions::get_longest_period( $longest_initial_period, $initial_period );
			$shortest_initial_period = AWC_Subscriptions::get_shortest_period( $shortest_initial_period, $initial_period );
