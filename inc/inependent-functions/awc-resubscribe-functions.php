 ...
 */
function awc_order_contains_resubscribe( $order ) {

	if ( ! is_a( $order, 'WC_Abstract_Order' ) ) {
		$order = wc_get_order( $order );
	}

	$related_subscriptions = awc_get_subscriptions_for_resubscribe_order( $order );
	...
	return apply_filters( 'awc_subscriptions_order_is_resubscribe', $is_resubscribe_order, $order );
}

/**
 * Create a resubscribe order to record a customer resubscribing to an expired or cancelled subscription.
 *
 * This method is a wrapper for @see awc_create_order() which creates an order with the same post meta, order
 ...
 */
function awc_create_resubscribe_order( $subscription ) {

	$resubscribe_order = awc_create_order_from_subscription( $subscription, 'resubscribe_order' );

	if ( is_wp_error( $resubscribe_order ) ) {
		return new WP_Error( 'resubscribe-order-error', $resubscribe_order->get_error_message() );
	}

	awc_Related_Order_Store::instance()->add_relation( $resubscribe_order, $subscription, 'resubscribe' );

	do_action( 'awc_resubscribe_order_created', $resubscribe_order, $subscription );
 ...
 */
function awc_get_users_resubscribe_link( $subscription ) {
	...
	return apply_filters( 'awc_users_resubscribe_link', $resubscribe_link, $subscription_id );
}
 ...
 */
function awc_get_users_resubscribe_link_for_product( $product_id ) {

	$renewal_url = '';

	if ( is_user_logged_in() ) {
		foreach ( awc_get_users_subscriptions() as $subscription ) {
			foreach ( $subscription->get_items() as $line_item ) {
				if ( ( $line_item['product_id'] == $product_id || $line_item['variation_id'] == $product_id ) && awc_can_user_resubscribe_to( $subscription ) ) {
					$renewal_url = awc_get_users_resubscribe_link( $subscription );
					break;
				}
			}
		}
	...
	return apply_filters( 'awc_users_resubscribe_link_for_product', $renewal_url, $product_id );
}
 ...
 */
function awc_cart_contains_resubscribe( $cart = '' ) {
	...
	return apply_filters( 'awc_cart_contains_resubscribe', $contains_resubscribe, $cart );
}
 ...
 */
function awc_get_subscriptions_for_resubscribe_order( $order ) {
	return awc_get_subscriptions_for_order( $order, array( 'order_type' => 'resubscribe' ) );
}
 ...
 */
function awc_can_user_resubscribe_to( $subscription, $user_id = '' ) {
	...
	if ( empty( $subscription ) ) {
		...
		foreach ( $subscription->get_items() as $line_item ) {
			...
			if ( 'active' === awc_get_product_limitation( $product ) ) {
				...
				if ( awc_user_has_subscription( $user_id, $limited_product_id, 'on-hold' ) || awc_user_has_subscription( $user_id, $limited_product_id, 'active' ) ) {
	...
	return apply_filters( 'awc_can_user_resubscribe_to_subscription', $can_user_resubscribe, $subscription, $user_id );
}
