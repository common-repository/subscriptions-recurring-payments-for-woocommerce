<?php
/**
 * Aco WooCommerce Subscriptions Limit Functions
 *
 */
 ...
 */
function awc_get_product_limitation( $product ) {
	...
	return apply_filters( 'woocommerce_subscriptions_product_limitation', AWC_Subscription_Products::awc_get_meta_data( $parent_product, 'subscription_limit', 'no', 'use_default_value' ), $product );
}
 ...
 * @return boolean
 */
function awc_is_product_limited_for_user( $product, $user_id = 0 ) {
	...
	if ( 'active' == $product_limitation && awc_user_has_subscription( $user_id, $product->get_id(), 'on-hold' ) ) {
		$is_limited_for_user = true;
	} elseif ( 'no' !== $product_limitation ) {
		$is_limited_for_user = awc_user_has_subscription( $user_id, $product->get_id(), $product_limitation );
		...
		if ( 'any' === $product_limitation && $is_limited_for_user ) {
			$is_limited_for_user = false;
			$user_subscriptions  = awc_get_subscriptions( array(
