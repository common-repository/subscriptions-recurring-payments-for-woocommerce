 ...
 */
function awc_get_subscription_period_strings( $number = 1, $period = '' ) {
	...
	$translated_periods = apply_filters( 'woocommerce_subscription_periods',
		array(
			...
			'day'   => sprintf( _nx( 'day',   '%s days',   $number, 'Subscription billing period.', 'subscriptions-recurring-payments-for-woocommerce' ), $number ), // phpcs:ignore WordPress.WP.I18n.MissingSingularPlaceholder,WordPress.WP.I18n.MismatchedPlaceholders
			// translators: placeholder is number of weeks. (e.g. "Bill this every week / 4 weeks")
			'week'  => sprintf( _nx( 'week',  '%s weeks',  $number, 'Subscription billing period.', 'subscriptions-recurring-payments-for-woocommerce' ), $number ), // phpcs:ignore WordPress.WP.I18n.MissingSingularPlaceholder,WordPress.WP.I18n.MismatchedPlaceholders
			// translators: placeholder is number of months. (e.g. "Bill this every month / 4 months")
			'month' => sprintf( _nx( 'month', '%s months', $number, 'Subscription billing period.', 'subscriptions-recurring-payments-for-woocommerce' ), $number ), // phpcs:ignore WordPress.WP.I18n.MissingSingularPlaceholder,WordPress.WP.I18n.MismatchedPlaceholders
			// translators: placeholder is number of years. (e.g. "Bill this every year / 4 years")
			'year'  => sprintf( _nx( 'year',  '%s years',  $number, 'Subscription billing period.', 'subscriptions-recurring-payments-for-woocommerce' ), $number ), // phpcs:ignore WordPress.WP.I18n.MissingSingularPlaceholder,WordPress.WP.I18n.MismatchedPlaceholders
		),
		$number
	);
	// phpcs:enable
 ...
 */
function awc_get_subscription_trial_period_strings( $number = 1, $period = '' ) {

	$translated_periods = apply_filters( 'woocommerce_subscription_trial_periods',
		array(
			// translators: placeholder is a number of days.
			'day'   => sprintf( _n( '%s day', 'a %s-day', $number, 'subscriptions-recurring-payments-for-woocommerce' ), $number ),
			// translators: placeholder is a number of weeks.
			'week'  => sprintf( _n( '%s week', 'a %s-week', $number, 'subscriptions-recurring-payments-for-woocommerce' ), $number ),
			// translators: placeholder is a number of months.
			'month' => sprintf( _n( '%s month', 'a %s-month', $number, 'subscriptions-recurring-payments-for-woocommerce' ), $number ),
			// translators: placeholder is a number of years.
			'year'  => sprintf( _n( '%s year', 'a %s-year', $number, 'subscriptions-recurring-payments-for-woocommerce' ), $number ),
		),
		$number
	);
 ...
 */
function awc_get_non_cached_subscription_ranges() {

	foreach ( array( 'day', 'week', 'month', 'year' ) as $period ) {

		$subscription_lengths = array(
			_x( 'Never expire', 'Subscription length', 'subscriptions-recurring-payments-for-woocommerce' ),
		);

		switch ( $period ) {
			case 'day':
				$subscription_lengths[] = _x( '1 day', 'Subscription lengths. e.g. "For 1 day..."', 'subscriptions-recurring-payments-for-woocommerce' );
				$subscription_range = range( 2, 90 );
				break;
			case 'week':
				$subscription_lengths[] = _x( '1 week', 'Subscription lengths. e.g. "For 1 week..."', 'subscriptions-recurring-payments-for-woocommerce' );
				$subscription_range = range( 2, 52 );
				break;
			case 'month':
				$subscription_lengths[] = _x( '1 month', 'Subscription lengths. e.g. "For 1 month..."', 'subscriptions-recurring-payments-for-woocommerce' );
				$subscription_range = range( 2, 24 );
				break;
			case 'year':
				$subscription_lengths[] = _x( '1 year', 'Subscription lengths. e.g. "For 1 year..."', 'subscriptions-recurring-payments-for-woocommerce' );
				$subscription_range = range( 2, 5 );
				break;
		}

		foreach ( $subscription_range as $number ) {
			$subscription_range[ $number ] = awc_get_subscription_period_strings( $number, $period );
		}
 ...
 * @return bool|mixed
 */
function awc_get_subscription_ranges( $subscription_period = '' ) {
	...
	$subscription_ranges = AWC_Subscriptions::$cache->awc_cache_and_get( 'asub-sub-ranges-' . $locale, 'awc_get_non_cached_subscription_ranges', array(), 3 * HOUR_IN_SECONDS );
 ...
 */
function awc_get_subscription_period_interval_strings( $interval = '' ) {

	$intervals = array( 1 => _x( 'every', 'period interval (eg "$10 _every_ 2 weeks")', 'subscriptions-recurring-payments-for-woocommerce' ) );

	foreach ( range( 2, 6 ) as $i ) {
		// translators: period interval, placeholder is ordinal (eg "$10 every _2nd/3rd/4th_", etc)
		$intervals[ $i ] = sprintf( _x( 'every %s', 'period interval with ordinal number (e.g. "every 2nd"', 'subscriptions-recurring-payments-for-woocommerce' ), AWC_Subscriptions::append_numeral_suffix( $i ) );
	}
 ...
 */
function awc_get_available_time_periods( $form = 'singular' ) {
	...
	$translated_periods = apply_filters( 'woocommerce_subscription_available_time_periods',
		array(
			'day'   => _nx( 'day',   'days',   $number, 'Used in the trial period dropdown. Number is in text field. 0, 2+ will need plural, 1 will need singular.', 'subscriptions-recurring-payments-for-woocommerce' ),
			'week'  => _nx( 'week',  'weeks',  $number, 'Used in the trial period dropdown. Number is in text field. 0, 2+ will need plural, 1 will need singular.', 'subscriptions-recurring-payments-for-woocommerce' ),
			'month' => _nx( 'month', 'months', $number, 'Used in the trial period dropdown. Number is in text field. 0, 2+ will need plural, 1 will need singular.', 'subscriptions-recurring-payments-for-woocommerce' ),
			'year'  => _nx( 'year',  'years',  $number, 'Used in the trial period dropdown. Number is in text field. 0, 2+ will need plural, 1 will need singular.', 'subscriptions-recurring-payments-for-woocommerce' ),
		)
	);
	// phpcs:enable
 ...
 */
function awc_get_subscription_trial_lengths( $subscription_period = '' ) {

	$all_trial_periods = awc_get_subscription_ranges();

	foreach ( $all_trial_periods as $period => $trial_periods ) {
		$all_trial_periods[ $period ][0] = _x( 'no', 'no trial period', 'subscriptions-recurring-payments-for-woocommerce' );
	}
 ...
 */
function awc_add_time( $number_of_periods, $period, $from_timestamp, $timezone_behaviour = 'no_offset' ) {

	if ( $number_of_periods > 0 ) {
		if ( 'month' == $period ) {
			$next_timestamp = awc_add_months( $from_timestamp, $number_of_periods, $timezone_behaviour );
		} else {
			$next_timestamp = awc_strtotime_dark_knight( "+ {$number_of_periods} {$period}", $from_timestamp );
		}
	} else {
 ...
 */
function awc_add_months( $from_timestamp, $months_to_add, $timezone_behaviour = 'no_offset' ) {
	...
	$days_in_next_month = gmdate( 't', awc_strtotime_dark_knight( "+ {$months_to_add} month", awc_date_to_time( $first_day_of_month ) ) );
	...
	if ( gmdate( 'd m Y', $from_timestamp ) === gmdate( 't m Y', $from_timestamp ) || gmdate( 'd', $from_timestamp ) > $days_in_next_month ) {
		for ( $i = 1; $i <= $months_to_add; $i++ ) {
			$next_month = awc_add_time( 3, 'days', $from_timestamp, $timezone_behaviour ); // Add 3 days to make sure we get to the next month, even when it's the 29th day of a month with 31 days
			$next_timestamp = $from_timestamp = awc_date_to_time( gmdate( 'Y-m-t H:i:s', $next_month ) ); // NB the "t" to get last day of next month
		}
	} else { // Safe to just add a month
		$next_timestamp = awc_strtotime_dark_knight( "+ {$months_to_add} month", $from_timestamp );
	}
 ...
 */
function awc_estimate_periods_between( $start_timestamp, $end_timestamp, $unit_of_time = 'month', $rounding_method = 'ceil' ) {
	...
	if ( $end_timestamp <= $start_timestamp ) {
		...
		if ( 'ceil' == $rounding_method ) {
			for ( $periods_until = 0; $timestamp < $end_timestamp; $periods_until++ ) {
				$timestamp = awc_add_months( $timestamp, 1 );
			}
		} else {
		...
		switch ( $unit_of_time ) {
			...
			case 'year':
				...
				// we need to adjust this because YEAR_IN_SECONDS assumes a 365 day year. See notes on awc_number_of_leap_days
				$seconds_until_timestamp = $seconds_until_timestamp - awc_number_of_leap_days( $start_timestamp, $end_timestamp ) * DAY_IN_SECONDS;
				break;
		}
 ...
 * This has caused problems where if there's a leap year, awc_estimate_periods_between would return 2 years instead of
 ...
 */
function awc_number_of_leap_days( $start_timestamp, $end_timestamp ) {
	...
	$leap_years = array_filter( $years, 'awc_is_leap_year' );
	$total_feb_29s = 0;
...
/**
 * Filter function used in awc_number_of_leap_days
 *
 * @param $year int A four digit year, eg 2017
 *
 * @return bool|string
 */
function awc_is_leap_year( $year ) {
 ...
 */
function awc_estimate_period_between( $last_date, $second_date, $interval = 1 ) {

	if ( ! is_int( $interval ) ) {
		$interval = 1;
	}

	$last_timestamp    = awc_date_to_time( $last_date );
	$second_timestamp  = awc_date_to_time( $second_date );
	...
	// check for months
	$full_months = awc_find_full_months_between( $earlier_timestamp, $later_timestamp, $interval );
	...
	$possible_periods_zero_filtered = array_filter( $possible_periods, 'awc_discard_zero_intervals' );
	...
	$possible_periods_no_hd = array_filter( $possible_periods, 'awc_discard_high_deviations' );
	...
	$possible_periods_interval_match = array_filter( $possible_periods, 'awc_match_intervals' );
	...
	usort( $possible_periods, 'awc_sort_by_intervals' );
 ...
 * leap years and variable number of days in months. Uses awc_add_months
 *
 ...
 */
function awc_find_full_months_between( $start_timestamp, $end_timestamp, $interval = 1 ) {
	$number_of_months = 0;
	$remainder = 0;
	...
	while ( 0 <= $remainder ) {
		...
		$start_timestamp = awc_add_months( $start_timestamp, 1 );
 ...
 */
function awc_discard_zero_intervals( $array ) {
 ...
 */
function awc_discard_high_deviations( $array ) {
	switch ( $array['period'] ) {
		case 'year':
 ...
 */
function awc_match_intervals( $array ) {
 ...
 */
function awc_sort_by_intervals( $a, $b ) {
 ...
 * @return int    0 if equal, -1 if $b is larger, 1 if $a is larger
 */
function awc_sort_by_fractions( $a, $b ) {
	if ( $a['fraction'] == $b['fraction'] ) {
		return 0;
	}
	return ( $a['fraction'] > $b['fraction'] ) ? -1 : 1;
 ...
 */
function awc_is_datetime_mysql_format( $time ) {
	if ( ! is_string( $time ) ) {
		return false;
	}
	...
	if ( function_exists( 'strptime' ) ) {
		...
		$valid_time = awc_date_to_time( $time );
	}
 ...
 */
function awc_date_to_time( $date_string ) {

	if ( 0 == $date_string ) {
		return 0;
	}

	$date_time = new AWC_DateTime( $date_string, new DateTimeZone( 'UTC' ) );
 ...
 * @return int Unix timestamp representation of the timestamp passed in without any changes for timezones
 */
function awc_strtotime_dark_knight( $time_string, $from_timestamp = null ) {
 ...
 */
function awc_get_days_in_cycle( $period, $interval ) {
	...
	return apply_filters( 'awc_get_days_in_cycle', $days_in_cycle, $period, $interval );
}
 ...
 * @param AWC_DateTime $date
 * @return AWC_DateTime
 */
function awc_set_local_timezone( AWC_DateTime $date ) {
 ...
 */
function awc_get_sites_timezone() {
 ...
 */
function awc_get_subscription_ranges_tlc() {

	return awc_get_non_cached_subscription_ranges();
}

/**
 * Take a date in the form of a timestamp, MySQL date/time string or DateTime object (or perhaps
 * a AWC_Datetime object when WC > 3.0 is active) and create a AWC_DateTime object.
 *
 
 * @param  string|integer|null $date UTC timestamp, or ISO 8601 DateTime. If the DateTime string has no timezone or offset, WordPress site timezone will be assumed. Null if their is no date.
 * @return null|AWC_DateTime in site's timezone
 */
function awc_get_datetime_from( $variable_date_type ) {

	try {
		if ( empty( $variable_date_type ) ) {
			$datetime = null;
		} elseif ( is_a( $variable_date_type, 'AWC_DateTime' ) ) {
			$datetime = $variable_date_type;
		} elseif ( is_numeric( $variable_date_type ) ) {
			$datetime = new AWC_DateTime( "@{$variable_date_type}", new DateTimeZone( 'UTC' ) );
			$datetime->setTimezone( new DateTimeZone( wc_timezone_string() ) );
		} else {
			$datetime = new AWC_DateTime( $variable_date_type, new DateTimeZone( wc_timezone_string() ) );
		}
	} catch ( Exception $e ) {
		$datetime = null;
	}
...
/**
 * Get a MySQL date/time string in UTC timezone from a AWC_Datetime object.
 *
 
 * @param AWC_DateTime
 * @return string MySQL date/time string representation of the DateTime object in UTC timezone
 */
function awc_get_datetime_utc_string( $datetime ) {
...
/**
 * Format a date for output, a wrapper for awc_format_datetime() introduced with WC 3.0.
 *
 ...
 * @return string
 */
function awc_format_datetime( $date, $format = '' ) {
