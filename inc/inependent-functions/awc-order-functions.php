 ...
 */
function awc_get_subscriptions_for_order( $order, $args = array() ) {
	...
	if ( $get_all || in_array( 'parent', $args['order_type'] ) ) {
		...
		$subscriptions = awc_get_subscriptions( $get_subscriptions_args );
	}

	$all_relation_types = awc_Related_Order_Store::instance()->get_relation_types();
	$relation_types     = $get_all ? $all_relation_types : array_intersect( $all_relation_types, $args['order_type'] );

	foreach ( $relation_types as $relation_type ) {

		$subscription_ids = awc_Related_Order_Store::instance()->get_related_subscription_ids( $order, $relation_type );
 ...
 */
function awc_copy_order_address( $from_order, $to_order, $address_type = 'all' ) {

	if ( in_array( $address_type, array( 'shipping', 'all' ) ) ) {
		$to_order->set_address( array(
			'first_name' => awc_get_objects_property( $from_order, 'shipping_first_name' ),
			'last_name'  => awc_get_objects_property( $from_order, 'shipping_last_name' ),
			'company'    => awc_get_objects_property( $from_order, 'shipping_company' ),
			'address_1'  => awc_get_objects_property( $from_order, 'shipping_address_1' ),
			'address_2'  => awc_get_objects_property( $from_order, 'shipping_address_2' ),
			'city'       => awc_get_objects_property( $from_order, 'shipping_city' ),
			'state'      => awc_get_objects_property( $from_order, 'shipping_state' ),
			'postcode'   => awc_get_objects_property( $from_order, 'shipping_postcode' ),
			'country'    => awc_get_objects_property( $from_order, 'shipping_country' ),
		), 'shipping' );
	}

	if ( in_array( $address_type, array( 'billing', 'all' ) ) ) {
		$to_order->set_address( array(
			'first_name' => awc_get_objects_property( $from_order, 'billing_first_name' ),
			'last_name'  => awc_get_objects_property( $from_order, 'billing_last_name' ),
			'company'    => awc_get_objects_property( $from_order, 'billing_company' ),
			'address_1'  => awc_get_objects_property( $from_order, 'billing_address_1' ),
			'address_2'  => awc_get_objects_property( $from_order, 'billing_address_2' ),
			'city'       => awc_get_objects_property( $from_order, 'billing_city' ),
			'state'      => awc_get_objects_property( $from_order, 'billing_state' ),
			'postcode'   => awc_get_objects_property( $from_order, 'billing_postcode' ),
			'country'    => awc_get_objects_property( $from_order, 'billing_country' ),
			'email'      => awc_get_objects_property( $from_order, 'billing_email' ),
			'phone'      => awc_get_objects_property( $from_order, 'billing_phone' ),
		), 'billing' );
	}
 ...
 * - awc_subscription_meta_query
 * - awc_subscription_meta
 * - awc_renewal_order_meta_query
 * - awc_renewal_order_meta
 *
 ...
 */
function awc_copy_order_meta( $from_order, $to_order, $type = 'subscription' ) {
	global $wpdb;

	if ( ! is_a( $from_order, 'WC_Abstract_Order' ) || ! is_a( $to_order, 'WC_Abstract_Order' ) ) {
		throw new InvalidArgumentException( _x( 'Invalid data. Orders expected aren\'t orders.', 'In awc_copy_order_meta error message. Refers to origin and target order objects.', 'subscriptions-recurring-payments-for-woocommerce' ) );
	}

	if ( ! is_string( $type ) ) {
		throw new InvalidArgumentException( _x( 'Invalid data. Type of copy is not a string.', 'Refers to the type of the copy being performed: "copy_order", "subscription", "renewal_order", "resubscribe_order"', 'subscriptions-recurring-payments-for-woocommerce' ) );
	}
	...
	$meta_query = $wpdb->prepare(
		...
		awc_get_objects_property( $from_order, 'id' )
	);
	...
	$meta_query = apply_filters( 'awc_' . $type . '_meta_query', $meta_query, $to_order, $from_order );
	$meta       = $wpdb->get_results( $meta_query, 'ARRAY_A' );
	$meta       = apply_filters( 'awc_' . $type . '_meta', $meta, $to_order, $from_order );

	// Pre WC 3.0 we need to save each meta individually, post 3.0 we can save the object once
	$save = AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ? 'save' : 'set_prop_only';

	foreach ( $meta as $meta_item ) {
		awc_set_objects_property( $to_order, $meta_item['meta_key'], maybe_unserialize( $meta_item['meta_value'] ), $save, '', 'omit_key_prefix' );
	}
 ...
 */
function awc_create_order_from_subscription( $subscription, $type ) {

	$type = awc_validate_new_order_type( $type );
	...
	try {
		...
		awc_copy_order_meta( $subscription, $new_order, $type );

		// Copy over line items and allow extensions to add/remove items or item meta
		$items = apply_filters( 'awc_new_order_items', $subscription->get_items( array( 'line_item', 'fee', 'shipping', 'tax', 'coupon' ) ), $new_order, $subscription );
		$items = apply_filters( "awc_{$type}_items", $items, $new_order, $subscription );

		foreach ( $items as $item_index => $item ) {

			$item_name = apply_filters( 'awc_new_order_item_name', $item['name'], $item, $subscription );
			$item_name = apply_filters( "awc_{$type}_item_name", $item_name, $item, $subscription );

			// Create order line item on the renewal order
			$order_item_id = wc_add_order_item( awc_get_objects_property( $new_order, 'id' ), array(
			...
			if ( AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
				...
				awc_copy_order_item( $item, $order_item );
				$order_item->save();
			}
			...
			if ( 'line_item' == $item['type'] && isset( $item['product_id'] ) ) {
				...
				if ( false !== $product ) {
					...
					if ( ! empty( $item['variation_id'] ) && null !== ( $variation_data = awc_get_objects_property( $product, 'variation_data' ) ) ) {
					...
					if ( isset( $order_item ) && is_callable( array( $order_item, 'set_backorder_meta' ) ) ) { // WC 3.0
						...
						wc_add_order_item_meta( $order_item_id, apply_filters( 'woocommerce_backordered_item_meta_name', __( 'Backordered', 'subscriptions-recurring-payments-for-woocommerce' ) ), $item['qty'] - max( 0, $product->get_total_stock() ) );
					}
					...
					if ( AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
						...
						do_action( 'woocommerce_order_add_product', awc_get_objects_property( $new_order, 'id' ), $order_item_id, $product, $item['qty'], $args );
					}
				}
			}
		}
		...
		return apply_filters( 'awc_new_order_created', $new_order, $subscription, $type );
 ...
 */
function awc_get_new_order_title( $type ) {

	$type = awc_validate_new_order_type( $type );

	// translators: placeholders are strftime() strings.
	$order_date = strftime( _x( '%b %d, %Y @ %I:%M %p', 'Used in subscription post title. "Subscription renewal order - <this>"', 'subscriptions-recurring-payments-for-woocommerce' ) ); // phpcs:ignore WordPress.WP.I18n.UnorderedPlaceholdersText

	switch ( $type ) {
		case 'renewal_order':
			// translators: placeholder is a date.
			$title = sprintf( __( 'Subscription Renewal Order &ndash; %s', 'subscriptions-recurring-payments-for-woocommerce' ), $order_date );
			break;
		case 'resubscribe_order':
			// translators: placeholder is a date.
			$title = sprintf( __( 'Resubscribe Order &ndash; %s', 'subscriptions-recurring-payments-for-woocommerce' ), $order_date );
			break;
		default:
			$title = '';
			break;
	...
	return apply_filters( 'awc_new_order_title', $title, $type, $order_date );
}
 ...
 */
function awc_validate_new_order_type( $type ) {
	if ( ! is_string( $type ) ) {
		return new WP_Error( 'order_from_subscription_type_type', sprintf( __( '$type passed to the function was not a string.', 'subscriptions-recurring-payments-for-woocommerce' ), $type ) );
	}

	if ( ! in_array( $type, apply_filters( 'awc_new_order_types', array( 'renewal_order', 'resubscribe_order', 'parent' ) ) ) ) {
		// translators: placeholder is an order type.
		return new WP_Error( 'order_from_subscription_type', sprintf( __( '"%s" is not a valid new order type.', 'subscriptions-recurring-payments-for-woocommerce' ), $type ) );
	}
 ...
 * @return array
 */
function awc_get_order_address( $order, $address_type = 'shipping' ) {
	if ( ! is_a( $order, 'WC_Abstract_Order' ) ) {
		return array();
	}
	...
	if ( 'billing' == $address_type ) {
		$address = array(
			'first_name' => awc_get_objects_property( $order, 'billing_first_name' ),
			'last_name'  => awc_get_objects_property( $order, 'billing_last_name' ),
			'company'    => awc_get_objects_property( $order, 'billing_company' ),
			'address_1'  => awc_get_objects_property( $order, 'billing_address_1' ),
			'address_2'  => awc_get_objects_property( $order, 'billing_address_2' ),
			'city'       => awc_get_objects_property( $order, 'billing_city' ),
			'state'      => awc_get_objects_property( $order, 'billing_state' ),
			'postcode'   => awc_get_objects_property( $order, 'billing_postcode' ),
			'country'    => awc_get_objects_property( $order, 'billing_country' ),
			'email'      => awc_get_objects_property( $order, 'billing_email' ),
			'phone'      => awc_get_objects_property( $order, 'billing_phone' ),
		);
	} else {
		$address = array(
			'first_name' => awc_get_objects_property( $order, 'shipping_first_name' ),
			'last_name'  => awc_get_objects_property( $order, 'shipping_last_name' ),
			'company'    => awc_get_objects_property( $order, 'shipping_company' ),
			'address_1'  => awc_get_objects_property( $order, 'shipping_address_1' ),
			'address_2'  => awc_get_objects_property( $order, 'shipping_address_2' ),
			'city'       => awc_get_objects_property( $order, 'shipping_city' ),
			'state'      => awc_get_objects_property( $order, 'shipping_state' ),
			'postcode'   => awc_get_objects_property( $order, 'shipping_postcode' ),
			'country'    => awc_get_objects_property( $order, 'shipping_country' ),
		);
	}

	return apply_filters( 'awc_get_order_address', $address, $address_type, $order );
}
 ...
 */
function awc_order_contains_subscription( $order, $order_type = array( 'parent', 'resubscribe', 'switch' ) ) {
	...
	if ( ( in_array( 'parent', $order_type ) || $get_all ) && count( awc_get_subscriptions_for_order( awc_get_objects_property( $order, 'id' ), array( 'order_type' => 'parent' ) ) ) > 0 ) {
		$contains_subscription = true;

	} elseif ( ( in_array( 'renewal', $order_type ) || $get_all ) && awc_order_contains_renewal( $order ) ) {
		$contains_subscription = true;

	} elseif ( ( in_array( 'resubscribe', $order_type ) || $get_all ) && awc_order_contains_resubscribe( $order ) ) {
		$contains_subscription = true;

	} elseif ( ( in_array( 'switch', $order_type ) || $get_all ) && awc_order_contains_switch( $order ) ) {
		$contains_subscription = true;
 ...
 */
function awc_get_subscription_orders( $return_fields = 'ids', $order_type = 'parent' ) {
	global $wpdb;
	...
	return apply_filters( 'awc_get_subscription_orders', $orders, $return_fields, $order_type );
}
 ...
 */
function awc_get_order_item( $item_id, $order ) {

	$item = array();

	if ( ! is_a( $order, 'WC_Abstract_Order' ) ) {
		throw new InvalidArgumentException( __( 'Invalid data. No valid subscription / order was passed in.', 'subscriptions-recurring-payments-for-woocommerce' ), 422 );
	}

	if ( ! absint( $item_id ) ) {
		throw new InvalidArgumentException( __( 'Invalid data. No valid item id was passed in.', 'subscriptions-recurring-payments-for-woocommerce' ), 422 );
	}
 ...
 */
function awc_update_order_item_type( $item_id, $new_type, $order_or_subscription_id = 0 ) {
 ...
 */
function awc_get_order_item_meta( $item, $product = null ) {
	if ( false === AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
		_deprecated_function( __FUNCTION__, '3.1 of WooCommerce and 2.2.9 of Subscriptions', 'WC_Order_Item_Product->get_formatted_meta_data() or wc_display_item_meta()' );
	}
 ...
 */
function awc_get_order_item_name( $order_item, $include = array() ) {
	...
	if ( $include['attributes'] && ! empty( $order_item['item_meta'] ) ) {
		...
		foreach ( $order_item['item_meta'] as $meta_key => $meta_value ) {
			...
			$meta_value = AWC_Subscriptions::is_woocommerce_pre( 3.0 ) ? $meta_value[0] : $meta_value;
	...
	return apply_filters( 'awc_get_order_item_name', $order_item_name, $order_item, $include );
}
 ...
 * @return string
 */
function awc_get_line_item_name( $line_item ) {
	...
	return apply_filters( 'awc_line_item_name', $line_item_name, $line_item );
}
 ...
 * @return void
 */
function awc_display_item_meta( $item, $order ) {
 ...
 * @return void
 */
function awc_display_item_downloads( $item, $order ) {
	_deprecated_function( __FUNCTION__, '2.5.0', 'wc_display_item_downloads( $item )' );
 ...
 */
function awc_copy_order_item( $from_item, &$to_item ) {

	if ( AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
		awc_doing_it_wrong( __FUNCTION__, 'This function uses data structures introduced in WC 3.0. To copy line item meta use $from_item[\'item_meta\'] and wc_add_order_item_meta().', '2.2' );
		return;
	}
 ...
 * @return bool
 */
function awc_order_contains_manual_subscription( $order, $order_type = 'any' ) {
	$subscriptions = awc_get_subscriptions_for_order( $order, array( 'order_type' => $order_type ) );
 ...
 * @param WC_Order $order
 */
function awc_copy_payment_method_to_order( $subscription, $order ) {
	...
	// Both post_meta and postmeta keys are supported by awc_set_payment_meta().
	$payment_meta = array_intersect_key( $subscription->get_payment_method_meta(), array_flip( array( 'post_meta', 'postmeta' ) ) );
	$payment_meta = (array) apply_filters( 'awc_copy_payment_meta_to_order', $payment_meta, $order, $subscription );

	if ( ! empty( $payment_meta ) ) {
		awc_set_payment_meta( $order, $payment_meta );
	}
 ...
 */
function awc_minutes_since_order_created( $order ) {
	$now             = new AWC_DateTime( 'now', $order->get_date_created()->getTimezone() );
 ...
 */
function awc_seconds_since_order_created( $order ) {
 ...
 */
function awc_find_matching_line_item( $order, $subscription_item, $match_type = 'match_product_ids' ) {
	$matching_item = false;
	...
	$subscription_item_canonical_product_id = awc_get_canonical_product_id( $subscription_item );

	foreach ( $order->get_items() as $order_item ) {
		if ( awc_get_canonical_product_id( $order_item ) !== $subscription_item_canonical_product_id ) {
			continue;
		}
 ...
 */
function awc_order_contains_product( $order, $product ) {
	$order_has_product = false;
	$product_id        = awc_get_canonical_product_id( $product );

	foreach ( $order->get_items() as $line_item ) {
		if ( awc_get_canonical_product_id( $line_item ) === $product_id ) {
 ...
 * @return bool
 **/
function awc_is_order_received_page() {
