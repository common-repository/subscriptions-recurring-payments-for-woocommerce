 ...
 * @return string
 */
function awc_help_tip( $tip, $allow_html = false ) {
 ...
 * @return mixed
 */
function awc_get_objects_property( $object, $property, $single = 'single', $default = null ) {
	...
	$prefixed_key          = awc_maybe_prefix_key( $property );
	...
	switch ( $property ) {
		case 'post':
			...
			if ( method_exists( $object, 'is_type' ) && $object->is_type( 'variation' ) ) {
				$value = get_post( awc_get_objects_property( $object, 'parent_id' ) );
			} else {
				$value = get_post( awc_get_objects_property( $object, 'id' ) );
			}
			break;

		case 'post_status':
			$value = awc_get_objects_property( $object, 'post' )->post_status;
			break;

		case 'variation_data':
			$value = wc_get_product_variation_attributes( awc_get_objects_property( $object, 'id' ) );
			break;
		...
		default:
			...
			if ( is_callable( array( $object, $function_name ) ) ) {
				...
				} elseif ( strtolower( $property ) !== 'id' && metadata_exists( 'post', awc_get_objects_property( $object, 'id' ), $prefixed_key ) ) {
					// If we couldn't find a property or function, fallback to using post meta as that's what many __get() methods in WC < 3.0 did.
					if ( 'single' === $single ) {
						$value = get_post_meta( awc_get_objects_property( $object, 'id' ), $prefixed_key, true );
					} else {
						// Get all the meta values.
						$value = get_post_meta( awc_get_objects_property( $object, 'id' ), $prefixed_key, false );
					}
				}
			}
			break;
 ...
 * @return mixed
 */
function awc_set_objects_property( &$object, $key, $value, $save = 'save', $meta_id = '', $prefix_meta_key = 'prefix_meta_key' ) {

	$prefixed_key = awc_maybe_prefix_key( $key );
	...
	// If we have a 3.0 object, use the setter if available.
		...
		// Prices include tax is stored as a boolean in props but saved in the database as a string yes/no, so we need to normalise it here to make sure if we have a string (which can be passed to it by things like awc_copy_order_meta()) that it's converted to a boolean before being set
	...
	if ( 'save' === $save ) {
		if ( is_callable( array( $object, 'save' ) ) ) { // WC 3.0+
			...
			wp_update_post(
				array(
					'ID'            => awc_get_objects_property( $object, 'id' ),
			...
			if ( ! empty( $meta_id ) ) {
				...
				update_post_meta( awc_get_objects_property( $object, 'id' ), $meta_key, $value );
			}
		}
	}
}
 ...
 * @return mixed
 */
function awc_delete_objects_property( &$object, $key, $save = 'save', $meta_id = '' ) {
	...
	if ( 'save' === $save ) {
		if ( method_exists( $object, 'save' ) ) { // WC 3.0+
			...
			delete_post_meta( awc_get_objects_property( $object, 'id' ), $prefixed_key );
		}
	}
}
 ...
 * @return mixed
 */
function awc_product_deprecated_property_handler( $property, $product ) {
	...
	if ( in_array( $property, array( 'product_type', 'parent_product_type', 'limit_subscriptions', 'subscription_limit', 'subscription_payment_sync_date', 'subscription_one_time_shipping' ) ) || ( is_callable( array( 'AWC_Subscription_Products', $function_name ) ) && false !== strpos( $property, 'subscription' ) ) ) {
		...
		switch ( $property ) {
			...
			case 'subscription_limit':
				$value       = awc_get_product_limitation( $product );
				$alternative = 'awc_get_product_limitation( $product )';
				break;

			case 'subscription_one_time_shipping':
				$value       = AWC_Subscription_Products::needs_one_time_shipping( $product );
				$alternative = 'AWC_Subscription_Products::needs_one_time_shipping( $product )';
				break;
			...
			case 'max_variation_period_interval':
				...
				$alternative = $class_name . '::get_meta( ' . $meta_key . ' ) or awc_get_min_max_variation_data( $product )';
				break;

			default:
				$value       = call_user_func( array( 'AWC_Subscription_Products', $function_name ), $product );
				$alternative = sprintf( 'AWC_Subscription_Products::%s( $product )', $function_name );
				break;
		}
 ...
 *
 * Similar to @see awc_get_objects_property
 *
 ...
 * @return mixed
 */
function awc_get_coupon_property( $coupon, $property ) {
 ...
 *
 * Similar to @see awc_set_objects_property
 *
 ...
 */
function awc_set_coupon_property( &$coupon, $property, $value ) {
 ...
 */
function awc_generate_order_key() {
 ...
 */
function awc_update_settings_option( $settings_api, $key, $value ) {
 ...
 */
function awc_is_rest_api_request() {
 ...
 */
function awc_get_used_coupon_codes( $order ) {
 ...
 * This function is a wrapper for @see AWC_Dependent_Hook_Manager::add_woocommerce_dependent_action().
 *
 ...
 */
function awc_add_woocommerce_dependent_action( $tag, $function, $woocommerce_version, $operator, $priority = 10, $number_of_args = 1 ) {
	AWC_Dependent_Hook_Manager::add_woocommerce_dependent_action( $tag, $function, $woocommerce_version, $operator, $priority, $number_of_args );
}
