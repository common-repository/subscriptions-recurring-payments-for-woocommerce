 ...
 */
if ( !class_exists( 'AWC_Admin_Post_Types' ) ){
	class AWC_Admin_Post_Types {
		 ...
		 * in @see awc_Admin_Post_Types::request_query function.
		 *
		...
		public function print_bulk_actions_script() {
			...
			$bulk_actions = apply_filters( 'woocommerce_subscription_bulk_actions', array(
				'active'    => _x( 'Activate', 'an action on a subscription', 'subscriptions-recurring-payments-for-woocommerce' ),
				'on-hold'   => _x( 'Put on-hold', 'an action on a subscription', 'subscriptions-recurring-payments-for-woocommerce' ),
				'cancelled' => _x( 'Cancel', 'an action on a subscription', 'subscriptions-recurring-payments-for-woocommerce' ),
			) );
		...
		public function parse_bulk_actions() {
			...
			foreach ( $subscription_ids as $subscription_id ) {
				...
				$order_note   = _x( 'Subscription status changed by bulk edit:', 'Used in order note. Reason why status changed.', 'subscriptions-recurring-payments-for-woocommerce' );
		...
		public function bulk_admin_notices() {
			...
			foreach ( $subscription_statuses as $slug => $name ) {
				...
				if ( isset( $_REQUEST[ 'marked_' . str_replace( 'wc-', '', $slug ) ] ) ) {
					...
					$message = sprintf( _n( '%s subscription status changed.', '%s subscription statuses changed.', $number, 'subscriptions-recurring-payments-for-woocommerce' ), number_format_i18n( $number ) );
					...
					if ( ! empty( $_REQUEST['error_count'] ) ) {
						...
						$message = sprintf( _n( '%1$s subscription could not be updated: %2$s', '%1$s subscriptions could not be updated: %2$s', $error_count, 'subscriptions-recurring-payments-for-woocommerce' ), number_format_i18n( $error_count ), $error_msg );
		...
		public function shop_subscription_columns( $existing_columns ) {
			...
			$columns = array(
				...
				'status'            => __( 'Status', 'subscriptions-recurring-payments-for-woocommerce' ),
				'order_title'       => __( 'Subscription', 'subscriptions-recurring-payments-for-woocommerce' ),
				'order_items'       => __( 'Items', 'subscriptions-recurring-payments-for-woocommerce' ),
				'recurring_total'   => __( 'Total', 'subscriptions-recurring-payments-for-woocommerce' ),
				'start_date'        => __( 'Start Date', 'subscriptions-recurring-payments-for-woocommerce' ),
				'trial_end_date'    => __( 'Trial End', 'subscriptions-recurring-payments-for-woocommerce' ),
				'next_payment_date' => __( 'Next Payment', 'subscriptions-recurring-payments-for-woocommerce' ),
				'last_payment_date' => __( 'Last Order Date', 'subscriptions-recurring-payments-for-woocommerce' ), // Keep deprecated 'last_payment_date' key for backward compatibility
				'end_date'          => __( 'End Date', 'subscriptions-recurring-payments-for-woocommerce' ),
				'orders'            => _x( 'Orders', 'number of orders linked to a subscription', 'subscriptions-recurring-payments-for-woocommerce' ),
			);
		...
		public function render_shop_subscription_columns( $column ) {
			...
			switch ( $column ) {
				case 'status':
					// The status label
					$column_content = sprintf( '<mark class="subscription-status order-status status-%1$s %1$s tips" data-tip="%2$s"><span>%3$s</span></mark>', sanitize_title( $the_subscription->get_status() ), awc_get_subscription_status_name( $the_subscription->get_status() ), awc_get_subscription_status_name( $the_subscription->get_status() ) );
					...
					$all_statuses = array(
						'active'    => __( 'Reactivate', 'subscriptions-recurring-payments-for-woocommerce' ),
						'on-hold'   => __( 'Suspend', 'subscriptions-recurring-payments-for-woocommerce' ),
						'cancelled' => _x( 'Cancel', 'an action on a subscription', 'subscriptions-recurring-payments-for-woocommerce' ),
						'trash'     => __( 'Trash', 'subscriptions-recurring-payments-for-woocommerce' ),
						'deleted'   => __( 'Delete Permanently', 'subscriptions-recurring-payments-for-woocommerce' ),
					);
							...
							if ( in_array( $status, array( 'trash', 'deleted' ) ) ) {
								...
								if ( current_user_can( $post_type_object->cap->delete_post, $post->ID ) ) {
									...
									if ( 'trash' == $post->post_status ) {
										$actions['untrash'] = '<a title="' . esc_attr( __( 'Restore this item from the Trash', 'subscriptions-recurring-payments-for-woocommerce' ) ) . '" href="' . wp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ) . '">' . __( 'Restore', 'subscriptions-recurring-payments-for-woocommerce' ) . '</a>';
									} elseif ( EMPTY_TRASH_DAYS ) {
										$actions['trash'] = '<a class="submitdelete" title="' . esc_attr( __( 'Move this item to the Trash', 'subscriptions-recurring-payments-for-woocommerce' ) ) . '" href="' . get_delete_post_link( $post->ID ) . '">' . __( 'Trash', 'subscriptions-recurring-payments-for-woocommerce' ) . '</a>';
									}

									if ( 'trash' == $post->post_status || ! EMPTY_TRASH_DAYS ) {
										$actions['delete'] = '<a class="submitdelete" title="' . esc_attr( __( 'Delete this item permanently', 'subscriptions-recurring-payments-for-woocommerce' ) ) . '" href="' . get_delete_post_link( $post->ID, '', true ) . '">' . __( 'Delete Permanently', 'subscriptions-recurring-payments-for-woocommerce' ) . '</a>';
									}
								}
							} else {

								if ( 'cancelled' === $status && 'pending-cancel' === $the_subscription->get_status() ) {
									$label = __( 'Cancel Now', 'subscriptions-recurring-payments-for-woocommerce' );
								}
				...
				case 'order_title':
					...
					if ( $address = $the_subscription->get_formatted_billing_address() ) {
						$customer_tip .= _x( 'Billing:', 'meaning billing address', 'subscriptions-recurring-payments-for-woocommerce' ) . ' ' . esc_html( $address );
					}

					if ( $the_subscription->get_billing_email() ) {
						// translators: placeholder is customer's billing email
						$customer_tip .= '<br/><br/>' . sprintf( __( 'Email: %s', 'subscriptions-recurring-payments-for-woocommerce' ), esc_attr( $the_subscription->get_billing_email() ) );
					}

					if ( $the_subscription->get_billing_phone() ) {
						// translators: placeholder is customer's billing phone number
						$customer_tip .= '<br/><br/>' . sprintf( __( 'Tel: %s', 'subscriptions-recurring-payments-for-woocommerce' ), esc_html( $the_subscription->get_billing_phone() ) );
					}
					...
					$column_content = sprintf( _x( '%1$s#%2$s%3$s for %4$s', 'Subscription title on admin table. (e.g.: #211 for John Doe)', 'subscriptions-recurring-payments-for-woocommerce' ), '<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $post->ID ) . '&action=edit' ) ) . '">', '<strong>' . esc_attr( $the_subscription->get_order_number() ) . '</strong>', '</a>', $username );

					$column_content .= '</div>';

					$column_content .= '<button type="button" class="toggle-row"><span class="screen-reader-text">' . __( 'Show more details', 'subscriptions-recurring-payments-for-woocommerce' ) . '</span></button>';
					...
					switch ( count( $subscription_items ) ) {
						...
						case 1:
							foreach ( $subscription_items as $item ) {
								$item_name      = wp_kses( self::get_item_name( $item, $item->get_product() ), array( 'a' => array( 'href' => array() ) ) );
								$item_meta_html = self::get_item_meta_html( $item );
								$meta_help_tip  = $item_meta_html ? awc_help_tip( $item_meta_html, true ) : '';
						...
						default:
							...
							$column_content .= '<a href="#" class="show_order_items">' . esc_html( apply_filters( 'woocommerce_admin_order_item_count', sprintf( _n( '%d item', '%d items', $the_subscription->get_item_count(), 'subscriptions-recurring-payments-for-woocommerce' ), $the_subscription->get_item_count() ), $the_subscription ) ) . '</a>';
							$column_content .= '<table class="order_items" cellspacing="0">';

							foreach ( $subscription_items as $item ) {
								$item_name      = self::get_item_name( $item, $item->get_product(), 'do_not_include_quantity' );
								$item_meta_html = self::get_item_meta_html( $item );
				...
				case 'recurring_total':
					...
					$column_content .= esc_html( sprintf( __( 'Via %s', 'subscriptions-recurring-payments-for-woocommerce' ), $the_subscription->get_payment_method_to_display() ) );

					if ( AWC_Subscriptions::is_duplicate_site() && $the_subscription->has_payment_gateway() && ! $the_subscription->get_requires_manual_renewal() ) {
						$column_content .= AWC_Staging::get_payment_method_tooltip( $the_subscription );
					}
		...
		public static function get_date_column_content( $subscription, $column ) {
			...
			if ( 0 == $subscription->get_time( $date_type, 'gmt' ) ) {
				...
				$column_content = sprintf( '<time class="%s" title="%s">%s</time>', esc_attr( $column ), esc_attr( date( __( 'Y/m/d g:i:s A', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription->get_time( $date_type, 'site' ) ) ), esc_html( $subscription->get_date_to_display( $date_type ) ) );
				...
				if ( 'next_payment_date' == $column && $subscription->payment_method_supports( 'gateway_scheduled_payments' ) && ! $subscription->is_manual() && $subscription->has_status( 'active' ) ) {
					$column_content .= '<div class="woocommerce-help-tip" data-tip="' . esc_attr__( 'This date should be treated as an estimate only. The payment gateway for this subscription controls when payments are processed.', 'subscriptions-recurring-payments-for-woocommerce' ) . '"></div>';
				}
			}
		...
		public function shop_subscription_search_custom_fields( $wp ) {
			...
			$post_ids = awc_subscription_search( $_GET['s'] );
			...
			if ( 'shop_subscription' === $typenow ) {
				...
				if ( isset( $_GET['_customer_user'] ) && $_GET['_customer_user'] > 0 ) {
					...
					$subscription_ids = apply_filters(
						'awc_admin_request_query_subscriptions_for_customer',
						awc_Customer_Store::instance()->get_users_subscription_ids( $customer_id ),
						$customer_id
					);
				...
				if ( isset( $_GET['_awc_product'] ) && $_GET['_awc_product'] > 0 ) {
					$product_id       = absint( $_GET['_awc_product'] );
					$subscription_ids = awc_get_product_subscriptions( $product_id );

					$subscription_ids = apply_filters(
						'awc_admin_request_query_subscriptions_for_product',
				...
				if ( empty( $vars['post_status'] ) ) {
					$vars['post_status'] = array_keys( awc_get_subscription_statuses() );
				}
			}
		...
		public static function awc_get_subscription_status_count(){
			$subscription_counts      = array();
			$subscription_counts_data = wp_count_posts( 'shop_subscription' );
			foreach ( awc_get_subscription_statuses() as $status_slug => $status_name ) {
		...
		public function post_updated_messages( $messages ) {
			...
			$messages['shop_subscription'] = array(
				...
				1  => __( 'Subscription updated.', 'subscriptions-recurring-payments-for-woocommerce' ),
				2  => __( 'Custom field updated.', 'subscriptions-recurring-payments-for-woocommerce' ),
				3  => __( 'Custom field deleted.', 'subscriptions-recurring-payments-for-woocommerce' ),
				4  => __( 'Subscription updated.', 'subscriptions-recurring-payments-for-woocommerce' ),
				// translators: placeholder is previous post title
				5  => isset( $_GET['revision'] ) ? sprintf( _x( 'Subscription restored to revision from %s', 'used in post updated messages', 'subscriptions-recurring-payments-for-woocommerce' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
				6  => __( 'Subscription updated.', 'subscriptions-recurring-payments-for-woocommerce' ),
				7  => __( 'Subscription saved.', 'subscriptions-recurring-payments-for-woocommerce' ),
				8  => __( 'Subscription submitted.', 'subscriptions-recurring-payments-for-woocommerce' ),
				// translators: php date string
				9  => sprintf( __( 'Subscription scheduled for: %1$s.', 'subscriptions-recurring-payments-for-woocommerce' ), '<strong>' . date_i18n( _x( 'M j, Y @ G:i', 'used in "Subscription scheduled for <date>"', 'subscriptions-recurring-payments-for-woocommerce' ), awc_date_to_time( $post->post_date ) ) . '</strong>' ),
				10 => __( 'Subscription draft updated.', 'subscriptions-recurring-payments-for-woocommerce' ),
			);
		 ...
		 */
		public static function get_related_orders_link( $the_subscription ) {
			return sprintf(
				'<a href="%s">%s</a>',
		...
		/**
		 * @access	public
		 * @return 	all subscription orders
		 */
		public static function awc_get_subscriptions($subscription_query = false){
			if(!$subscription_query){
				$subscription_query = [
					'posts_per_page' => 10,
					'page'           => -1,
					'subscription_status' => array('active', 'cancelled', 'on-hold')
				];
			}

			// Reference function render_shop_subscription_columns
			$subscriptions = awc_get_subscriptions( $subscription_query, true );

			
			$subscriptions_map = array_map(function($v){
				
				$newitem = new stdClass();
				$newitem->id = $v->get_id();
				$newitem->status_html = sprintf( '<span class="subscription-status order-status status-%1$s %1$s tips">%3$s</span>', sanitize_title( $v->get_status() ), awc_get_subscription_status_name( $v->get_status() ), awc_get_subscription_status_name( $v->get_status() ) );
				$newitem->status = $v->get_status();

				$newitem->items = array_map(function($i){
					return array(
						'id' => $i->get_product_id(),
						'name' => wp_kses( self::get_item_name( $i, $i->get_product() ), array( 'a' => array( 'href' => array() ) ) )
					);
				}, $v->get_items());

				$newitem->user = array(
					'id' => $v->get_customer_id(),
					'name' => get_userdata($v->get_customer_id())->display_name
				);

				// $address = $v->get_address();

				$customer_tip = '';
				if ( $address = $v->get_formatted_billing_address() ) 
					$customer_tip .= _x( 'Billing:', 'meaning billing address', 'subscriptions-recurring-payments-for-woocommerce' ) . ' ' .  $address ;
				

				if ( $v->get_billing_email() )
					$customer_tip .= '<br/><br/>' . sprintf( __( 'Email: %s', 'subscriptions-recurring-payments-for-woocommerce' ), esc_attr( $v->get_billing_email() ) );
				

				if ( $v->get_billing_phone() ) 
					$customer_tip .= '<br/><br/>' . sprintf( __( 'Tel: %s', 'subscriptions-recurring-payments-for-woocommerce' ),  $v->get_billing_phone() );
				
				$newitem->tooltip = $customer_tip;



				$require_total = esc_html( strip_tags( $v->get_formatted_order_total() ) );
				$require_total .= '<small class="meta">';
				$require_total .= esc_html( sprintf( __( 'Via %s', 'subscriptions-recurring-payments-for-woocommerce' ), $v->get_payment_method_to_display() ) );

				if ( AWC_Subscriptions::is_duplicate_site() && $v->has_payment_gateway() && ! $v->get_requires_manual_renewal() ) {
					$require_total .= AWC_Staging::get_payment_method_tooltip( $v );
				}
				$require_total .= '</small>';

				$newitem->require_total = $require_total;

				$newitem->last_order_date = self::get_date_column_content($v, 'last_payment_date');
				$newitem->start_date = self::get_date_column_content($v, 'start_date');
				$newitem->next_payment = self::get_date_column_content($v, 'next_payment_date');
				$newitem->end_date = self::get_date_column_content($v, 'end_date');
				$newitem->orders = self::get_related_orders_link($v);

				return $newitem;
			}, $subscriptions->subscriptions);

			return apply_filters( 'awc_backend_subscriptions_lists', array('subscriptions' => $subscriptions_map, 'max_num_pages' => $subscriptions->max_num_pages) );
		}
		/**
		 ...
		 * @return string
		 */
		public static function get_item_name( $item, $_product, $include_quantity = 'include_quantity' ) {
			...
			// if ( $_product ) {
			// 	$item_name = sprintf( '<a href="%s">%s</a>', get_edit_post_link( ( $_product->is_type( 'variation' ) ) ? awc_get_objects_property( $_product, 'parent_id' ) : $_product->get_id() ), $item_name );
			// }
		...
		protected static function get_item_display_row( $item, $item_name, $item_meta_html ) {
			...
			<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_admin_order_item_class', '', $item ) ); ?>">
				...
				<td class="name">
					...
					if ( $item_meta_html ) {
						echo awc_help_tip( $item_meta_html, true );
					} ?>
				</td>
			</tr>
			<?php
		...
		protected static function get_item_display( $item, $subscription = '', $element = 'div' ) {
			...
			if ( 'div' === $element ) {
				$item_html = self::get_item_display_div( $item, self::get_item_name( $item, $_product ), $item_meta_html );
			} else {
				$item_html = self::get_item_display_row( $item, self::get_item_name( $item, $_product, 'do_not_include_quantity' ), $item_meta_html );
		...
		protected static function get_item_display_div( $item, $item_name, $item_meta_html ) {
			...
			if ( $item_meta_html ) {
				$item_html .= awc_help_tip( $item_meta_html, true );
			}
