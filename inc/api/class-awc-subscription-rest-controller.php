...
class AWC_Subscription_REST_Controller extends WC_REST_Orders_V1_Controller {
	...
	public function filter_get_subscription_response( $response, $post, $request ) {
		...
		if ( ! empty( $post->post_type ) && ! empty( $post->ID ) && 'shop_subscription' == $post->post_type ) {
			...
			$resubscribed_subscriptions                  = array_filter( $subscription->get_related_orders( 'ids', 'resubscribe' ), 'awc_is_subscription' );
			$response->data['resubscribed_from']         = strval( awc_get_objects_property( $subscription, 'subscription_resubscribe' ) );
	...
	protected function create_order( $request ) {
		try {
			if ( ! is_null( $request['customer_id'] ) && 0 !== $request['customer_id'] && false === get_user_by( 'id', $request['customer_id'] ) ) {
				throw new WC_REST_Exception( 'woocommerce_rest_invalid_customer_id', __( 'Customer ID is invalid.', 'subscriptions-recurring-payments-for-woocommerce' ), 400 );
			}
			...
			// Store the post meta on the subscription after it's saved, this is to avoid compat. issue with the filters in AWC_Subscriptions::set_payment_method_meta() expecting the $subscription to have an ID (therefore it needs to be called after the WC_Subscription has been saved)
	...
	protected function update_order( $request ) {
		try {
			...
			// Update the post meta on the subscription after it's saved, this is to avoid compat. issue with the filters in AWC_Subscriptions::set_payment_method_meta() expecting the $subscription to have an ID (therefore it needs to be called after the WC_Subscription has been saved)
			$payment_data = ( ! empty( $request['payment_details'] ) ) ? $request['payment_details'] : array();
	...
	public function get_subscription_orders( $request ) {
		...
		if ( empty( $id ) || ! awc_is_subscription( $id ) ) {
			return new WP_Error( 'woocommerce_rest_invalid_shop_subscription_id', __( 'Invalid subscription id.', 'subscriptions-recurring-payments-for-woocommerce' ), array( 'status' => 404 ) );
		}
		...
		foreach ( $subscription_orders as $order_id ) {
			...
			foreach ( array( 'parent', 'renewal', 'switch' ) as $order_type ) {
				if ( awc_order_contains_subscription( $order_id, $order_type ) ) {
		...
		return apply_filters( 'awc_rest_subscription_orders_response', $response, $request );
	}
	...
	public function get_statuses() {
		return rest_ensure_response( awc_get_subscription_statuses() );
	}
	...
	protected function get_order_statuses() {
		...
		foreach ( array_keys( awc_get_subscription_statuses() ) as $status ) {
	...
	public function update_payment_method( $subscription, $data, $updating = false ) {
		...
		try {
			if ( $updating && ! array_key_exists( $payment_method, awc_Change_Payment_Method_Admin::get_valid_payment_methods( $subscription ) ) ) {
				throw new Exception( __( 'Gateway does not support admin changing the payment method on a Subscription.', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}
			...
			$subscription = awc_get_subscription( $subscription->get_id() );
			...
			throw new WC_REST_Exception( 'woocommerce_rest_invalid_payment_data', sprintf( __( 'Subscription payment method could not be set to %1$s with error message: %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), $payment_method, $e->getMessage() ), 400 );
		}
	}
	...
	protected function prepare_item_for_database( $request ) {
		...
		try {
			...
			throw new WC_REST_Exception( 'woocommerce_rest_cannot_update_subscription_dates', sprintf( __( 'Updating subscription dates errored with message: %s', 'subscriptions-recurring-payments-for-woocommerce' ), $e->getMessage() ), 400 );
		}
	...
	public function get_item_schema() {
		...
		$subscriptions_schema = array(
			'transition_status'         => array(
				'description' => __( 'The status to transition the subscription to. Unlike the "status" param, this will calculate and update the subscription dates.', 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'string',
				'enum'        => $this->get_order_statuses(),
				'context'     => array( 'edit' ),
			),
			'billing_interval'          => array(
				'description' => __( 'The number of billing periods between subscription renewals.', 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'integer',
				'context'     => array( 'view', 'edit' ),
			),
			'billing_period'            => array(
				'description' => __( 'Billing period for the subscription.', 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'string',
				'enum'        => array_keys( awc_get_subscription_period_strings() ),
				'context'     => array( 'view', 'edit' ),
			),
			'payment_details'           => array(
				'description' => __( 'Subscription payment details.', 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'object',
				'context'     => array( 'edit' ),
				'properties'  => array(
					'method_id' => array(
						'description' => __( 'Payment gateway ID.', 'subscriptions-recurring-payments-for-woocommerce' ),
						'type'        => 'string',
						'context'     => array( 'edit' ),
					),
				),
			...
			'start_date'                => array(
				'description' => __( "The subscription's start date.", 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'date-time',
				'context'     => array( 'view', 'edit' ),
			),
			'trial_end_date'            => array(
				'description' => __( "The subscription's trial date", 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'date-time',
				'context'     => array( 'view', 'edit' ),
			),
			'next_payment_date'         => array(
				'description' => __( "The subscription's next payment date.", 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'date-time',
				'context'     => array( 'view', 'edit' ),
			),
			'end_date'                  => array(
				'description' => __( "The subscription's end date.", 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'date-time',
				'context'     => array( 'view', 'edit' ),
			),
			'resubscribed_from'         => array(
				'description' => __( "The subscription's original subscription ID if this is a resubscribed subscription.", 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'string',
				'context'     => array( 'view' ),
				'readonly'    => true,
			),
			'resubscribed_subscription' => array(
				'description' => __( "The subscription's resubscribed subscription ID.", 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'string',
				'context'     => array( 'view' ),
				'readonly'    => true,
			),
			'date_completed_gmt'        => array(
				'description' => __( "The date the subscription's latest order was completed, in GMT.", 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'date-time',
				'context'     => array( 'view' ),
				'readonly'    => true,
			),
			'date_paid_gmt'             => array(
				'description' => __( "The date the subscription's latest order was paid, in GMT.", 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'date-time',
				'context'     => array( 'view' ),
				'readonly'    => true,
			),
			'removed_line_items'        => array(
				'description' => __( 'Removed line items data.', 'subscriptions-recurring-payments-for-woocommerce' ),
				'type'        => 'array',
				'context'     => array( 'view', 'edit' ),
				'items'       => array(
					'type'       => 'object',
					'properties' => array(
						'id'           => array(
							'description' => __( 'Item ID.', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'integer',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'name'         => array(
							'description' => __( 'Product name.', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'mixed',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'sku'          => array(
							'description' => __( 'Product SKU.', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'product_id'   => array(
							'description' => __( 'Product ID.', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'mixed',
							'context'     => array( 'view', 'edit' ),
						),
						'variation_id' => array(
							'description' => __( 'Variation ID, if applicable.', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'integer',
							'context'     => array( 'view', 'edit' ),
						),
						'quantity'     => array(
							'description' => __( 'Quantity ordered.', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'integer',
							'context'     => array( 'view', 'edit' ),
						),
						'tax_class'    => array(
							'description' => __( 'Tax class of product.', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'price'        => array(
							'description' => __( 'Product price.', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'subtotal'     => array(
							'description' => __( 'Line subtotal (before discounts).', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'subtotal_tax' => array(
							'description' => __( 'Line subtotal tax (before discounts).', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'total'        => array(
							'description' => __( 'Line total (after discounts).', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'total_tax'    => array(
							'description' => __( 'Line total tax (after discounts).', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'taxes'        => array(
							'description' => __( 'Line taxes.', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'array',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
							'items'       => array(
								...
								'properties' => array(
									'id'       => array(
										'description' => __( 'Tax rate ID.', 'subscriptions-recurring-payments-for-woocommerce' ),
										'type'        => 'integer',
										'context'     => array( 'view', 'edit' ),
										'readonly'    => true,
									),
									'total'    => array(
										'description' => __( 'Tax total.', 'subscriptions-recurring-payments-for-woocommerce' ),
										'type'        => 'string',
										'context'     => array( 'view', 'edit' ),
										'readonly'    => true,
									),
									'subtotal' => array(
										'description' => __( 'Tax subtotal.', 'subscriptions-recurring-payments-for-woocommerce' ),
										'type'        => 'string',
										'context'     => array( 'view', 'edit' ),
										'readonly'    => true,
									),
						...
						'meta'         => array(
							'description' => __( 'Removed line item meta data.', 'subscriptions-recurring-payments-for-woocommerce' ),
							'type'        => 'array',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
							'items'       => array(
								...
								'properties' => array(
									'key'   => array(
										'description' => __( 'Meta key.', 'subscriptions-recurring-payments-for-woocommerce' ),
										'type'        => 'string',
										'context'     => array( 'view', 'edit' ),
										'readonly'    => true,
									),
									'label' => array(
										'description' => __( 'Meta label.', 'subscriptions-recurring-payments-for-woocommerce' ),
										'type'        => 'string',
										'context'     => array( 'view', 'edit' ),
										'readonly'    => true,
									),
									'value' => array(
										'description' => __( 'Meta value.', 'subscriptions-recurring-payments-for-woocommerce' ),
										'type'        => 'mixed',
										'context'     => array( 'view', 'edit' ),
										'readonly'    => true,
									),
