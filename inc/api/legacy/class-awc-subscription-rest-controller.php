 ...
 */
class AWC_Subscription_REST_Controller extends WC_REST_Orders_Controller {
	...
	/**
	 * Create AWC_Subscription object.
	 *
	 
	 * @param array $args subscription args.
	 * @return AWC_Subscription
	 */
	protected function create_base_order( $args ) {
		$subscription = awc_create_subscription( $args );

		if ( is_wp_error( $subscription ) ) {
			// translators: placeholder is an error message.
			throw new WC_REST_Exception( 'woocommerce_rest_cannot_create_subscription', sprintf( __( 'Cannot create subscription: %s.', 'subscriptions-recurring-payments-for-woocommerce' ), implode( ', ', $subscription->get_error_messages() ) ), 400 );
		}
