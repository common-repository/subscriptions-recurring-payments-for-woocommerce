 ...
 *
 * @package AWC Subscriptions
 * @subpackage WC_Subscriptions_Order
 * @category Class
 
 */
class AWC_Subscription_Renewal_Order {
	...
	public static function init() {
		...
		add_filter( 'awc_renewal_order_created', array( __CLASS__, 'add_order_note' ), 10, 2 );
		...
		add_filter( 'awc_new_order_items', array( __CLASS__, 'remove_switch_item_meta_keys' ), 10, 1 );
	}
	...
	public static function maybe_record_subscription_payment( $order_id, $orders_old_status, $orders_new_status ) {
		...
		if ( $order_completed && $order_needed_payment ) {
			...
			if ( AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
				...
				if ( apply_filters( 'awc_renewal_order_payment_update_date_created', false, $order, $subscriptions ) ) {
	...
	public static function add_order_note( $renewal_order, $subscription ) {
		...
		if ( is_a( $renewal_order, 'WC_Order' ) && awc_is_subscription( $subscription ) ) {

			// translators: %s: order number.
			$order_number = sprintf( _x( '#%s', 'hash before order number', 'subscriptions-recurring-payments-for-woocommerce' ), $renewal_order->get_order_number() );

			// translators: placeholder is order ID
			$subscription->add_order_note( sprintf( __( 'Order %s created to record renewal.', 'subscriptions-recurring-payments-for-woocommerce' ), sprintf( '<a href="%s">%s</a> ', esc_url( awc_get_edit_post_link( awc_get_objects_property( $renewal_order, 'id' ) ) ), $order_number ) ) );
		}
	...
	public static function prevent_cancelling_renewal_orders() {
		if ( isset( $_GET['cancel_order'] ) && isset( $_GET['order'] ) && isset( $_GET['order_id'] ) ) {
			...
			if ( awc_order_contains_renewal( $order ) ) {
				...
				wc_add_notice( __( 'Subscription renewal orders cannot be cancelled.', 'subscriptions-recurring-payments-for-woocommerce' ), 'notice' );
	 ...
	 * the @see AWC_Subscription_Manager::process_subscription_payment() function. Because manual payments will
	...
	public static function generate_paid_renewal_order( $user_id, $subscription_key ) {
		...
		return awc_get_objects_property( $renewal_order, 'id' );
	}
	 ...
	 * gateway calls the @see AWC_Subscription_Manager::process_subscription_payment_failure()
	 *
	 * @param int $user_id The id of the user who purchased the subscription
	 * @param string $subscription_key A subscription key of the form created by @see AWC_Subscription_Manager::get_subscription_key()
	...
	public static function generate_failed_payment_renewal_order( $user_id, $subscription_key ) {
		$renewal_order = awc_create_renewal_order( awc_get_subscription_from_key( $subscription_key ) );
	...
	public static function maybe_generate_manual_renewal_order( $user_id, $subscription_key ) {
		self::maybe_create_manual_renewal_order( awc_get_subscription_from_key( $subscription_key ) );
	}
	...
	public static function get_parent_order_id( $renewal_order ) {
		...
		return ( null === $parent_order ) ? null : awc_get_objects_property( $parent_order, 'id' );
	}
	 ...
	 *  self::get_parent_subscription() is the better function to use now as a renewal order
	 */
	public static function get_parent_order( $renewal_order ) {
		...
		$subscriptions = awc_get_subscriptions_for_renewal_order( $renewal_order );
	...
	public static function get_renewal_order_count( $order_id ) {
		...
		$subscriptions_for_order = awc_get_subscriptions_for_order( $order_id, array( 'order_type' => 'parent' ) );
	...
	public static function get_users_renewal_link( $subscription_key, $role = 'parent' ) {
		return awc_get_users_resubscribe_link( awc_get_subscription_from_key( $subscription_key ) );
	}
	...
	public static function get_users_renewal_link_for_product( $product_id ) {
		return awc_get_users_resubscribe_link_for_product( $product_id );
	}
	...
	public static function can_subscription_be_renewed( $subscription_key, $user_id = '' ) {
		return awc_can_user_resubscribe_to( awc_get_subscription_from_key( $subscription_key ), $user_id );
	}
	 ...
	 *         'checkout_renewal' bool Indicates if invoked from an interactive cart/checkout session and certain order items are not set, like taxes, shipping as they need to be set in teh calling function, like @see AWC_Subscriptions_Checkout::filter_woocommerce_create_order(). Default false.
	...
	public static function generate_renewal_order( $original_order, $product_id, $args = array() ) {
		...
		if ( ! awc_order_contains_subscription( $original_order, 'parent' ) ) {
			return false;
		}
		...
		$subscriptions = awc_get_subscriptions_for_order( $original_order, array( 'order_type' => 'parent' ) );
		$subscription  = array_shift( $subscriptions );

		if ( 'parent' == $args['new_order_role'] ) {
			$new_order = awc_create_resubscribe_order( $subscription );
		} else {
			$new_order = awc_create_renewal_order( $subscription );
		}

		return awc_get_objects_property( $new_order, 'id' );
	}
	...
	public static function is_renewal( $order, $args = array() ) {
		...
		$is_resubscribe_order = awc_order_contains_resubscribe( $order );
		$is_renewal_order     = awc_order_contains_renewal( $order );
	 ...
	 * @see AWC_Subscription_Manager::process_subscription_payments_on_order() function would
