 ...
 * 
 */
class AWC_Query extends WC_Query {
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			...
			add_filter( 'woocommerce_get_query_vars', array( $this, 'add_awc_query_vars' ) );

			// Inserting your new tab/page into the My Account page.
			add_filter( 'woocommerce_account_menu_items', array( $this, 'add_menu_items' ) );

			// Since WC 3.3.0, add_awc_query_vars() is enough for custom endpoints to work.
			if ( AWC_Subscriptions::is_woocommerce_pre( '3.3.0' ) ) {
		...
		if ( AWC_Subscriptions::is_woocommerce_pre( '3.4' ) ) {
	...
	public function get_endpoint_title( $endpoint, $action='' ) {
		...
		switch ( $endpoint ) {
			case 'view-subscription':
				...
				$title        = ( $subscription ) ? sprintf( _x( 'Subscription #%s', 'hash before order number', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription->get_order_number() ) : '';
				break;
			case 'subscriptions':
				if ( ! empty( $wp->query_vars['subscriptions'] ) ) {
					// translators: placeholder is a page number.
					$title = sprintf( __( 'Subscriptions (page %d)', 'subscriptions-recurring-payments-for-woocommerce' ), intval( $wp->query_vars['subscriptions'] ) );
				} else {
					$title = __( 'Subscriptions', 'subscriptions-recurring-payments-for-woocommerce' );
				}
				break;
			default:
				$title = '';
	...
	public function add_menu_items( $menu_items ) {
		...
		if ( 1 === count( awc_get_users_subscriptions() ) && apply_filters( 'awc_my_account_redirect_to_single_subscription', true ) ) {
			$label = __( 'Subscription', 'subscriptions-recurring-payments-for-woocommerce' );
		} else {
		...
		if ( array_key_exists( 'orders', $menu_items ) ) {
			$menu_items = awc_array_insert_after( 'orders', $menu_items, 'subscriptions', $label );
		} else {
	...
	public function maybe_redirect_to_only_subscription( $url, $endpoint ) {
		if ( $this->query_vars['subscriptions'] === $endpoint && is_account_page() ) {
			...
			if ( is_array( $subscriptions ) && 1 == count( $subscriptions ) && apply_filters( 'awc_my_account_redirect_to_single_subscription', true ) ) {
	...
	public function endpoint_content( $current_page = 1 ) {
		...
		wc_get_template( 'my-account/subscriptions.php', array( 'current_page' => $current_page ), '', plugin_dir_path( AWC_Subscriptions::$plugin_file ) . 'temp/' );
	}
	...
	protected function is_query( $query_var ) {
		...
		return apply_filters( 'awc_query_is_query', $is_view_subscription_query, $query_var );
	}
	...
	public function maybe_redirect_payment_methods( $query ) {
		...
		$subscription = awc_get_subscription( absint( $query->get( 'subscription-payment-method' ) ) );
		if ( ! $subscription ) {
			return;
		}
	...
	private function get_view_subscription_endpoint() {
		...
		if ( isset( $value ) ) {
			awc_doing_it_wrong( 'woocommerce_myaccount_view_subscriptions_endpoint', sprintf( '%1$s option is deprecated. Use %2$s option instead.', 'woocommerce_myaccount_view_subscriptions_endpoint', 'woocommerce_myaccount_view_subscription_endpoint' ), '2.2.17' );
	...
	public function add_endpoint_account_settings( $settings ) {
		$subscriptions_endpoint_setting = array(
			'title'    => __( 'Subscriptions', 'subscriptions-recurring-payments-for-woocommerce' ),
			'desc'     => __( 'Endpoint for the My Account &rarr; Subscriptions page', 'subscriptions-recurring-payments-for-woocommerce' ),
		...
		$view_subscription_endpoint_setting = array(
			'title'    => __( 'View subscription', 'subscriptions-recurring-payments-for-woocommerce' ),
			'desc'     => __( 'Endpoint for the My Account &rarr; View Subscription page', 'subscriptions-recurring-payments-for-woocommerce' ),
		...
		$subscription_payment_method_endpoint_setting = array(
			'title'    => __( 'Subscription payment method', 'subscriptions-recurring-payments-for-woocommerce' ),
			'desc'     => __( 'Endpoint for the My Account &rarr; Change Subscription Payment Method page', 'subscriptions-recurring-payments-for-woocommerce' ),
		...
		AWC_Subscriptions_Admin::insert_setting_after( $settings, 'woocommerce_myaccount_view_order_endpoint', array( $subscriptions_endpoint_setting, $view_subscription_endpoint_setting, $subscription_payment_method_endpoint_setting ), 'multiple_settings' );
		return $settings;
	}
	 ...
	 */
	public function add_awc_query_vars( $query_vars ) {
	...
	public function maybe_add_active_class( $classes, $endpoint ) {
		if ( ! isset( $classes['is-active'] ) && 'subscriptions' === $endpoint && awc_is_view_subscription_page() ) {
			$classes[] = 'is-active';
		}
