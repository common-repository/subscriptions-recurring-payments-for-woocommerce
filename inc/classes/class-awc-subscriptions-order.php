...
class AWC_Subscriptions_Order {
	 ...
	 *
	 * Similar to AWC_Subscription::get_sign_up_fee() except that it sums the sign-up fees for all subscriptions purchased in an order.
	 *
	 * @param mixed $order A WC_Order object or the ID of the order which the subscription was purchased in.
	...
	/**
	 * Gets an item by a subscription key of the form created by @see AWC_Subscription_Manager::get_subscription_key().
	 *
	 * @param WC_Order|int $order The WC_Order object or ID of the order for which the meta should be sought.
	 * @param int $product_id The product/post ID of a subscription product.
	 ...
	 * by @see AWC_Subscription_Manager::get_subscription_key().
	 *
	 * @param WC_Order|int $order The WC_Order object or ID of the order for which the meta should be sought.
	 * @param int $product_id The product/post ID of a subscription product.
	...
	public static function get_meta( $order, $meta_key, $default = 0 ) {
		...
		if ( isset( $order->$meta_key ) ) { // WC 2.1+ magic __isset() & __get() methods
			...
			$meta_value = get_post_meta( awc_get_objects_property( $order, 'id' ), '_' . $meta_key, true );
	...
	public static function subscription_thank_you( $order_id ) {
		if ( awc_order_contains_subscription( $order_id, 'any' ) ) {
			$subscriptions                = awc_get_subscriptions_for_order( $order_id, array( 'order_type' => 'any' ) );
			...
			if ( $subscription_count ) {
				foreach ( $subscriptions as $subscription ) {
					if ( ! $subscription->has_status( 'active' ) ) {
						$thank_you_message = '<p>' . _n( 'Your subscription will be activated when payment clears.', 'Your subscriptions will be activated when payment clears.', $subscription_count, 'subscriptions-recurring-payments-for-woocommerce' ) . '</p>';
						break;
					}
				}
			}
			...
			$thank_you_message .= '<p>' . sprintf( _n( 'View the status of your subscription in %1$syour account%2$s.', 'View the status of your subscriptions in %1$syour account%2$s.', $subscription_count, 'subscriptions-recurring-payments-for-woocommerce' ), '<a href="' . $my_account_subscriptions_url . '">', '</a>' ) . '</p>';
			echo wp_kses(
				apply_filters(
	...
	public static function add_contains_subscription_hidden_field( $column ) {
		...
		if ( 'order_status' == $column ) {
			$contains_subscription = awc_order_contains_subscription( $post->ID, 'parent' ) ? 'true' : 'false';
	...
	public static function contains_subscription_hidden_field( $order_id ) {
		...
		$has_subscription = awc_order_contains_subscription( $order_id, 'parent' ) ? 'true' : 'false';
	...
	*/
	public static function add_contains_subscription_column( $columns ) {
		...
		$column_header = '<span class="subscription_head tips" data-tip="' . esc_attr__( 'Subscription Relationship', 'subscriptions-recurring-payments-for-woocommerce' ) . '">' . esc_attr__( 'Subscription Relationship', 'subscriptions-recurring-payments-for-woocommerce' ) . '</span>';

		$new_columns = awc_array_insert_after( 'shipping_address', $columns, 'subscription_relationship', $column_header );
	...
	*/
	public static function add_contains_subscription_column_content( $column ) {
		...
		if ( 'subscription_relationship' == $column ) {
			if ( awc_order_contains_subscription( $post->ID, 'renewal' ) ) {
				echo '<span class="subscription_renewal_order tips" data-tip="' . esc_attr__( 'Renewal Order', 'subscriptions-recurring-payments-for-woocommerce' ) . '"></span>';
			} elseif ( awc_order_contains_subscription( $post->ID, 'resubscribe' ) ) {
				echo '<span class="subscription_resubscribe_order tips" data-tip="' . esc_attr__( 'Resubscribe Order', 'subscriptions-recurring-payments-for-woocommerce' ) . '"></span>';
			} elseif ( awc_order_contains_subscription( $post->ID, 'parent' ) ) {
				echo '<span class="subscription_parent_order tips" data-tip="' . esc_attr__( 'Parent Order', 'subscriptions-recurring-payments-for-woocommerce' ) . '"></span>';
			} else {
	...
	public static function maybe_record_subscription_payment( $order_id, $old_order_status, $new_order_status ) {
		...
		if ( awc_order_contains_subscription( $order_id, 'parent' ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				if ( $order_completed && ! $subscription->has_status( awc_get_subscription_ended_statuses() ) && ! $subscription->has_status( 'active' ) ) {
					...
					if ( abs( $new_start_date_offset ) > HOUR_IN_SECONDS ) {
						...
						if ( WC_Subscriptions_Synchroniser::subscription_contains_synced_product( $subscription ) ) {
							...
							if ( ( $trial_end > 0 && $trial_end < awc_date_to_time( $dates['start'] ) ) || ( $next_payment > 0 && $next_payment < awc_date_to_time( $dates['start'] ) ) ) {

								foreach ( $subscription->get_items() as $item ) {
									$product_id = awc_get_canonical_product_id( $item );

									if ( WC_Subscriptions_Synchroniser::is_product_synced( $product_id ) ) {
										$dates['trial_end']    = AWC_Subscription_Products::get_trial_expiration_date( $product_id, $dates['start'] );
										$dates['next_payment'] = WC_Subscriptions_Synchroniser::calculate_first_payment_date( $product_id, 'mysql', $dates['start'] );
										$dates['end']          = AWC_Subscription_Products::get_expiration_date( $product_id, $dates['start'] );
										break;
									}
								}
							}
	...
	public static function is_item_subscription( $order, $order_item ) {
		...
		$order_items_product_id = awc_get_canonical_product_id( $order_item );
		$item_is_subscription   = false;

		foreach ( awc_get_subscriptions_for_order( $order, array( 'order_type' => 'parent' ) ) as $subscription ) {
			foreach ( $subscription->get_items() as $line_item ) {
				if ( awc_get_canonical_product_id( $line_item ) == $order_items_product_id ) {
	...
	public static function order_needs_payment( $needs_payment, $order, $valid_order_statuses ) {
		...
		if ( false === $needs_payment && 0 == $order->get_total() && in_array( $order->get_status(), $valid_order_statuses ) && awc_order_contains_subscription( $order ) && self::get_recurring_total( $order ) > 0 && AWC_Settings::get_option('turn_of_automatic_payment') ) {
			$needs_payment = true;
		}
	...
	public static function add_sub_info_email( $order, $is_admin_email, $plaintext = false ) {
		...
		$subscriptions = awc_get_subscriptions_for_order( $order, array( 'order_type' => 'any' ) );

		if ( ! empty( $subscriptions ) ) {

			$template_base  = plugin_dir_path( AWC_Subscriptions::$plugin_file ) . 'temp/';
			$template = ( $plaintext ) ? 'emails/text/subscription-info.php' : 'emails/subscription-info.php';
	...
	public static function restrict_manage_subscriptions() {
		...
		<select name='shop_order_subtype' id='dropdown_shop_order_subtype'>
			<option value=""><?php esc_html_e( 'All orders types', 'subscriptions-recurring-payments-for-woocommerce' ); ?></option>
			<?php
			$order_types = apply_filters( 'woocommerce_subscriptions_order_type_dropdown', array(
				'original'    => _x( 'Original', 'An order type', 'subscriptions-recurring-payments-for-woocommerce' ),
				'parent'      => _x( 'Subscription Parent', 'An order type', 'subscriptions-recurring-payments-for-woocommerce' ),
				'renewal'     => _x( 'Subscription Renewal', 'An order type', 'subscriptions-recurring-payments-for-woocommerce' ),
				'resubscribe' => _x( 'Subscription Resubscribe', 'An order type', 'subscriptions-recurring-payments-for-woocommerce' ),
				'switch'      => _x( 'Subscription Switch', 'An order type', 'subscriptions-recurring-payments-for-woocommerce' ),
				'regular'     => _x( 'Non-subscription', 'An order type', 'subscriptions-recurring-payments-for-woocommerce' ),
			) );
	...
	public static function orders_by_type_query( $vars ) {
		...
		if ( 'shop_order' == $typenow && ! empty( $_GET['shop_order_subtype'] ) ) {
			...
			if ( 'original' == $_GET['shop_order_subtype'] || 'regular' == $_GET['shop_order_subtype'] ) {
				...
				$vars['post__in'] = awc_get_subscription_orders();
			...
			if ( 'regular' == $_GET['shop_order_subtype'] ) {
				$vars['post__not_in'] = awc_get_subscription_orders();
			}
		}
	...
	public static function add_subscriptions_to_view_order_templates( $order_id ) {
		...
		$template      = 'my-account/related-subscriptions.php';
		$subscriptions = awc_get_subscriptions_for_order( $order_id, array( 'order_type' => 'any' ) );
	...
	*/
	public static function maybe_remove_pay_action( $actions, $order ) {
		...
		if ( isset( $actions['pay'] ) && awc_order_contains_subscription( $order, array( 'any' ) ) ) {
			$subscriptions = awc_get_subscriptions_for_order( awc_get_objects_property( $order, 'id' ), array( 'order_type' => 'any' ) );

			foreach ( $subscriptions as $subscription ) {
				if ( awc_get_objects_property( $order, 'id' ) != $subscription->get_last_order( 'ids', 'any' ) ) {
					unset( $actions['pay'] );
					break;
				}
			}
	...
	public static function is_order_editable( $is_editable, $order ) {
		_deprecated_function( __METHOD__, '2.0', 'AWC_Subscription::is_editable()' );
		return $is_editable;
	}
	...
	public static function hide_order_itemmeta( $hidden_meta_keys ) {
		...
		if ( ! defined( 'awc_DEBUG' ) || true !== awc_DEBUG ) {
			...
			foreach ( $old_recurring_meta_keys as $index => $meta_key ) {
				$old_recurring_meta_keys[ $index ] = sprintf( '_awc_migrated_recurring%s', $meta_key );
			}
			...
			foreach ( $old_subscription_meta_keys as $index => $meta_key ) {
				$old_subscription_meta_keys[ $index ] = sprintf( '_awc_migrated_subscription%s', $meta_key );
			}
	...
	public static function maybe_cancel_subscription_on_full_refund( $order ) {
		...
		if ( awc_order_contains_subscription( $order, array( 'parent', 'renewal' ) ) ) {

			$subscriptions = awc_get_subscriptions_for_order( awc_get_objects_property( $order, 'id' ), array( 'order_type' => array( 'parent', 'renewal' ) ) );

			foreach ( $subscriptions as $subscription ) {
				$latest_order = $subscription->get_last_order();

				if ( awc_get_objects_property( $order, 'id' ) == $latest_order && $subscription->has_status( 'pending-cancel' ) && $subscription->can_be_updated_to( 'cancelled' ) ) {
					// translators: $1: opening link tag, $2: order number, $3: closing link tag
					$subscription->update_status( 'cancelled', wp_kses( sprintf( __( 'Subscription cancelled for refunded order %1$s#%2$s%3$s.', 'subscriptions-recurring-payments-for-woocommerce' ), sprintf( '<a href="%s">', esc_url( awc_get_edit_post_link( awc_get_objects_property( $order, 'id' ) ) ) ), $order->get_order_number(), '</a>' ), array( 'a' => array( 'href' => true ) ) ) );
				}
			}
		}
	}
	...
	public static function maybe_display_shipping_address( $needs_shipping, $hidden_shipping_methods, $order ) {
		...
		if ( ! $needs_shipping && awc_order_contains_subscription( $order ) && empty( $order_shipping_methods ) ) {
	...
	public static function maybe_autocomplete_order( $new_order_status, $order_id, $order = null ) {
		...
		if ( 0 != $order->get_subtotal() || ! awc_order_contains_subscription( $order ) ) {
			return $new_order_status;
		}
		...
		foreach ( $order->get_items() as $item ) {
			...
			if ( AWC_Subscription_Products::awc_is_subscription( $item->get_product() ) ) {
				continue;
			}
		...
		if ( awc_order_contains_resubscribe( $order ) ) {
			$new_order_status = 'completed';
		} elseif ( awc_order_contains_switch( $order ) ) {
			$all_switched = true;
	...
	public static function add_subscription_order_query_args( $query, $args ) {
		...
		if ( isset( $args['subscription_parent'] ) ) {
			...
			if ( empty( $value ) ) {
				$query['post__not_in'] = array_values( awc_get_subscription_orders() );
			} elseif ( true === $value ) {
				$query['post__in']     = array_values( awc_get_subscription_orders() );
			} elseif ( is_array( $value ) ) {
	...
	public static function order_contains_subscription( $order ) {
		return awc_order_contains_subscription( $order );
	}
	...
	public static function get_recurring_discount_total( $order, $product_id = '' ) {
		...
		$ex_tax = ( 'excl' === get_option( 'woocommerce_tax_display_cart' ) && awc_get_objects_property( $order, 'display_totals_ex_tax' ) );
		...
		$order_version = awc_get_objects_property( $order, 'version' );
	...
	/**
	 * Returns the recurring shipping price .
	 *
	 * @access	public
	 * @param WC_Order|int $order A WC_Order object or ID of a WC_Order order.
	 */
	 ...
	 * @access	public
	 * @return return requiring shipping methods lists as array
	 */
	...
	public static function get_recurring_items( $order ) {
		...
		foreach ( $order->get_items() as $item_id => $item_details ) {
			...
			$order_items_product_id = awc_get_canonical_product_id( $item_details );
	 ...
	 * @param mixed $order A WC_Order object or the ID of the order which the subscription was purchased in.
	 * @param int $product_id (optional) The post ID of the subscription WC_Product object purchased in the order. Defaults to the ID of the first product purchased in the order.
	 * @return string (subscription trail preion as day / months/ weeks etc.)
	 * @access	public
	 ...
	 *
	 *
	 * @param mixed $order A WC_Order object 
	 ...
	 *
	 * A convenience wrapper for @see AWC_Subscription_Manager::get_next_payment_date() to get the next
	...
	public static function get_next_payment_date( $order, $product_id, $deprecated = null ) {
		_deprecated_function( __METHOD__, '2.0', 'AWC_Subscription::get_date( "next_payment" )' );
	 ...
	 *
	 * A convenience wrapper for @see AWC_Subscription_Manager::get_last_payment_date() to get the next
	 * payment date for a subscription when all you have is the order and product.
	 *
	 * @param mixed $order A WC_Order object or the ID of the order which the subscription was purchased in.
	 * @param int $product_id The product/post ID of the subscription
	...
	public static function get_last_payment_date( $order, $product_id ) {
		...
		} elseif ( null !== ( $last_payment_date = awc_get_objects_property( $order, 'date_paid' ) ) ) {
			$last_payment_date = $last_payment_date->date( 'Y-m-d H:i:s' );
		} else {
			$last_payment_date = awc_get_datetime_utc_string( awc_get_objects_property( $order, 'date_created' ) ); // get_date_created() can return null, but if it does, we have an error anyway
		}
	...
	public static function calculate_next_payment_date( $order, $product_id, $type = 'mysql', $from_date = '' ) {
		...
		$next_payment = ( 'mysql' == $type && 0 != $next_payment ) ? $next_payment : awc_date_to_time( $next_payment );
	 ...
	 * @param string $subscription_key A subscription key of the form created by @see AWC_Subscription_Manager::get_subscription_key()
	...
	public static function get_order_item_totals( $total_rows, $order ) {
		_deprecated_function( __METHOD__, '2.0', 'AWC_Subscription::get_order_item_totals()' );
		return $total_rows;
	}
	...
	public static function maybe_record_order_payment( $order ) {
		...
		if ( ! awc_order_contains_renewal( $order ) ) {
	...
	public static function get_order_currency( $order ) {
		return awc_get_objects_property( $order, 'currency' );
	}
}
