...
class AWC_Subscription_Legacy extends AWC_Subscription {
	...
	public function get_date_created( $context = 'view' ) {
		...
		if ( '0000-00-00 00:00:00' != $this->post->post_date_gmt ) {
			$datetime = new AWC_DateTime( $this->post->post_date_gmt, new DateTimeZone( 'UTC' ) );
			$datetime->setTimezone( new DateTimeZone( wc_timezone_string() ) );
		} else {
			$datetime = new AWC_DateTime( $this->post->post_date, new DateTimeZone( wc_timezone_string() ) );
		}

		// Cache it in $this->schedule for backward compatibility
		if ( ! isset( $this->schedule->start ) ) {
			$this->schedule->start = awc_get_datetime_utc_string( $datetime );
		}
	...
	public function get_date_modified( $context = 'view' ) {
		...
		if ( '0000-00-00 00:00:00' != $this->post->post_modified_gmt ) {
			$datetime = new AWC_DateTime( $this->post->post_modified_gmt, new DateTimeZone( 'UTC' ) );
			$datetime->setTimezone( new DateTimeZone( wc_timezone_string() ) );
		} else {
			$datetime = new AWC_DateTime( $this->post->post_modified, new DateTimeZone( wc_timezone_string() ) );
		}
	...
	protected function get_date_prop( $date_type ) {
		...
		if ( ! isset( $this->schedule->{$date_type} ) ) {
			if ( ! is_object( $datetime ) ) {
				...
				$this->schedule->{$date_type} = awc_get_datetime_utc_string( $datetime );
			}
		}

		return awc_get_datetime_from( awc_date_to_time( $datetime ) );
	}
	...
	public function set_status( $new_status, $note = '', $manual_update = false ) {
		...
		wp_update_post(
			array(
				'ID'          => $this->get_id(),
				'post_status' => awc_maybe_prefix_key( $new_status, 'wc-' ),
			)
		);
		$this->post_status = $this->post->post_status = awc_maybe_prefix_key( $new_status, 'wc-' );
	...
	protected function set_date_prop( $date_type, $value ) {
		$datetime = awc_get_datetime_from( $value );
		$date     = ! is_null( $datetime ) ? awc_get_datetime_utc_string( $datetime ) : 0;
	...
	protected function set_last_order_date( $date_type, $date = null ) {
		...
		if ( $last_order ) {
			...
			$datetime = awc_get_datetime_from( $date );
			...
			switch ( $date_type ) {
				...
				case 'date_modified':
					wp_update_post( array(
						...
						'post_modified_gmt' => awc_get_datetime_utc_string( $datetime ),
					) );
				break;
				...
				case 'date_created':
					wp_update_post( array(
						...
						'post_date_gmt' => awc_get_datetime_utc_string( $datetime ),
					) );
				break;
			}
		}
	...
	public function set_date_created( $date = null ) {
		...
		if ( ! is_null( $date ) ) {
			...
			$datetime_string = awc_get_datetime_utc_string( awc_get_datetime_from( $date ) );
