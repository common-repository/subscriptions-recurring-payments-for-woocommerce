<?php
/**
 * Download Handler for Aco WooCommerce Subscriptions
 *
...
class AWC_Download_Handler {
	...
	public static function maybe_revoke_immediate_access( $grant_access, $download_id, $product_id, $order ) {
		...
		if ( 'yes' == get_option( AWC_Subscriptions_Admin::$option_prefix . '_drip_downloadable_content_on_renewal', 'no' ) && ( awc_is_subscription( awc_get_objects_property( $order, 'id' ) ) || awc_order_contains_subscription( $order, 'any' ) ) ) {
			$grant_access = false;
		}
		return $grant_access;
	}
	...
	public static function save_downloadable_product_permissions( $order_id ) {
		...
		if ( awc_order_contains_subscription( $order, 'any' ) ) {
			$subscriptions = awc_get_subscriptions_for_order( $order, array( 'order_type' => array( 'any' ) ) );
		} else {
			return;
		}
	...
	public static function get_item_downloads( $files, $item, $order ) {
		...
		if ( awc_order_contains_subscription( $order, array( 'parent', 'renewal', 'switch' ) ) ) {
			$subscriptions = awc_get_subscriptions_for_order( $order, array( 'order_type' => array( 'parent', 'renewal', 'switch' ) ) );
		} else {
			return $files;
		}
		...
		foreach ( $subscriptions as $subscription ) {
			foreach ( $subscription->get_items() as $subscription_item ) {
				if ( awc_get_canonical_product_id( $subscription_item ) === $product_id ) {
	...
	public static function grant_new_file_product_permissions( $product_id, $variation_id, $downloadable_files ) {
		...
		$existing_download_ids = array_keys( (array) awc_get_objects_property( $product, 'downloads' ) );
		...
		if ( ! empty( $new_download_ids ) ) {
			...
			$subscriptions        = awc_get_product_subscriptions( $product_id );
			...
			foreach ( $subscriptions as $subscription_id ) {
				...
				if ( ! in_array( $subscription_id, array_keys( $permissions_by_order_id ) ) || false === AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
