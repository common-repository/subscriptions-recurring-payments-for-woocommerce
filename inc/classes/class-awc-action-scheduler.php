 ...
 *
 * @class     awc_Action_Scheduler
 * @package   WooCommerce Subscriptions/Classes
 * 
 */
class AWC_Action_Scheduler extends AWC_Scheduler {

	/**
	 * An internal cache of action hooks and corresponding date types.
	 *
	 * This variable will be removed completely in the future. You should use AWC_Action_Scheduler::get_scheduled_action_hook() and awc_Action_Scheduler::get_date_types_to_schedule() instead.
	 *
	...
	protected $action_hooks = array(
		...
		'awc_subscription_scheduled_payment_retry' => 'payment_retry',
	...
	public function update_date( $subscription, $date_type, $datetime ) {
		...
		if ( in_array( $date_type, $this->get_date_types_to_schedule() ) ) {
			...
			if ( ! empty( $action_hook ) ) {
				...
				$timestamp      = awc_date_to_time( $datetime );
	 ...
	 *
	 * @param object $subscription An instance of a AWC_Subscription object
		...
		switch ( $new_status ) {
			case 'active':
				...
				foreach ( $this->get_date_types_to_schedule() as $date_type ) {
					...
					// If there's no payment retry date, avoid calling get_action_args() because it calls the resource intensive AWC_Subscription::get_last_order() / get_related_orders()
			...
			case 'pending-cancel':
				...
				// The end date was set in AWC_Subscriptions::update_dates() to the appropriate value, so we can schedule our action for that time
	 ...
	 *
	 * @param object $subscription An instance of AWC_Subscription to get the hook for
	...
	protected function get_scheduled_action_hook( $subscription, $date_type ) {
		...
		switch ( $date_type ) {
			case 'next_payment':
				...
				break;
			case 'payment_retry':
				$hook = 'awc_subscription_scheduled_payment_retry';
				break;
			case 'trial_end':
