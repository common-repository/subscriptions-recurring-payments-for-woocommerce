...
if(!class_exists('AWC_Initial_Cart_Stock_Manager')){
	class AWC_Initial_Cart_Stock_Manager extends AWC_Renewal_Cart_Stock_Manager {
		...
		*/
		public static function attach_callbacks() {
			...
			remove_action( 'awc_before_renewal_setup_cart_subscription', 'awc_Renewal_Cart_Stock_Manager::maybe_adjust_stock_cart', 10, 2 );
			add_action( 'awc_before_parent_order_setup_cart', array( get_called_class(), 'maybe_adjust_stock_cart' ), 10, 2 );
		}
		...
		*/
		protected static function get_order_from_query_vars() {
			...
			if ( isset( $wp->query_vars['order-pay'] ) ) {
				...
				if ( static::has_handled_stock( $order ) && awc_order_contains_subscription( $order, 'parent' ) ) {
					$parent_order = $order;
				}
			}
