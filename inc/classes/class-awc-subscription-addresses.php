 ...
 *
 */
class AWC_Subscription_Addresses {
	...
	public static function init() {
		...
		add_filter( 'awc_view_details_subscription_myaccount_actions', __CLASS__ . '::add_edit_address_subscription_action', 10, 2 );
	...
	public static function add_edit_address_subscription_action( $actions, $subscription ) {
		...
		if ( $subscription->needs_shipping_address() && $subscription->has_status( array( 'active', 'on-hold' ) ) ) {
			$actions['change_address'] = array(
				...
				'name' => __( 'Change address', 'subscriptions-recurring-payments-for-woocommerce' ),
			);
		}
	...
	public static function maybe_add_edit_address_checkbox() {
		...
		if ( awc_user_has_subscription() ) {

			if ( isset( $_GET['subscription'] ) ) {

				echo '<p>' . esc_html__( 'Both the shipping address used for the subscription and your default shipping address for future purchases will be updated.', 'subscriptions-recurring-payments-for-woocommerce' ) . '</p>';
				...
				$label = sprintf( esc_html__( 'Update the %1$s used for %2$sall%3$s of my active subscriptions', 'subscriptions-recurring-payments-for-woocommerce' ), awc_get_address_type_to_display( $address_type ), '<strong>', '</strong>' );
				...
				woocommerce_form_field(
					...
					array(
						...
						'label'   => $label,
						'default' => apply_filters( 'awc_update_all_subscriptions_addresses_checked', false ),
					)
				);
			}

			wp_nonce_field( 'awc_edit_address', '_asubnonce' );
	...
	public static function maybe_update_subscription_addresses( $user_id, $address_type ) {
		...
		if ( ! awc_user_has_subscription( $user_id ) || wc_notice_count( 'error' ) > 0 || empty( $_POST['_asubnonce'] ) || ! wp_verify_nonce( $_POST['_asubnonce'], 'awc_edit_address' ) ) {
			return;
		}
		...
		if ( isset( $_POST['update_all_subscriptions_addresses'] ) ) {
			...
			$subscription = awc_get_subscription( intval( $_POST['update_subscription_address'] ) );
	...
	public static function maybe_populate_subscription_addresses( $address ) {
		...
		if ( isset( $_GET['subscription'] ) ) {
			$subscription = awc_get_subscription( absint( $_GET['subscription'] ) );
	...
	public static function change_addresses_breadcrumb( $crumbs ) {
		if ( isset( $_GET['subscription'] ) && is_wc_endpoint_url() && 'edit-address' === WC()->query->get_current_endpoint() ) {
			...
			$crumbs[3] = array(
				...
				sprintf( _x( 'Change %s address', 'change billing or shipping address', 'subscriptions-recurring-payments-for-woocommerce' ), $wp_query->query_vars['edit-address'] ),
				'',
			);
		}
