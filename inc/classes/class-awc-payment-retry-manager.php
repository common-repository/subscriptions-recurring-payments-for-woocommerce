...
class AWC_Payment_Retry_Manager {
	...
	/* property to store the instance of AWC_Retry_Admin */
	protected static $admin;

	/**
	 * Background updater to process retries from old store.
	 *
	 * @var AWC_Retry_Background_Migrator
	 */
	...
	public static function init() {
		self::$setting_id = AWC_Subscriptions_Admin::$option_prefix . '_enable_retry';
		self::$admin      = new AWC_Retry_Admin( self::$setting_id );

		if ( self::is_retry_enabled() ) {
			AWC_Retry_Email::init();
			...
			add_filter( 'awc_subscription_dates_return', __CLASS__ . '::add_retry_date_type' );
			...
			add_action( 'awc_subscription_scheduled_payment_retry', __CLASS__ . '::maybe_retry_payment' );
			...
			if ( ! self::$table_maker ) {
				self::$table_maker = new AWC_Retry_Table_Maker();
	...
	public static function check_order_statuses_for_payment( $statuses, $order = null ) {
		...
		$last_retry = self::store()->get_last_retry_for_order( awc_get_objects_property( $order, 'id' ) );
		if ( $last_retry ) {
	...
	public static function is_retry_enabled() {
		return (bool) apply_filters( 'awc_is_retry_enabled', AWC_Settings::get_option('retry_failed_payment') );
	}
	...
	public static function add_retry_date_type( $subscription_date_types ) {
		...
		$subscription_date_types = awc_array_insert_after( 'next_payment', $subscription_date_types, 'payment_retry', _x( 'Renewal Payment Retry', 'table heading', 'subscriptions-recurring-payments-for-woocommerce' ) );
	...
	public static function maybe_cancel_retry( $subscription, $new_status, $old_status ) {
		...
		if ( $subscription->get_date( 'payment_retry' ) > 0 ) {
			...
			$last_retry = ( $last_order ) ? self::store()->get_last_retry_for_order( awc_get_objects_property( $last_order, 'id' ) ) : null;
	 ...
	 *
	 * @param object $retry An instance of a awc_Retry object
	...
	public static function maybe_delete_payment_retry_date( $retry, $new_status ) {
		if ( ! in_array( $new_status, array( 'pending', 'processing' ) ) ) {
			...
			if ( $retry->get_id() === $last_retry->get_id() ) {
				foreach ( awc_get_subscriptions_for_renewal_order( $retry->get_order_id() ) as $subscription ) {
					$subscription->delete_date( 'payment_retry' );
				}
			}
		}
	...
	public static function maybe_apply_retry_rule( $subscription, $last_order ) {
		...
		$retry_count = self::store()->get_retry_count_for_order( awc_get_objects_property( $last_order, 'id' ) );

		if ( self::rules()->has_rule( $retry_count, awc_get_objects_property( $last_order, 'id' ) ) ) {

			$retry_rule = self::rules()->get_rule( $retry_count, awc_get_objects_property( $last_order, 'id' ) );

			do_action( 'woocommerce_subscriptions_before_apply_retry_rule', $retry_rule, $last_order, $subscription );

			$retry_id = self::store()->save( new awc_Retry( array(
				'status'   => 'pending',
				'order_id' => awc_get_objects_property( $last_order, 'id' ),
			...
			foreach ( array( 'order' => $last_order, 'subscription' => $subscription ) as $object_key => $object ) { // phpcs:ignore WordPress.Arrays.ArrayDeclarationSpacing.AssociativeArrayFound
				...
				if ( '' !== $new_status && ! $object->has_status( $new_status ) ) {
					$object->update_status( $new_status, _x( 'Retry rule applied:', 'used in order note as reason for why status changed', 'subscriptions-recurring-payments-for-woocommerce' ) );
				}
			}

			if ( $retry_rule->get_retry_interval() > 0 ) {
				// by calling this after changing the status, this will also schedule the 'awc_subscription_scheduled_payment_retry' action.
	...
	public static function maybe_reapply_last_retry_rule( $subscription, $last_order ) {
		...
		foreach ( array( 'order' => $last_order, 'subscription' => $subscription ) as $object_type => $object ) { // phpcs:ignore WordPress.Arrays.ArrayDeclarationSpacing.AssociativeArrayFound
			...
			if ( '' !== $new_status && ! $object->has_status( $new_status ) ) {
				$object->update_status( $new_status, _x( 'Retry rule reapplied:', 'used in order note as reason for why status changed', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}
		}
	}
	...
	public static function maybe_retry_payment( $last_order ) {
		...
		$subscriptions = awc_get_subscriptions_for_renewal_order( $last_order );
		$last_retry    = self::store()->get_last_retry_for_order( awc_get_objects_property( $last_order, 'id' ) );
		...
		if ( null !== $last_retry && 'pending' === $last_retry->get_status() ) {
			...
			if ( $last_order->needs_payment() ) {
				...
				if ( $valid_order_status && $valid_subscription_status ) {
					...
					$last_order->update_status( 'pending', _x( 'Subscription renewal payment retry:', 'used in order note as reason for why order status changed', 'subscriptions-recurring-payments-for-woocommerce' ), true );
					...
					foreach ( $subscriptions as $subscription ) {
						...
						$subscription->update_status( 'on-hold', _x( 'Subscription renewal payment retry:', 'used in order note as reason for why subscription status changed', 'subscriptions-recurring-payments-for-woocommerce' ) );
					...
					if ( awc_order_contains_manual_subscription( $last_order, 'renewal' ) ) {
						...
						throw new Exception( __( 'Payment retry attempted on renewal order with multiple related subscriptions with no payment method in common.', 'subscriptions-recurring-payments-for-woocommerce' ) );
					} else {
						...
						awc_copy_payment_method_to_order( $subscription, $last_order );
						$last_order->save();

						AWC_Subscription_Payment_Gateways::trigger_gateway_renewal_payment_hook( $last_order );

						// Now that we've attempted to process the payment, refresh the order
						$last_order = wc_get_order( awc_get_objects_property( $last_order, 'id' ) );
					}
	...
	*/
	...
	public static function load_dependant_classes() {
		if ( ! self::$background_migrator ) {
			self::$background_migrator = new AWC_Retry_Background_Migrator( wc_get_logger() );
	...
	public static function upgrade( $new_version, $old_version ) {
		...
		if ( version_compare( $new_version, '2.4.0', '>' ) ) {
			AWC_Retry_Migrator::set_needs_migration();
		}
	}

	/**
	 * Is `woocommerce_scheduled_subscription_payment` or `awc_subscription_scheduled_payment_retry` current action?
	 *
	 * @return boolean
	 *
	...
	protected static function is_scheduled_payment_attempt() {
		 ...
		 *
		 * @param boolean doing_action( 'woocommerce_scheduled_subscription_payment' ) || doing_action( 'awc_subscription_scheduled_payment_retry' )
		 
		 */
		return (bool) apply_filters( 'awc_is_scheduled_payment_attempt', doing_action( 'woocommerce_scheduled_subscription_payment' ) || doing_action( 'awc_subscription_scheduled_payment_retry' ) );
	}
	...
	public static function store() {
		if ( empty( self::$store ) ) {
			if ( ! did_action( 'plugins_loaded' ) ) {
				awc_doing_it_wrong( __METHOD__, 'This method was called before the "plugins_loaded" hook. It applies a filter to the retry data store instantiated. For that to work, it should first be called after all plugins are loaded.', '2.4.1' );
			}
	...
	protected static function get_store_class() {
		$default_store_class = 'AWC_Retry_Database_Store';
		if ( awc_Retry_Migrator::needs_migration() ) {
			$default_store_class = 'awc_Retry_Hybrid_Store';
		}

		return apply_filters( 'awc_retry_store_class', $default_store_class );
	}
	...
	protected static function get_rules_class() {
		return apply_filters( 'awc_retry_rules_class', 'awc_Retry_Rules' );
	}
