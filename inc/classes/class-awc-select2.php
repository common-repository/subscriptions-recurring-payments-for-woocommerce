...
class AWC_Select2 {
	...
	protected function get_property_name( $property ) {
		$data_properties = AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ? array( 'placeholder', 'selected', 'allow_clear' ) : array( 'placeholder', 'allow_clear' );
	...
	protected function attributes_to_html( array $attributes ) {
		...
		foreach ( $attributes as $property => $value ) {
			if ( ! is_scalar( $value ) ) {
				$value = awc_json_encode( $value );
			}

			$html[] = $this->get_property_name( $property ) . '="' . esc_attr( $value, 'subscriptions-recurring-payments-for-woocommerce' ) . '"';
		}
