...
class AWC_Subscription extends WC_Order {
	...
	public function __isset( $key ) {
		...
		if ( ! AWC_Subscriptions::is_woocommerce_pre( '3.0' ) && in_array( $key, $this->deprecated_properties ) ) {
	...
	public function __set( $key, $value ) {
		...
		if ( in_array( $key, $this->deprecated_properties ) ) {
			...
			switch ( $key ) {
				...
				case 'order':
					$function = 'AWC_Subscription::set_parent_id( $order_id )';
					$this->set_parent_id( awc_get_objects_property( $value, 'id' ) );
					$this->order = $value;
					break;

				case 'requires_manual_renewal':
					$function = 'AWC_Subscription::set_requires_manual_renewal()';
					$this->set_requires_manual_renewal( $value );
					break;

				case 'payment_gateway':
					$function = 'AWC_Subscription::set_payment_method()';
					$this->set_payment_method( $value );
					break;

				case 'suspension_count':
					$function = 'AWC_Subscription::set_suspension_count()';
					$this->set_suspension_count( $value );
					break;

				default:
					$function = 'AWC_Subscription::update_dates()';
					$this->update_dates( array( $key => $value ) );
					break;
			}

			if ( ! AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
				awc_doing_it_wrong( $key, sprintf( 'Subscription properties should not be set directly as WooCommerce 3.0 no longer supports direct property access. Use %s instead.', $function ), '2.2.0' );
			}
		} else {
	...
	public function __get( $key ) {
		...
		if ( in_array( $key, $this->deprecated_properties ) ) {
			...
			switch ( $key ) {
				...
				case 'order':
					$function = 'AWC_Subscription::get_parent()';
					$value    = $this->get_parent();
					break;

				case 'requires_manual_renewal':
					$function = 'AWC_Subscription::get_requires_manual_renewal()';
				...
				case 'suspension_count':
					$function = 'AWC_Subscription::get_suspension_count()';
					$value    = $this->get_suspension_count();
					break;

				default:
					$function = 'AWC_Subscription::get_date( ' . $key . ' )';
					$value    = $this->get_date( $key );
					break;
			}

			if ( ! AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
				awc_doing_it_wrong( $key, sprintf( 'Subscription properties should not be accessed directly as WooCommerce 3.0 no longer supports direct property access. Use %s instead.', $function ), '2.2.0' );
			}
		} else {
	...
	public function payment_method_supports( $payment_gateway_feature ) {
		...
		return apply_filters( 'awc_subscription_payment_gateway_supports', $payment_gateway_supports, $payment_gateway_feature, $this );
	}
	...
	public function can_be_updated_to( $new_status ) {
		...
		switch ( $new_status ) {
			...
			case 'on-hold':
				...
				break;
			case 'cancelled':
				if ( $this->payment_method_supports( 'subscription_cancellation' ) && ( $this->has_status( 'pending-cancel' ) || ! $this->has_status( awc_get_subscription_ended_statuses() ) ) ) {
					$can_be_updated = true;
				} else {
					$can_be_updated = false;
				}
			...
			case 'expired':
				...
				break;
			case 'trash':
				if ( $this->has_status( awc_get_subscription_ended_statuses() ) || $this->can_be_updated_to( 'cancelled' ) ) {
					$can_be_updated = true;
				} else {
					$can_be_updated = false;
				}
	...
	public function update_status( $new_status, $note = '', $manual = false ) {
		...
		if ( $new_status !== $old_status || ! in_array( $old_status_key, array_keys( awc_get_subscription_statuses() ) ) ) {
			...
			if ( ! $this->can_be_updated_to( $new_status ) ) {
				...
				$message = sprintf( __( 'Unable to change subscription status to "%s".', 'subscriptions-recurring-payments-for-woocommerce' ), $new_status );
			...
			try {
				...
				switch ( $new_status ) {
					...
					case 'pending-cancel':
						...
						if ( 0 == $end_date || awc_date_to_time( $end_date ) < current_time( 'timestamp', true ) ) {
					...
					case 'active':
						...
						awc_make_user_active( $this->get_user_id() );
					break;
				...
				$log->add( 'asub-update-status-failures', $log_entry );
				...
				$this->add_order_note( sprintf( __( 'Unable to change subscription status to "%1$s". Exception: %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), $new_status, $e->getMessage() ) );
		...
		try {
			...
			if ( ! empty( $status_transition['from'] ) ) {
				$transition_note = sprintf(
					...
					__( 'Status changed from %1$s to %2$s.', 'subscriptions-recurring-payments-for-woocommerce' ),
					awc_get_subscription_status_name( $status_transition['from'] ),
					awc_get_subscription_status_name( $status_transition['to'] )
				);
				...
				$transition_note = sprintf( __( 'Status set to %s.', 'subscriptions-recurring-payments-for-woocommerce' ), awc_get_subscription_status_name( $status_transition['to'] ) );
			}
			...
			$this->add_order_note( __( 'Error during subscription status transition.', 'subscriptions-recurring-payments-for-woocommerce' ) . ' ' . $e->getMessage() );
		}
	...
	public function is_manual() {
		...
		if ( AWC_Subscriptions::is_duplicate_site() || true === $this->get_requires_manual_renewal() || false === wc_get_payment_gateway_by_order( $this ) ) {
			$is_manual = true;
		} else {
			$is_manual = false;
		}
	...
	public function get_valid_statuses() {
		return array_keys( awc_get_subscription_statuses() );
	}
	...
	public function get_payment_count( $payment_type = 'completed', $order_types = '' ) {
		...
		switch ( $payment_type ) {
			case 'completed':
				...
				/**
				 * Previously the @see AWC_Subscription::get_completed_payment_count() function would filter the completed renewal and parent order count as a combined total.
	...
	public function get_date( $date_type, $timezone = 'gmt' ) {
		...
		$date_type = awc_normalise_date_type_key( $date_type, true );
		...
		if ( empty( $date_type ) ) {
			...
			switch ( $date_type ) {
				case 'date_created':
					...
					$date = is_null( $date ) ? awc_get_datetime_from( get_the_date( 'Y-m-d H:i:s', $this->get_id() ) ) : $date; // When a post is first created via the Add Subscription screen, it has a post_date but not a date_created value yet
					break;
				case 'date_modified':
	 ...
	 * @return AWC_DateTime|NULL object if the date is set or null if there is no date.
	 */
	...
	protected function get_date_prop_key( $date_type ) {
		$prefixed_date_type = awc_maybe_prefix_key( $date_type, 'schedule_' );
	...
	protected function get_related_orders_date( $date_type, $order_type = 'any' ) {
		...
		if ( 'last' === $order_type ) {
			...
			$date       = ( ! $last_order ) ? null : awc_get_objects_property( $last_order, $date_type );
		} else {
			...
			foreach ( $this->get_related_orders( 'all', $order_type ) as $related_order_id ) {
				...
				$date          = ( ! $related_order ) ? null : awc_get_objects_property( $related_order, $date_type );
				if ( is_a( $date, 'AWC_Datetime' ) ) {
					break;
				}
			}
		}
	...
	public function get_date_to_display( $date_type = 'next_payment' ) {
		...
		if ( $timestamp_gmt > 0 ) {
			...
			switch ( $date_type ) {
				case 'end':
					$date_to_display = __( 'Not yet ended', 'subscriptions-recurring-payments-for-woocommerce' );
					break;
				case 'cancelled':
					$date_to_display = __( 'Not cancelled', 'subscriptions-recurring-payments-for-woocommerce' );
					break;
				case 'next_payment':
				case 'trial_end':
				default:
					$date_to_display = _x( '-', 'original denotes there is no date to display', 'subscriptions-recurring-payments-for-woocommerce' );
					break;
			}
		}
	...
	public function get_time( $date_type, $timezone = 'gmt' ) {
		...
		$datetime = awc_date_to_time( $datetime );
	...
	public function update_dates( $dates, $timezone = 'gmt' ) {
		...
		foreach ( $dates as $date_type => $datetime ) {
			...
			$utc_timestamp = ( 0 === $datetime ) ? null : awc_date_to_time( $datetime );
	...
	public function delete_date( $date_type ) {
		...
		switch ( $date_type ) {
			case 'date_created':
				$message = __( 'The creation date of a subscription can not be deleted, only updated.', 'subscriptions-recurring-payments-for-woocommerce' );
			break;
			case 'start':
				$message = __( 'The start date of a subscription can not be deleted, only updated.', 'subscriptions-recurring-payments-for-woocommerce' );
			break;
			case 'last_order_date_created':
			case 'last_order_date_modified':
				// translators: %s: date type (e.g. "trial_end").
				$message = sprintf( __( 'The %s date of a subscription can not be deleted. You must delete the order.', 'subscriptions-recurring-payments-for-woocommerce' ), $date_type );
			break;
			default:
				$message = '';
			break;
		...
		if ( ! empty( $message ) ) {
			...
			throw new Exception( sprintf( __( 'Subscription #%d: ', 'subscriptions-recurring-payments-for-woocommerce' ), $this->get_id() ) . $message );
		}
	...
	public function can_date_be_updated( $date_type ) {
		...
		switch ( $date_type ) {
			...
			case 'trial_end':
				...
				if ( $this->get_payment_count() < 2 && ! $this->has_status( awc_get_subscription_ended_statuses() ) && ( $this->has_status( 'pending' ) || $this->payment_method_supports( 'subscription_date_changes' ) ) ) {
					$can_date_be_updated = true;
				} else {
					$can_date_be_updated = false;
				}
			...
			case 'next_payment':
			case 'end':
				if ( ! $this->has_status( awc_get_subscription_ended_statuses() ) && ( $this->has_status( 'pending' ) || $this->payment_method_supports( 'subscription_date_changes' ) ) ) {
					$can_date_be_updated = true;
				} else {
					$can_date_be_updated = false;
				}
	...
	protected function calculate_next_payment_date() {
		...
		if ( $trial_end_time > current_time( 'timestamp', true ) ) {
			...
			} elseif ( $last_payment_time > $start_time && apply_filters( 'awc_calculate_next_payment_from_last_payment', true, $this ) ) {
			...
			$next_payment_timestamp = awc_add_time( $this->get_billing_interval(), $this->get_billing_period(), $from_timestamp, 'offset_site_time' );
			...
			while ( $next_payment_timestamp < ( current_time( 'timestamp', true ) + 2 * HOUR_IN_SECONDS ) && $i < 3000 ) {
				$next_payment_timestamp = awc_add_time( $this->get_billing_interval(), $this->get_billing_period(), $next_payment_timestamp, 'offset_site_time' );
				$i += 1;
			}
		}
	...
	public function get_formatted_line_subtotal( $item, $tax_display = '' ) {
		...
		if ( $this->is_one_payment() ) {
			...
			$subtotal = awc_price_string( $this->get_price_string_details( $line_subtotal ) );
	...
	public function get_formatted_order_total( $tax_display = '', $display_refunded = true ) {
		if ( $this->get_total() > 0 && '' !== $this->get_billing_period() && ! $this->is_one_payment() ) {
			$formatted_order_total = awc_price_string( $this->get_price_string_details( $this->get_total() ) );
		} else {
	...
	/**
	 * Get the details of the subscription for use with @see awc_price_string()
	 *
	...
	public function payment_complete( $transaction_id = '' ) {
		...
		if ( AWC_Subscription_Change_Payment_Gateway::$is_request_to_change_payment ) {
			return;
		}
	...
	public function payment_complete_for_order( $last_order ) {
		...
		awc_update_users_role( $this->get_user_id(), 'default_subscriber_role' );

		// Add order note depending on initial payment
		$this->add_order_note( __( 'Payment status marked complete.', 'subscriptions-recurring-payments-for-woocommerce' ) );

		$this->update_status( 'active' ); // also saves the subscription

		do_action( 'awc_subscription_payment_complete', $this );

		if ( false !== $last_order && awc_order_contains_renewal( $last_order ) ) {
	...
	public function payment_failed( $new_status = 'on-hold' ) {
		...
		if ( false !== $last_order && false === $last_order->has_status( 'failed' ) ) {
			remove_filter( 'woocommerce_order_status_changed', 'AWC_Subscription_Renewal_Order::maybe_record_subscription_payment' );
			$last_order->update_status( 'failed' );
			add_filter( 'woocommerce_order_status_changed', 'AWC_Subscription_Renewal_Order::maybe_record_subscription_payment', 10, 3 );
		}

		// Log payment failure on order
		$this->add_order_note( __( 'Payment failed.', 'subscriptions-recurring-payments-for-woocommerce' ) );
		...
		if ( 'cancelled' == $new_status || apply_filters( 'woocommerce_subscription_max_failed_payments_exceeded', false, $this ) ) {
			if ( $this->can_be_updated_to( 'cancelled' ) ) {
				$this->update_status( 'cancelled', __( 'Subscription Cancelled: maximum number of failed payments reached.', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}
	...
	public function get_related_orders_query( $subscription_id ) {
		_deprecated_function( __METHOD__, '2.3.0', 'awc_Subscription_Data_Store_CPT::get_related_order_ids( awc_get_subscription( $subscription_id ) ) via awc_Subscription_Data_Store::instance()' );
		return awc_Related_Order_Store::instance()->get_related_order_ids( awc_get_subscription( $subscription_id ), 'renewal' );
	}
	...
	protected function get_related_order_ids( $order_type = 'any' ) {
		...
		if ( 'parent' !== $order_type ) {
			...
			foreach ( $relation_types as $relation_type ) {
				$related_order_ids = array_merge( $related_order_ids, awc_Related_Order_Store::instance()->get_related_order_ids( $this, $relation_type ) );
			}
		}
	...
	public function get_payment_method_to_display( $context = 'admin' ) {
		...
		if ( $this->is_manual() ) {
			...
			$payment_method_to_display = __( 'Manual Renewal', 'subscriptions-recurring-payments-for-woocommerce' );
		...
		if ( 'customer' === $context ) {
			...
			$payment_method_to_display = sprintf( __( 'Via %s', 'subscriptions-recurring-payments-for-woocommerce' ), $payment_method_to_display );
	...
	protected function set_payment_method_meta( $payment_method_id, $payment_meta ) {
		...
		awc_set_payment_meta( $this, $payment_meta );
	}
	...
	public function get_view_order_url() {
		...
		return apply_filters( 'awc_get_view_subscription_url', $view_subscription_url, $this->get_id() );
	}
	...
	public function get_items_sign_up_fee( $line_item, $tax_inclusive_or_exclusive = 'exclusive_of_tax' ) {
		...
		if ( ! is_object( $line_item ) ) {
			$line_item = awc_get_order_item( $line_item, $this );
		}
	...
	public function is_one_payment() {
		...
		if ( 0 != ( $end_time = $this->get_time( 'end' ) ) ) {
			...
			$next_payment_timestamp = awc_add_time( $this->get_billing_interval(), $this->get_billing_period(), $from_timestamp );
	...
	public function get_item_downloads( $item ) {
		...
		if ( ! AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
			_deprecated_function( __METHOD__, '2.2.0', 'WC_Order_Item_Product::get_item_downloads(), because WooCommerce 3.0+ now uses that' );
		}
	...
	public function validate_date_updates( $dates, $timezone = 'gmt' ) {
		...
		if ( ! is_array( $dates ) ) {
			throw new InvalidArgumentException( __( 'Invalid format. First parameter needs to be an array.', 'subscriptions-recurring-payments-for-woocommerce' ) );
		}

		if ( empty( $dates ) ) {
			throw new InvalidArgumentException( __( 'Invalid data. First parameter was empty when passed to update_dates().', 'subscriptions-recurring-payments-for-woocommerce' ) );
		}

		$passed_date_keys = array_map( 'awc_normalise_date_type_key', array_keys( $dates ) );
		$extra_keys       = array_diff( $passed_date_keys, $this->get_valid_date_types() );

		if ( ! empty( $extra_keys ) ) {
			throw new InvalidArgumentException( __( 'Invalid data. First parameter has a date that is not in the registered date types.', 'subscriptions-recurring-payments-for-woocommerce' ) );
		}
		...
		foreach ( $this->get_valid_date_types() as $date_type ) {
			...
			if ( isset( $dates[ $date_type ] ) ) {
				...
				if ( ! empty( $datetime ) && false === awc_is_datetime_mysql_format( $datetime ) ) {
					// translators: placeholder is date type (e.g. "end", "next_payment"...)
					throw new InvalidArgumentException( sprintf( _x( 'Invalid %s date. The date must be of the format: "Y-m-d H:i:s".', 'appears in an error message if date is wrong format', 'subscriptions-recurring-payments-for-woocommerce' ), $date_type ) );
				}
				...
				if ( empty( $datetime ) ) {
					...
					$timestamps[ $date_type ] = awc_date_to_time( $datetime );
				}
			...
			switch ( $date_type ) {
				case 'end':
					if ( array_key_exists( 'cancelled', $timestamps ) && $timestamp < $timestamps['cancelled'] ) {
						...
						$messages[] = sprintf( __( 'The %s date must occur after the cancellation date.', 'subscriptions-recurring-payments-for-woocommerce' ), $date_type );
					}

				case 'cancelled':
					if ( array_key_exists( 'last_order_date_created', $timestamps ) && $timestamp < $timestamps['last_order_date_created'] ) {
						// translators: %s: date type (e.g. "end").
						$messages[] = sprintf( __( 'The %s date must occur after the last payment date.', 'subscriptions-recurring-payments-for-woocommerce' ), $date_type );
					}

					if ( array_key_exists( 'next_payment', $timestamps ) && $timestamp <= $timestamps['next_payment'] ) {
						// translators: %s: date type (e.g. "end").
						$messages[] = sprintf( __( 'The %s date must occur after the next payment date.', 'subscriptions-recurring-payments-for-woocommerce' ), $date_type );
					}
				case 'next_payment':
					...
					if ( array_key_exists( 'trial_end', $timestamps ) && $timestamp < $timestamps['trial_end'] ) {
						// translators: %s: date type (e.g. "end").
						$messages[] = sprintf( __( 'The %s date must occur after the trial end date.', 'subscriptions-recurring-payments-for-woocommerce' ), $date_type );
					}
				case 'trial_end':
					if ( ! in_array( $date_type, array( 'end', 'cancelled' ) ) && $timestamp <= $timestamps['start'] ) {
						// translators: %s: date type (e.g. "next_payment").
						$messages[] = sprintf( __( 'The %s date must occur after the start date.', 'subscriptions-recurring-payments-for-woocommerce' ), $date_type );
					}
			}
		...
		if ( $this->object_read && ! empty( $messages ) ) {
			...
			throw new Exception( sprintf( __( 'Subscription #%d: ', 'subscriptions-recurring-payments-for-woocommerce' ), $this->get_id() ) . join( ' ', $messages ) );
		}
	...
	public function add_product( $product, $qty = 1, $args = array() ) {
		...
		if ( $item_id && $product->backorders_require_notification() && $product->is_on_backorder( $qty ) ) {
			wc_delete_order_item_meta( $item_id, apply_filters( 'woocommerce_backordered_item_meta_name', __( 'Backordered', 'subscriptions-recurring-payments-for-woocommerce' ) ) );
		}
	...
	protected function get_valid_date_types() {
		...
		if ( empty( $this->valid_date_types ) ) {
			$this->valid_date_types = apply_filters( 'woocommerce_subscription_valid_date_types', array_merge(
				array_keys( awc_get_subscription_date_types() ),
				array(
					'date_created',
					'date_modified',
					'date_paid',
	...
	public function get_change_payment_method_url() {
		...
		return apply_filters( 'awc_get_change_payment_method_url', $change_payment_method_url, $this->get_id() );
	}
	...
	public function update_parent( $order ) {
		...
		$this->set_parent_id( awc_get_objects_property( $order, 'id' ) );
	...
	protected function update_last_payment_date( $datetime ) {
		...
		$timestamp = ( $datetime > 0 ) ? awc_date_to_time( $datetime ) : 0;
	...
	/**
	 * Apply the deprecated 'awc_subscription_payment_completed_count' filter
	...
	protected function apply_deprecated_completed_payment_count_filter( $count ) {
		$deprecated_filter_hook = 'awc_subscription_payment_completed_count';

		if ( has_filter( $deprecated_filter_hook ) ) {
			awc_deprecated_hook( $deprecated_filter_hook, '2.6.0', '"woocommerce_subscription_parent_payment_completed_count" and "woocommerce_subscription_renewal_payment_completed_count" to provide the discrete counts summed in the "' . $deprecated_filter_hook . '" filter' );
