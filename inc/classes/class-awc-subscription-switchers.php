...
class AWC_Subscription_Switchers {
	...
	public static function init() {
		...
		// Render "awc_switching_options" field
		add_action( 'woocommerce_admin_field_awc_switching_options', __CLASS__ . '::switching_options_field_html' );
		...
		// Set totals for subscription switch orders (needs to be hooked just before AWC_Subscription_Cart::calculate_subscription_totals())
		...
		add_filter( 'awc_renewal_order_meta_query', array( __CLASS__, 'remove_renewal_order_meta_query' ), 10 );
		...
		add_filter( 'awc_recurring_cart_next_payment_date', array( __CLASS__, 'recurring_cart_next_payment_date' ), 100, 2 );

		// Make sure the new end date starts from the end of the time that has already paid for
		add_filter( 'awc_recurring_cart_end_date', array( __CLASS__, 'recurring_cart_end_date' ), 100, 3 );
		...
		add_action( 'awc_subscriptions_switch_completed', array( __CLASS__, 'maybe_add_switched_callback' ), 10, 1 );
		...
		// Require payment when switching from a $0 / period subscription to a non-zero subscription to process automatic payments
		add_action( 'awc_subscriptions_switch_completed', array( __CLASS__, 'maybe_set_payment_method_after_switch' ), 10, 1 );
		...
		add_action( 'awc_recurring_cart_start_date', array( __CLASS__, 'maybe_unset_free_trial' ), 0, 1 );
		add_action( 'awc_recurring_cart_end_date', array( __CLASS__, 'maybe_set_free_trial' ), 100, 1 );
		...
		add_action( 'awc_subscriptions_switch_completed', array( __CLASS__, 'grant_download_permissions' ), 9, 1 );
	...
	public static function subscription_switch_handler() {
		...
		if ( isset( $_GET['switch-subscription'] ) && isset( $_GET['item'] ) ) {
			...
			$subscription = awc_get_subscription( $_GET['switch-subscription'] );
			$line_item    = awc_get_order_item( $_GET['item'], $subscription );

			// Visiting a switch link for someone elses subscription or if the switch link doesn't contain a valid nonce
			if ( ! is_object( $subscription ) || empty( $_GET['_asubnonce'] ) || ! wp_verify_nonce( $_GET['_asubnonce'], 'awc_switch_request' ) || empty( $line_item ) || ! self::can_item_be_switched_by_user( $line_item, $subscription ) ) {

				wp_redirect( remove_query_arg( array( 'switch-subscription', 'auto-switch', 'item', '_asubnonce' ) ) );
				exit();

			} else {

				if ( isset( $_GET['auto-switch'] ) ) {
					$switch_message = __( 'You have a subscription to this product. Choosing a new subscription will replace your existing subscription.', 'subscriptions-recurring-payments-for-woocommerce' );
				} else {
					$switch_message = __( 'Choose a new subscription.', 'subscriptions-recurring-payments-for-woocommerce' );
				}
			...
			foreach ( $switch_items as $cart_item_key => $switch_item ) {
				...
				$subscription  = awc_get_subscription( $switch_item['subscription_id'] );
				...
				if ( $is_valid_item ) {
					if ( empty( $switch_item['item_id'] ) ) {
						...
						$item = awc_get_order_item( $switch_item['item_id'], $subscription );
			...
			if ( $removed_item_count > 0 ) {
				wc_add_notice( _n( 'Your cart contained an invalid subscription switch request. It has been removed.', 'Your cart contained invalid subscription switch requests. They have been removed.', $removed_item_count, 'subscriptions-recurring-payments-for-woocommerce' ), 'error' );
			...
			if ( $product->is_type( 'grouped' ) ) { // If we're on a grouped product's page, we need to check if this grouped product has children which are limited and may need to be switched
				...
				foreach ( $child_ids as $child_id ) {
					...
					if ( 'no' != awc_get_product_limitation( $product ) && awc_is_product_switchable_type( $product ) ) {
						$limited_switchable_products[] = $product;
					}
				}
			} elseif ( 'no' != awc_get_product_limitation( $product ) && awc_is_product_switchable_type( $product ) ) {
			...
			if ( ! empty( $limited_switchable_products ) ) {
				...
				$subscriptions = awc_get_users_subscriptions();
				...
				foreach ( $subscriptions as $subscription ) {
					foreach ( $limited_switchable_products as $product ) {
						...
						$limitation = awc_get_product_limitation( $product );

						if ( 'any' == $limitation || $subscription->has_status( $limitation ) ) {

							$subscribed_notice = __( 'You have already subscribed to this product and it is limited to one per customer. You can not purchase the product again.', 'subscriptions-recurring-payments-for-woocommerce' );
							...
							if ( $subscription->needs_payment() ) {
								...
								if ( $last_order->needs_payment() ) {
									...
									$subscribed_notice = sprintf( __( '%1$s Complete payment on %2$sOrder %3$s%4$s to be able to change your subscription.', 'subscriptions-recurring-payments-for-woocommerce' ), $subscribed_notice, sprintf( '<a href="%s">', $last_order->get_checkout_payment_url() ), $last_order->get_order_number(), '</a>' );
								}
								...
								if ( apply_filters( 'awc_initiate_auto_switch', self::can_item_be_switched_by_user( $item, $subscription ), $item, $subscription ) ) {
	...
	public static function add_switch_query_arg_post_link( $permalink, $post ) {
		...
		$type    = awc_get_objects_property( $product, 'type' );
		...
		switch ( $type ) {
			...
			case 'grouped':
				...
				foreach ( $children as $child ) {
					...
					if ( 'subscription' === awc_get_objects_property( $child_product, 'type' ) ) {
						return self::add_switch_query_args( $_GET['switch-subscription'], $_GET['item'], $permalink );
					}
				}
	...
	/**
	 * Render the awc_switching_options setting field.
	 *
	...
	public static function switching_options_field_html( $data ) {
		...
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label for="awc_switching_options">
					<?php esc_html_e( 'Allow Switching', 'subscriptions-recurring-payments-for-woocommerce' ) ?>
				</label>
			</th>
			<td class="forminp forminp-awc_switching_options">
				<div class="awc_setting_switching_options" id="woocommerce_subscriptions_allow_switching">
					<label>
						<input <?php checked( $allow_switching_variable_checked ); ?> type="checkbox" name="<?php echo esc_attr( AWC_Subscriptions_Admin::$option_prefix . '_allow_switching_variable' ) ?>"/>
						<?php echo esc_html_x( 'Between Subscription Variations', 'when to allow switching', 'subscriptions-recurring-payments-for-woocommerce' ); ?>
					</label>
					<label>
						<input <?php checked( $allow_switching_grouped_checked ); ?> type="checkbox" name="<?php echo esc_attr( AWC_Subscriptions_Admin::$option_prefix . '_allow_switching_grouped' ) ?>"/>
						<?php echo esc_html_x( 'Between Grouped Subscriptions', 'when to allow switching', 'subscriptions-recurring-payments-for-woocommerce' ); ?>
					</label>
					<?php
					...
					foreach ( $extra_switching_options as $option ) {
						...
						$label = $option['label'];
						$name  = AWC_Subscriptions_Admin::$option_prefix . '_allow_switching_' . $option['id'];
						...
						echo isset( $option['desc_tip'] ) ? awc_help_tip( $option['desc_tip'], true ) : '';
						echo '</label>';
					}
					?>
				</div>
	...
	public static function print_switch_link( $item_id, $item, $subscription ) {
		...
		if ( awc_is_order( $subscription ) || 'shop_subscription' !== $subscription->get_type() || ! self::can_item_be_switched_by_user( $item, $subscription ) ) {
			return;
		}

		$switch_url  = esc_url( self::get_switch_url( $item_id, $item, $subscription ) );
		$switch_text = get_option( AWC_Subscriptions_Admin::$option_prefix . '_switch_button_text', __( 'Upgrade or Downgrade', 'subscriptions-recurring-payments-for-woocommerce' ) );
		$switch_link = sprintf( '<a href="%s" class="asub-switch-link button">%s</a>', $switch_url, $switch_text );
	 ...
	 *
	 * @param AWC_Subscription $subscription An instance of AWC_Subscription
	...
	public static function get_switch_url( $item_id, $item, $subscription ) {
		...
		$parent_products       = AWC_Subscription_Products::get_visible_grouped_parent_product_ids( $product );
	...
	protected static function add_switch_query_args( $subscription_id, $item_id, $permalink, $additional_query_args = array() ) {
		...
		$query_args = array_merge(
			...
			array(
				...
				'item'                => absint( $item_id ),
				'_asubnonce'           => wp_create_nonce( 'awc_switch_request' ),
			)
		);
	...
	protected static function is_action_allowed( $action, $item, $subscription = null ) {
		...
		$product_id = awc_get_canonical_product_id( $item );

		if ( 'switch' === $action ) {
			if ( 'line_item' == $item['type'] && awc_is_product_switchable_type( $product_id ) ) {
	...
	public static function add_order_meta( $order_id, $posted ) {
		...
		awc_Related_Order_Store::instance()->delete_relations( $order, 'switch' );
		...
		if ( false !== $switches ) {
			...
			foreach ( $switches as $switch_details ) {
				awc_Related_Order_Store::instance()->add_relation( $order, awc_get_subscription( $switch_details['subscription_id'] ), 'switch' );
			}
		}
	}
	...
	public static function add_order_item_meta( $order_item_id, $cart_item, $cart_item_key ) {
		...
		if ( false === AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
			...
			if ( $switches = self::cart_contains_switches() ) {
				foreach ( $switches as $switch_item_key => $switch_details ) {
					if ( $cart_item_key == $switch_item_key ) {
						wc_add_order_item_meta( $order_item_id, '_switched_subscription_sign_up_fee_prorated', awc_get_objects_property( WC()->cart->cart_contents[ $cart_item_key ]['data'], 'subscription_sign_up_fee_prorated', 'single', 0 ), true );
						wc_add_order_item_meta( $order_item_id, '_switched_subscription_price_prorated', awc_get_objects_property( WC()->cart->cart_contents[ $cart_item_key ]['data'], 'subscription_price_prorated', 'single', 0 ), true );
					}
				}
			}
	...
	/**
	 * Handle any subscription switch items on checkout (and before AWC_Subscriptions_Checkout::process_checkout())
	 *
	 ...
	 * be removed and the new item will be added to a new subscription by @see AWC_Subscriptions_Checkout::process_checkout()
	 *
		...
		try {
			...
			foreach ( WC()->cart->recurring_carts as $recurring_cart_key => $recurring_cart ) {
				...
				foreach ( $recurring_cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					$subscription = awc_get_subscription( $cart_item['subscription_switch']['subscription_id'] );
					...
					if ( 0 == $cart_item['subscription_switch']['first_payment_timestamp'] ) {
						$cart_item['subscription_switch']['first_payment_timestamp'] = awc_date_to_time( $recurring_cart->next_payment_date );
					}
					...
					if ( ! empty( $cart_item['subscription_switch']['item_id'] ) ) {
						$existing_item                          = awc_get_order_item( $cart_item['subscription_switch']['item_id'], $subscription );
						$switch_item                            = new awc_Switch_Cart_Item( $cart_item, $subscription, $existing_item );
					...
					if ( $is_single_item_subscription || ( false === $is_different_billing_schedule && false === $is_different_payment_date && false === $is_different_length ) ) {
						...
						// Add the new item
						if ( AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
							$item_id = AWC_Subscriptions_Checkout::add_cart_item( $subscription, $cart_item, $cart_item_key );
							awc_update_order_item_type( $item_id, 'line_item_pending_switch', $subscription->get_id() );
						} else {
						...
						// Remove the item from the cart so that AWC_Subscriptions_Checkout doesn't add it to a subscription
						if ( 1 == count( WC()->cart->recurring_carts[ $recurring_cart_key ]->get_cart() ) ) {
							// If this is the only item in the cart, clear out recurring carts so AWC_Subscriptions_Checkout doesn't try to create an empty subscription
					...
					if ( $is_single_item_subscription ) {
						...
						if ( $is_different_billing_schedule ) {
							$switch_order_data[ $subscription->get_id() ]['billing_schedule']['_billing_period']   = AWC_Subscription_Products::awc_get_period( $cart_item['data'] );
							$switch_order_data[ $subscription->get_id() ]['billing_schedule']['_billing_interval'] = absint( AWC_Subscription_Products::awc_get_interval( $cart_item['data'] ) );
						}

						$updated_dates = array();

						if ( '1' == AWC_Subscription_Products::awc_get_length( $cart_item['data'] ) || ( 0 != $recurring_cart->end_date && gmdate( 'Y-m-d H:i:s', $cart_item['subscription_switch']['first_payment_timestamp'] ) >= $recurring_cart->end_date ) ) {
					...
					AWC_Subscriptions_Checkout::add_shipping( $subscription, $recurring_cart );
					...
					foreach ( $subscription->get_shipping_methods() as $shipping_line_item_id => $shipping_meta ) {
						...
						if ( ! in_array( $shipping_line_item_id, $current_shipping_line_items ) ) {
							awc_update_order_item_type( $shipping_line_item_id, 'shipping_pending_switch', $subscription->get_id() );
			...
			foreach ( $switch_order_data as $subscription_id => $switch_data ) {
				...
				$switch_orders = awc_get_switch_orders_for_subscription( $subscription_id );

				foreach ( $switch_orders as $switch_order_id => $switch_order ) {
					if ( awc_get_objects_property( $order, 'id' ) !== $switch_order_id && in_array( $switch_order->get_status(), apply_filters( 'woocommerce_valid_order_statuses_for_payment', array( 'pending', 'failed', 'on-hold' ), $switch_order ) ) ) {
						// translators: %s: order number.
						$switch_order->update_status( 'cancelled', sprintf( __( 'Switch order cancelled due to a new switch order being created #%s.', 'subscriptions-recurring-payments-for-woocommerce' ), $order->get_order_number() ) );
					}
				}
			}

			if ( ! empty( $switch_order_data ) ) {
				awc_set_objects_property( $order, 'subscription_switch_data', $switch_order_data );
	...
	public static function update_shipping_methods( $subscription, $recurring_cart ) {
		...
		foreach ( $subscription->get_shipping_methods() as $shipping_method_id => $shipping_method ) {
			awc_update_order_item_type( $shipping_method_id, 'shipping_switched', $subscription->get_id() );
		}
	 ...
	 *
	 * @param  WC_Order $order The new order
	 * @param  AWC_Subscription $subscription The original subscription
	 */
	public static function maybe_update_subscription_address( $order, $subscription ) {
		...
		if ( method_exists( $subscription, 'get_address' ) ) {
			...
			$order_billing         = awc_get_order_address( $order, 'billing' );
			$order_shipping        = awc_get_order_address( $order );
			$subscription_billing  = awc_get_order_address( $subscription, 'billing' );
			$subscription_shipping = awc_get_order_address( $subscription );
	...
	public static function switch_order_meta_box_rows( $post ) {
		...
		if ( awc_is_subscription( $post->ID ) ) {
			...
			$orders = awc_get_switch_orders_for_subscription( $post->ID );

			foreach ( $orders as $order_id => $order ) {
				awc_set_objects_property( $order, 'relationship', __( 'Switch Order', 'subscriptions-recurring-payments-for-woocommerce' ), 'set_prop_only' );
			}

			// Select the subscriptions which had item/s switched to this subscription by its parent order
			if ( ! empty( $post->post_parent ) ) {
				$switched_subscriptions = awc_get_subscriptions_for_switch_order( $post->post_parent );
			}

		// On the Edit Order screen, show any subscriptions with items switched by this order
		} else {
			$switched_subscriptions = awc_get_subscriptions_for_switch_order( $post->ID );
		}

		if ( is_array( $switched_subscriptions ) ) {
			foreach ( $switched_subscriptions as $subscription_id => $subscription ) {
				awc_set_objects_property( $subscription, 'relationship', __( 'Switched Subscription', 'subscriptions-recurring-payments-for-woocommerce' ), 'set_prop_only' );
				$orders[ $subscription_id ] = $subscription;
			}
		}

		foreach ( $orders as $order ) {
			include( plugin_dir_path( AWC_Subscriptions::$plugin_file ) . 'includes/admin/meta-boxes/view/html-related-orders-row.php' );
		}
	...
	public static function cart_contains_switches( $item_action = 'switch' ) {
		...
		foreach ( WC()->cart->cart_contents as $cart_item_key => $cart_item ) {
			...
			if ( ! awc_is_subscription( $cart_item['subscription_switch']['subscription_id'] ) ) {
				WC()->cart->remove_cart_item( $cart_item_key );
				wc_add_notice( __( 'Your cart contained an invalid subscription switch request. It has been removed.', 'subscriptions-recurring-payments-for-woocommerce' ), 'error' );
				continue;
			}
	...
	public static function cart_contains_switch_for_product( $product ) {
		...
		if ( false !== $switch_items ) {
			...
			foreach ( $switch_items as $switch_item_details ) {
				...
				$switch_product  = wc_get_product( awc_get_order_items_product_id( $switch_item_details['item_id'] ) );
				$parent_products = AWC_Subscription_Products::get_parent_ids( $product );
	...
	public static function trigger_switch_added_to_cart_hook( $cart_item_key, $product_id, $quantity, $variation_id, $variation_attributes, $cart_item_data ) {
		...
		$subscription  = awc_get_subscription( $cart_item_data['subscription_switch']['subscription_id'] );
		$existing_item = awc_get_order_item( $cart_item_data['subscription_switch']['item_id'], $subscription );
	...
	public static function retain_coupons( $subscription ) {
		foreach ( awc_get_used_coupon_codes( $subscription ) as $coupon_code ) {
	...
	public static function validate_switch_request( $is_valid, $product_id, $quantity, $variation_id = '' ) {
		...
		try {
			...
			if ( empty( $_GET['_asubnonce'] ) || ! wp_verify_nonce( $_GET['_asubnonce'], 'awc_switch_request' ) ) {
				return false;
			}

			$subscription = awc_get_subscription( $_GET['switch-subscription'] );
			$item_id      = absint( $_GET['item'] );
			$item         = awc_get_order_item( $item_id, $subscription );

			// Prevent switching to non-subscription product
			if ( ! AWC_Subscription_Products::awc_is_subscription( $product_id ) ) {
				throw new Exception( __( 'You can only switch to a subscription product.', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}
			...
			if ( empty( $item ) ) {
				...
				throw new Exception( __( 'We can not find your old subscription item.', 'subscriptions-recurring-payments-for-woocommerce' ) );
				...
				if ( $is_identical_product ) {
					throw new Exception( __( 'You can not switch to the same subscription.', 'subscriptions-recurring-payments-for-woocommerce' ) );
				}
	...
	public static function set_switch_details_in_cart( $cart_item_data, $product_id, $variation_id ) {
		...
		try {
			...
			if ( ! current_user_can( 'switch_shop_subscription', $subscription->get_id() ) ) {
				wc_add_notice( __( 'You can not switch this subscription. It appears you do not own the subscription.', 'subscriptions-recurring-payments-for-woocommerce' ), 'error' );
				WC()->cart->empty_cart( true );
				wp_redirect( get_permalink( $product_id ) );
				exit();
			}

			$item = awc_get_order_item( absint( $_GET['item'] ), $subscription );
			...
			wc_add_notice( __( 'There was an error locating the switch details.', 'subscriptions-recurring-payments-for-woocommerce' ), 'error' );
			WC()->cart->empty_cart( true );
	...
	public static function subscription_items_sign_up_fee( $sign_up_fee, $line_item, $subscription, $tax_inclusive_or_exclusive = '' ) {
		...
		foreach ( awc_get_switch_orders_for_subscription( $subscription->get_id() ) as $order ) {
			foreach ( $order->get_items() as $order_item_id => $order_item ) {
				if ( awc_get_canonical_product_id( $line_item ) == awc_get_canonical_product_id( $order_item ) ) {
					...
					if ( 'inclusive_of_tax' == $tax_inclusive_or_exclusive && awc_get_objects_property( $order, 'prices_include_tax' ) ) {
	...
	/**
	 * Set the subscription prices to be used in calculating totals by @see AWC_Subscription_Cart::calculate_subscription_totals()
	 *
	...
	public static function calculate_prorated_totals( $cart ) {
		if ( self::cart_contains_switches( 'any' ) ) {
			self::$switch_totals_calculator = new AWC_Switch_Totals_Calculator( $cart );
	...
	public static function recurring_cart_next_payment_date( $first_renewal_date, $cart ) {
		...
		foreach ( $cart->get_cart() as $cart_item_key => $cart_item ) {
			if ( isset( $cart_item['subscription_switch']['first_payment_timestamp'] ) && 0 != $cart_item['subscription_switch']['first_payment_timestamp'] ) {
				$first_renewal_date = ( '1' != AWC_Subscription_Products::awc_get_length( $cart_item['data'] ) ) ? gmdate( 'Y-m-d H:i:s', $cart_item['subscription_switch']['first_payment_timestamp'] ) : 0;
			}
		}
		...
		if ( 0 !== $end_date ) {
			foreach ( $cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( isset( $cart_item['subscription_switch']['subscription_id'] ) && isset( $cart_item['data'] ) && $product == $cart_item['data'] ) {
					...
					if ( 1 == AWC_Subscription_Products::awc_get_length( $cart_item['data'] ) && 0 !== $next_payment_time && isset( $cart_item['subscription_switch']['recurring_payment_prorated'] ) ) {
					...
					} elseif ( 0 !== $next_payment_time && AWC_Subscription_Products::awc_get_length( $cart_item['data'] ) > 1 ) {
						// remove trial period on the switched subscription when calculating the new end date
						$trial_length = awc_get_objects_property( $cart_item['data'], 'subscription_trial_length' );
						awc_set_objects_property( $cart_item['data'], 'subscription_trial_length', 0, 'set_prop_only' );

						$end_date = AWC_Subscription_Products::get_expiration_date( $cart_item['data'], gmdate( 'Y-m-d H:i:s', $next_payment_time ) );

						// add back the trial length if it has been spoofed
						awc_set_objects_property( $cart_item['data'], 'subscription_trial_length', $trial_length, 'set_prop_only' );
	...
	public static function get_recurring_cart_key( $cart_key, $cart_item ) {
		...
		if ( isset( $cart_item['subscription_switch']['first_payment_timestamp'] ) ) {
			...
			$cart_key = AWC_Subscription_Cart::get_recurring_cart_key( $cart_item, $cart_item['subscription_switch']['first_payment_timestamp'] );
	...
	public static function customise_product_string_inclusions( $inclusions, $product ) {
		...
		if ( isset( $_GET['switch-subscription'] ) || self::cart_contains_switch_for_product( $product ) ) {
			...
			$apportion_length      = get_option( AWC_Subscriptions_Admin::$option_prefix . '_apportion_length', 'no' );
			$apportion_sign_up_fee = get_option( AWC_Subscriptions_Admin::$option_prefix . '_apportion_sign_up_fee', 'no' );
	 ...
	 *
	 * Commits all the changes calculated and saved by @see AWC_Subscription_Switchers::process_checkout(), updating subscription
	...
	public static function process_subscription_switches( $order_id, $order_old_status, $order_new_status ) {
		...
		$order            = wc_get_order( $order_id );
		$switch_processed = awc_get_objects_property( $order, 'completed_subscription_switch' );

		if ( ! awc_order_contains_switch( $order ) || 'true' == $switch_processed ) {
			return;
		}
		...
		if ( $order_completed ) {
			try {
				...
				awc_set_objects_property( $order, 'completed_subscription_switch', 'true' );
			...
			do_action( 'awc_subscriptions_switch_completed', $order );
		}
	}
	...
	public static function is_product_of_switchable_type( $product ) {
		...
		$switch_setting  = get_option( AWC_Subscriptions_Admin::$option_prefix . '_allow_switching', 'no' );

		// does the current switch setting allow switching for variable or variable_grouped
		if ( 'variable_grouped' == $switch_setting || ( $product->is_type( array( 'variable-subscription', 'subscription_variation' ) ) && 'variable' == $switch_setting ) || ( 'grouped' == $switch_setting && ( $product->is_type( 'grouped' ) || awc_get_objects_property( $product, 'parent_id' ) ) ) ) {
	...
	public static function hidden_order_itemmeta( $hidden_meta_keys ) {
		...
		if ( apply_filters( 'woocommerce_subscriptions_hide_switch_itemmeta', ! defined( 'awc_DEBUG' ) || true !== awc_DEBUG ) ) {
	...
	public static function add_cart_item_switch_direction( $product_subtotal, $cart_item, $cart_item_key ) {
		...
		if ( ! empty( $cart_item['subscription_switch'] ) ) {
			...
			switch ( $cart_item['subscription_switch']['upgraded_or_downgraded'] ) {
				case 'downgraded':
					$direction = _x( 'Downgrade', 'a switch type', 'subscriptions-recurring-payments-for-woocommerce' );
					break;
				case 'upgraded':
					$direction = _x( 'Upgrade', 'a switch type', 'subscriptions-recurring-payments-for-woocommerce' );
					break;
				default:
					$direction = _x( 'Crossgrade', 'a switch type', 'subscriptions-recurring-payments-for-woocommerce' );
				break;
			}

			// translators: %1: product subtotal, %2: HTML span tag, %3: direction (upgrade, downgrade, crossgrade), %4: closing HTML span tag
			$product_subtotal = sprintf( _x( '%1$s %2$s(%3$s)%4$s', 'product subtotal string', 'subscriptions-recurring-payments-for-woocommerce' ), $product_subtotal, '<span class="subscription-switch-direction">', $direction, '</span>' );
	...
	public static function maybe_add_switched_callback( $order ) {
		if ( awc_order_contains_switch( $order ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				foreach ( $subscription->get_items() as $new_order_item ) {
					if ( isset( $new_order_item['switched_subscription_item_id'] ) ) {
						$product_id = awc_get_canonical_product_id( $new_order_item );
						...
						foreach ( $order->get_items() as $order_item ) {
							if ( awc_get_canonical_product_id( $order_item ) == $product_id ) {
								do_action( 'woocommerce_subscriptions_switched_item', $subscription, $new_order_item, AWC_Subscriptions_Order::get_item_by_id( $new_order_item['switched_subscription_item_id'] ) );
								break;
							}
						}
					}
	...
	*/
	public static function remove_download_permissions_after_switch( $subscription, $new_item, $old_item ) {
		...
		$product_id = awc_get_canonical_product_id( $old_item );
		awc_Download_Handler::revoke_downloadable_file_permission( $product_id, $subscription->get_id(), $subscription->get_user_id() );
	 ...
	 *
	 * Performs all the changes calculated and saved by @see AWC_Subscription_Switchers::process_checkout(), updating subscription
	 * line items, schedule, dates and totals to reflect the changes made in this switch order.
	 *
	 * @param WC_Order $order
	...
	public static function complete_subscription_switches( $order ) {
		...
		// Get the switch meta
		$switch_order_data = awc_get_objects_property( $order, 'subscription_switch_data' );
		...
		foreach ( $switch_order_data as $subscription_id => $switch_data ) {
			...
			if ( ! $subscription instanceof AWC_Subscription ) {
				continue;
			}
			...
			if ( ! empty( $switch_data['switches'] ) && is_array( $switch_data['switches'] ) ) {
				foreach ( $switch_data['switches'] as $order_item_id => $switched_item_data ) {
					...
					$switch_order_item        = awc_get_order_item( $order_item_id, $order );

					// If we are adding a line item to an existing subscription...
					if ( $add_subscription_item ) {
						awc_update_order_item_type( $switched_item_data['add_line_item'], 'line_item', $subscription->get_id() );
					...
					if ( $remove_subscription_item ) {
						$old_subscription_item = awc_get_order_item( $switched_item_data['remove_line_item'], $subscription );
					}

					if ( $remove_subscription_item && empty( $old_subscription_item ) ) {
						throw new Exception( __( 'The original subscription item being switched cannot be found.', 'subscriptions-recurring-payments-for-woocommerce' ) );
					} elseif ( empty( $switch_order_item ) ) {
						throw new Exception( __( 'The item on the switch order cannot be found.', 'subscriptions-recurring-payments-for-woocommerce' ) );
					}
					...
					$new_item_name = awc_get_order_item_name( $switch_order_item, array( 'attributes' => true ) );

					if ( $remove_subscription_item ) {
						awc_update_order_item_type( $switched_item_data['remove_line_item'], 'line_item_switched', $subscription->get_id() );

						// translators: 1$: old item, 2$: new item when switching
						$add_note = sprintf( _x( 'Customer switched from: %1$s to %2$s.', 'used in order notes', 'subscriptions-recurring-payments-for-woocommerce' ), awc_get_order_item_name( $old_subscription_item, array( 'attributes' => true ) ), $new_item_name );
					} else {
						// translators: %s: new item name.
						$add_note = sprintf( _x( 'Customer added %s.', 'used in order notes', 'subscriptions-recurring-payments-for-woocommerce' ), $new_item_name );
					}
			...
			foreach ( $subscription->get_items( 'coupon' ) as $coupon_id => $coupon ) {
				awc_update_order_item_type( $coupon_id, 'coupon_switched', $subscription->get_id() );
			}
			...
			if ( ! empty( $switch_data['coupons'] ) && is_array( $switch_data['coupons'] ) ) {
				...
				foreach ( $switch_data['coupons'] as $coupon_code ) {
					awc_update_order_item_type( $coupon_code, 'coupon', $subscription->get_id() );
				}
			}

			// Archive the old fees
			foreach ( $subscription->get_fees() as $fee_item_id => $fee ) {
				awc_update_order_item_type( $fee_item_id, 'fee_switched', $subscription->get_id() );
			}
			...
			if ( ! empty( $switch_data['fee_items'] ) && is_array( $switch_data['fee_items'] ) ) {
				...
				foreach ( $switch_data['fee_items'] as $fee_item_id ) {
					awc_update_order_item_type( $fee_item_id, 'fee', $subscription->get_id() );
				}
			}
			...
			if ( ! empty( $switch_data['shipping_line_items'] ) && is_array( $switch_data['shipping_line_items'] ) ) {
				...
				foreach ( $subscription->get_shipping_methods() as $shipping_line_item_id => $item ) {
					awc_update_order_item_type( $shipping_line_item_id, 'shipping_switched', $subscription->get_id() );
				}

				// Flip the switched shipping line items "on"
				foreach ( $switch_data['shipping_line_items'] as $shipping_line_item_id ) {
					awc_update_order_item_type( $shipping_line_item_id, 'shipping', $subscription->get_id() );
				}
			}
			...
			awc_get_subscription( $subscription->get_id() )->calculate_totals();
		}
	}
	...
	public static function set_force_payment_flag_in_cart( $total ) {
		...
		if ( $total > 0 || 'yes' == get_option( AWC_Subscriptions_Admin::$option_prefix . '_turn_off_automatic_payments', 'no' ) || false === self::cart_contains_switches( 'any' ) ) {
			return $total;
		}
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			$menualRenewal = AWC_Settings::get_option('manual_renewal_payment') ? 'yes' : 'no';
			$force_automatic_payments = ! $is_manual_subscription || 'no' === $menualRenewal;
	...
	public static function maybe_set_payment_method_after_switch( $order ) {
		...
		if ( AWC_Settings::get_option('turn_of_automatic_payment') ) {
			return;
		}

		foreach ( awc_get_subscriptions_for_switch_order( $order ) as $subscription ) {

			if ( false === $subscription->is_manual() ) {
				continue;
			}

			if ( $subscription->get_payment_method() !== awc_get_objects_property( $order, 'payment_method' ) ) {
	...
	public static function delay_granting_download_permissions( $order_id ) {
		if ( awc_order_contains_switch( $order_id ) ) {
			remove_action( 'woocommerce_grant_product_download_permissions', 'awc_Download_Handler::save_downloadable_product_permissions' );
		}
	}
	...
	public static function grant_download_permissions( $order ) {
		awc_Download_Handler::save_downloadable_product_permissions( awc_get_objects_property( $order, 'id' ) );

		// reattach the hook detached in @see self::delay_granting_download_permissions()
		add_action( 'woocommerce_grant_product_download_permissions', 'awc_Download_Handler::save_downloadable_product_permissions' );
	}
	 ...
	 *
	 * @param AWC_Subscription $subscription         The Subscription.
	...
	public static function calculate_total_paid_since_last_order( $subscription, $subscription_item, $include_sign_up_fees = 'include_sign_up_fees' ) {
		...
		awc_sort_objects( $orders, 'date_paid', 'descending' );
		...
		foreach ( $orders as $order ) {
			...
			$order_item = awc_find_matching_line_item( $order, $subscription_item );
			...
			if ( $has_been_switched && awc_order_contains_switch( $order ) ) {
				...
				if ( isset( $switched_subscription_items[ $switched_subscription_item_id ] ) ) {
					...
					if ( $switch_order_item_id && (bool) awc_get_order_item( $switch_order_item_id, $order ) ) {
			...
			if ( $order_is_parent || ( awc_order_contains_renewal( $order ) && ! awc_order_contains_early_renewal( $order ) ) ) {
				break;
			}
		}
	...
	/**
	 * Logs information about all the switches in the cart to the asub-switch-cart-items log.
	 *
	...
	public static function subscription_switch_autocomplete( $new_order_status, $order_id ) {
		return AWC_Subscriptions_Order::maybe_autocomplete_order( $new_order_status, $order_id );
	}
	...
	public static function maybe_set_payment_method( $payment_processing_result, $order_id ) {
		...
		if ( awc_order_contains_switch( $order_id ) && false != get_post_meta( $order_id, '_paid_date', true ) ) {
	...
	public static function maybe_set_free_trial( $total = '' ) {
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			if ( isset( $cart_item['subscription_switch']['first_payment_timestamp'] ) && 0 != $cart_item['subscription_switch']['first_payment_timestamp'] ) {
				awc_set_objects_property( WC()->cart->cart_contents[ $cart_item_key ]['data'], 'subscription_trial_length', 1, 'set_prop_only' );
			}
		}
	...
	public static function maybe_unset_free_trial( $total = '' ) {
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			if ( isset( $cart_item['subscription_switch']['first_payment_timestamp'] ) && 0 != $cart_item['subscription_switch']['first_payment_timestamp'] ) {
				awc_set_objects_property( WC()->cart->cart_contents[ $cart_item_key ]['data'], 'subscription_trial_length', 0, 'set_prop_only' );
			}
		}
		return $total;
	}
	...
	protected static function has_different_billing_schedule( $cart_item, $subscription ) {
		return AWC_Subscription_Products::awc_get_period( $cart_item['data'] ) != $subscription->get_billing_period() || AWC_Subscription_Products::awc_get_interval( $cart_item['data'] ) != $subscription->get_billing_interval();
	}
	...
	protected static function has_different_length( $recurring_cart, $subscription ) {
		$recurring_cart_end_date = gmdate( 'Y-m-d', awc_date_to_time( $recurring_cart->end_date ) );
	 ...
	 * @param array $subscription A subscription of the form created by @see AWC_Subscription_Manager::get_subscription()
	...
	public static function add_switch_button( $all_actions, $subscriptions ) {
		...
		foreach ( $all_actions as $subscription_key => $actions ) {
			...
			if ( AWC_Subscription_Manager::can_subscription_be_changed_to( 'new-subscription', $subscription_key, $user_id ) ) {
				$all_actions[ $subscription_key ] = array(
					'switch' => array(
						'url'  => self::get_switch_link( $subscription_key ),
						'name' => get_option( AWC_Subscriptions_Admin::$option_prefix . '_switch_button_text', __( 'Upgrade or Downgrade', 'subscriptions-recurring-payments-for-woocommerce' ) ),
					),
	...
	/**
	 * Add a 'new-subscription' handler to the AWC_Subscription_Manager::can_subscription_be_changed_to() function.
	 *
	 ...
	 * @param object $args Set of values used in @see AWC_Subscription_Manager::can_subscription_be_changed_to() for determining if a subscription can be changed
	...
	public static function calculate_first_payment_date( $next_payment_date, $order, $product_id, $type ) {
		...
		return self::get_first_payment_date( $next_payment_date, AWC_Subscription_Manager::get_subscription_key( awc_get_objects_property( $order, 'id' ), $product_id ), $order->get_user_id(), $type );
	}
	...
	public static function get_first_payment_date( $next_payment_date, $subscription_key, $user_id, $type ) {
		...
		$subscription = awc_get_subscription_from_key( $subscription_key );

		if ( $subscription->has_status( 'active' ) && $subscription->get_parent_id() && awc_order_contains_switch( $subscription->get_parent_id() ) && 1 >= $subscription->get_payment_count() ) {
	...
	public static function add_switched_status_string( $status_string ) {
		...
		if ( 'switched' === strtolower( $status_string ) ) {
			$status_string = _x( 'Switched', 'Subscription status', 'subscriptions-recurring-payments-for-woocommerce' );
		}
	 ...
	 * @return bool Returns true if the order switched a subscription
	 */
	public static function order_contains_subscription_switch( $order_id ) {
		return awc_order_contains_switch( $order_id );
	}
	...
	/**
	 * Filter the AWC_Subscription::get_related_orders() method to include switch orders.
	 *
	 *
	...
	public static function add_related_orders( $related_orders, $subscription, $return_fields, $order_type ) {
		_deprecated_function( __METHOD__, '2.3.0', 'awc_get_switch_orders_for_subscription()' );
		if ( in_array( $order_type, array( 'all', 'switch' ) ) ) {

			$switch_orders = awc_get_switch_orders_for_subscription( $subscription->get_id() );
