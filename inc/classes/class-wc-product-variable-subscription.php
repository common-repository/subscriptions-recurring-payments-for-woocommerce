...
class WC_Product_Variable_Subscription extends WC_Product_Variable {
		/**
	 * A cache of the variable product's min and max data generated by @see awc_get_min_max_variation_data().
	 *
	 * @var array
	 */
	...
	public function get_price_html( $price = '' ) {
		...
		if ( empty( $min_price_variation_id ) ) {
			AWC_Subscription_Products::awc_variable_subscription_product_sync( $this );
			$min_price_variation_id = $this->get_meta( '_min_price_variation_id' );
		}
		...
		$price = AWC_Subscription_Products::awc_get_price( $min_price_variation_id );
		$price = 'incl' == $tax_display_mode ? awc_get_price_including_tax( $this, array( 'price' => $price ) ) : awc_get_price_excluding_tax( $this, array( 'price' => $price ) );
	 ...
	 *
	 * @param array $min_and_max_data The min and max variation data returned by @see awc_get_min_max_variation_data(). Optional.
	...
	public function set_min_and_max_variation_data( $min_and_max_data = array(), $variation_ids = array() ) {
		...
		if ( empty( $min_and_max_data ) ) {
			$min_and_max_data = awc_get_min_max_variation_data( $this, $variation_ids );
		}
	 ...
	 *
	 * This is a wrapper for @see awc_get_min_max_variation_data() but to avoid calling
	...
	public function get_min_and_max_variation_data( $variation_ids ) {
		...
		if ( $variation_ids_hash === $this->get_meta( '_min_max_variation_ids_hash', true ) ) {
			...
			$min_and_max_variation_data = awc_get_min_max_variation_data( $this, $variation_ids );
	...
	public function add_to_cart_handler( $handler, $product ) {
		_deprecated_function( __METHOD__, '2.2.0', 'AWC_Subscription_Cart::add_to_cart_handler( $handler, $product )' );
		return AWC_Subscription_Cart::add_to_cart_handler( $handler, $product );
	}
	...
	public function variable_product_sync( $product_id = '' ) {
		_deprecated_function( __METHOD__, '2.2,0', 'AWC_Subscription_Products::awc_variable_subscription_product_sync( $this )' );
	...
	protected function get_price_prefix( $prices ) {
		...
		if ( $min_max_variation_data['identical'] ) {
			...
			$prefix = awc_get_price_html_from_text( $this );
		}
