...
class AWC_Cart_Renewal {
	...
	public function __construct() {
		...
		add_action( 'awc_before_renewal_setup_cart_subscriptions', array( &$this, 'clear_coupons' ), 10 );

		// Handles renew of password-protected products.
		add_action( 'awc_before_renewal_setup_cart_subscriptions', 'awc_allow_protected_products_to_renew' );
		add_action( 'awc_after_renewal_setup_cart_subscriptions', 'awc_disallow_protected_product_add_to_cart_validation' );
	...
	public function setup_hooks() {
		...
		awc_add_woocommerce_dependent_action( 'woocommerce_before_cart_item_quantity_zero', array( &$this, 'maybe_remove_items' ), '3.7.0', '<' );
	...
	public function maybe_setup_cart() {
		...
		if ( isset( $_GET['pay_for_order'] ) && isset( $_GET['key'] ) && isset( $wp->query_vars['order-pay'] ) ) {
			...
			if ( awc_get_objects_property( $order, 'order_key' ) == $order_key && $order->has_status( array( 'pending', 'failed' ) ) && awc_order_contains_renewal( $order ) ) {
				...
				$subscriptions = awc_get_subscriptions_for_renewal_order( $order );

				do_action( 'awc_before_renewal_setup_cart_subscriptions', $subscriptions, $order );

				foreach ( $subscriptions as $subscription ) {

					do_action( 'awc_before_renewal_setup_cart_subscription', $subscription, $order );

					// Check if order/subscription can be paid for
					if ( empty( $subscription ) || ! $subscription->has_status( array( 'on-hold', 'pending' ) ) ) {
						wc_add_notice( __( 'This order can no longer be paid because the corresponding subscription does not require payment at this time.', 'subscriptions-recurring-payments-for-woocommerce' ), 'error' );
					} else {
					...
					do_action( 'awc_after_renewal_setup_cart_subscription', $subscription, $order );
				}

				do_action( 'awc_after_renewal_setup_cart_subscriptions', $subscriptions, $order );

				if ( WC()->cart->cart_contents_count != 0 ) {
					// Store renewal order's ID in session so it can be re-used after payment
					WC()->session->set( 'order_awaiting_payment', $order_id );
					wc_add_notice( __( 'Complete checkout to renew your subscription.', 'subscriptions-recurring-payments-for-woocommerce' ), 'success' );
				}
	...
	protected function setup_cart( $subscription, $cart_item_data, $validation_type = 'all_items_not_required' ) {
		...
		foreach ( $subscription->get_items() as $item_id => $line_item ) {
			...
			$product_deleted_error_message = apply_filters( 'woocommerce_subscriptions_renew_deleted_product_error_message', __( 'The %s product has been deleted and can no longer be renewed. Please choose a new product or contact us for assistance.', 'subscriptions-recurring-payments-for-woocommerce' ) );
		...
		if ( ! $success && 'all_items_required' === $validation_type ) {
			if ( awc_is_subscription( $subscription ) ) {
				...
				wc_add_notice( sprintf( esc_html__( 'Subscription #%s has not been added to the cart.', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription->get_order_number() ), 'error' );
			} else {
				// translators: %s is order's number
				wc_add_notice( sprintf( esc_html__( 'Order #%s has not been added to the cart.', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription->get_order_number() ), 'error' );
			}
	...
	public function cart_items_loaded_from_session( $cart ) {
		...
		foreach ( $cart->cart_contents as $key => $item ) {
			if ( isset( $item[ $this->cart_item_key ]['subscription_id'] ) && ! awc_is_subscription( $item[ $this->cart_item_key ]['subscription_id'] ) ) {
		...
		if ( $removed_count_subscription ) {
			$error_message = esc_html( _n( 'We couldn\'t find the original subscription for an item in your cart. The item was removed.', 'We couldn\'t find the original subscriptions for items in your cart. The items were removed.', $removed_count_subscription, 'subscriptions-recurring-payments-for-woocommerce' ) );
		...
		if ( $removed_count_order ) {
			$error_message = esc_html( _n( 'We couldn\'t find the original renewal order for an item in your cart. The item was removed.', 'We couldn\'t find the original renewal orders for items in your cart. The items were removed.', $removed_count_order, 'subscriptions-recurring-payments-for-woocommerce' ) );
			if ( ! wc_has_notice( $error_message, 'notice' ) ) {
				wc_add_notice( $error_message, 'notice' );
			}
		}
	...
	public function get_cart_item_from_session( $cart_item_session_data, $cart_item, $key ) {
		...
		if ( isset( $cart_item[ $this->cart_item_key ]['subscription_id'] ) ) {
			...
			if ( $subscription ) {
				...
				awc_set_objects_property( $_product, 'subscription_sign_up_fee', 0, 'set_prop_only' );
				...
				awc_set_objects_property( $_product, 'name', apply_filters( 'woocommerce_subscriptions_renewal_product_title', $line_item_name, $_product ), 'set_prop_only' );
	...
	public function checkout_get_value( $value, $key ) {
		...
		if ( $this->cart_contains() && did_action( 'woocommerce_checkout_init' ) > 0 ) {
			...
			// Guard against the fake WC_Checkout singleton, see https://github.com/woocommerce/aco-woo-subscriptions/issues/427#issuecomment-260763250
			...
			if ( array_key_exists( $key, $address_fields ) && false !== ( $item = $this->cart_contains() ) ) {
				...
				if ( ( $order_value = awc_get_objects_property( $order, $key ) ) ) {
					$value = $order_value;
				}
			}
		}
	...
	public function maybe_update_subscription_customer_data( $update_customer_data, $checkout_object ) {
		...
		if ( false !== $cart_renewal_item ) {
			...
			$subscription = awc_get_subscription( $cart_renewal_item[ $this->cart_item_key ]['subscription_id'] );
	...
	public function maybe_preserve_order_status( $order_status ) {
		...
		if ( null !== WC()->session && 'failed' !== $order_status ) {
			...
			if ( $order_id > 0 && ( $order = wc_get_order( $order_id ) ) && awc_order_contains_renewal( $order ) && $order->has_status( 'failed' ) ) {
				$order_status = 'failed';
			}
	...
	public function maybe_remove_items( $cart_item_key ) {
		...
		if ( isset( WC()->cart->cart_contents[ $cart_item_key ][ $this->cart_item_key ]['subscription_id'] ) ) {
			...
			if ( $removed_item_count > 1 && 'woocommerce_before_cart_item_quantity_zero' == current_filter() ) {
				wc_add_notice( esc_html__( 'All linked subscription items have been removed from the cart.', 'subscriptions-recurring-payments-for-woocommerce' ), 'notice' );
			}
		}
	}
	...
	public function items_removed_title( $product_title, $cart_item ) {
		...
		if ( isset( $cart_item[ $this->cart_item_key ]['subscription_id'] ) ) {
			$subscription  = $this->get_order( $cart_item );
			$product_title = ( count( $subscription->get_items() ) > 1 ) ? esc_html_x( 'All linked subscription items were', 'Used in WooCommerce by removed item notification: "_All linked subscription items were_ removed. Undo?" Filter for item title.', 'subscriptions-recurring-payments-for-woocommerce' ) : $product_title;
		}
	...
	public function renewal_coupon_data( $data, $code ) {
		...
		foreach ( $renewal_coupons as $order_id => $coupons ) {
			...
			foreach ( $coupons as $coupon_code => $coupon_properties ) {
				...
				if ( $coupon_code == $code ) {
					$expiry_date_property = AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ? 'expiry_date' : 'date_expires';
	 ...
	 *
	 * @param  object WC_Order | AWC_Subscription $order
	...
	protected function store_coupon( $order_id, $coupon ) {
		if ( ! empty( $order_id ) && ! empty( $coupon ) ) {
			$renewal_coupons   = WC()->session->get( 'awc_renewal_coupons', array() );
			$use_bools         = AWC_Subscriptions::is_woocommerce_pre( '3.0' ); // Some coupon properties have changed from accepting 'no' and 'yes' to true and false args.
			...
			if ( array_key_exists( $order_id, $renewal_coupons ) ) {
				$renewal_coupons[ $order_id ][ awc_get_coupon_property( $coupon, 'code' ) ] = $coupon_properties;
			} else {
				$renewal_coupons[ $order_id ] = array( awc_get_coupon_property( $coupon, 'code' ) => $coupon_properties );
			}

			WC()->session->set( 'awc_renewal_coupons', $renewal_coupons );
		}
	}
	...
	public function clear_coupons() {
		...
		WC()->session->set( 'awc_renewal_coupons', array() );
	}
	...
	protected function set_cart_hash( $order_id ) {
		...
		awc_set_objects_property( $order, 'cart_hash', $cart_hash );
	}
	...
	function maybe_redirect_after_login( $redirect ) {
		if ( isset( $_GET['awc_redirect'], $_GET['awc_redirect_id'] ) && 'pay_for_order' == $_GET['awc_redirect'] ) {
			$order = wc_get_order( $_GET['awc_redirect_id'] );
	 ...
	 *
	 * This is required so that changes made by @see AWC_Cart_Renewal->add_line_item_meta() (or @see
	 * AWC_Cart_Renewal->update_line_item_cart_data() for WC < 3.0), are also reflected
	 * in the session cart.
	 *
	...
	*/
	 ...
	 * @see AWC_Cart_Renewal->update_line_item_cart_data()
	 *
	...
	public function hidden_order_itemmeta( $hidden_meta_keys ) {
		...
		if ( apply_filters( 'woocommerce_subscriptions_hide_itemmeta', ! defined( 'AWC_DEBUG' ) || true !== AWC_DEBUG ) ) {
	 ...
	 * @see AWC_Cart_Renewal->add_order_line_item_meta() replaces this function
	 *
	 * @param int $item_id
	 * @param array $cart_item_data
	 ...
	 *
	 * Used when WC 3.0 or newer is active. When prior versions are active,
	 * @see AWC_Cart_Renewal->add_order_item_meta() replaces this function
	 *
	...
	public function maybe_preserve_order_created_via( $order ) {
		...
		if ( isset( $changes['created_via'], $current_data['created_via'] ) && 'subscription' === $current_data['created_via'] && 'checkout' === $changes['created_via'] && awc_order_contains_renewal( $order ) ) {
	...
	public function maybe_disable_manual_renewal_stock_validation() {
		if ( apply_filters( 'woocommerce_subscriptions_disable_manual_renewal_stock_validation', false ) ) {
			awc_Renewal_Cart_Stock_Manager::attach_callbacks();
		}
	}
	...
	public function maybe_setup_discounts( $subscription, $order = null ) {
		...
		if ( awc_is_subscription( $order ) || awc_order_contains_renewal( $order ) ) {

			$used_coupons   = awc_get_used_coupon_codes( $order );
			$order_discount = awc_get_objects_property( $order, 'cart_discount' );
			...
			if ( ! empty( $used_coupons ) ) {
				...
				foreach ( $coupon_items as $coupon_item ) {
					...
					if ( true === awc_get_coupon_property( $coupon, 'exists' ) ) {
						...
						if ( in_array( $coupon_type, array( 'recurring_percent', 'recurring_fee' ) ) ) {
							...
							if ( 'recurring_percent' == $coupon_type ) {
								awc_set_coupon_property( $coupon, 'discount_type', 'renewal_percent' );
							} elseif ( 'recurring_fee' == $coupon_type ) {
								awc_set_coupon_property( $coupon, 'discount_type', 'renewal_fee' );
							}

							// Adjust coupon code to reflect that it is being applied to a renewal
							$coupon_code = awc_get_coupon_property( $coupon, 'code' );
						}
					} else {
						...
						awc_set_coupon_property( $coupon, 'discount_type', 'renewal_cart' );
						...
						awc_set_coupon_property( $coupon, 'coupon_amount', $coupon_amount );
					}
					...
					if ( ! empty( $coupon_code ) ) {
						...
						awc_set_coupon_property( $coupon, 'product_ids', $this->get_products( $order ) );

						// Store the coupon info for later
						$this->store_coupon( awc_get_objects_property( $order, 'id' ), $coupon );
			...
			// If there are no coupons but there is still a discount (i.e. it might have been manually added), we need to account for that as well
				...
				awc_set_coupon_property( $coupon, 'coupon_amount', $order_discount );
