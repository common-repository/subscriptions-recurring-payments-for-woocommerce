...
class AWC_Change_Payment_Method_Admin {
	...
	public static function display_fields( $subscription ) {
		...
		if ( ! $subscription->is_manual() && ! isset( $valid_payment_methods[ $payment_method ] ) ) {
			$subscription_payment_gateway = AWC_Subscription_Payment_Gateways::get_payment_gateway( $payment_method );
		...
		if ( count( $valid_payment_methods ) > 1 ) {
			...
			$found_method = false;
			echo '<label>' . esc_html__( 'Payment Method', 'subscriptions-recurring-payments-for-woocommerce' ) . ':</label>';
			echo '<select class="awc_payment_method_selector" name="_payment_method" id="_payment_method" class="first">';
			...
			echo '<strong>' . esc_html__( 'Payment Method', 'subscriptions-recurring-payments-for-woocommerce' ) . ':</strong><br/>' . esc_html( current( $valid_payment_methods ) );
			// translators: %s: gateway ID.
			echo awc_help_tip( sprintf( _x( 'Gateway ID: [%s]', 'The gateway ID displayed on the Edit Subscriptions screen when editing payment method.', 'subscriptions-recurring-payments-for-woocommerce' ), key( $valid_payment_methods ) ) );
		...
		if ( is_array( $payment_method_table ) ) {
			...
			foreach ( $payment_method_table as $payment_method_id => $payment_method_meta ) {
				...
				echo '<div class="awc_payment_method_meta_fields" id="awc_' . esc_attr( $payment_method_id ) . '_fields" ' . ( ( $payment_method_id != $payment_method || $subscription->is_manual() ) ? 'style="display:none;"' : '' ) . ' >';
		...
		wp_nonce_field( 'awc_change_payment_method_admin', '_asubnonce' );
	...
	public static function save_meta( $subscription ) {
		...
		if ( empty( $_POST['_asubnonce'] ) || ! wp_verify_nonce( $_POST['_asubnonce'], 'awc_change_payment_method_admin' ) ) {
			return;
		}
		...
		if ( ! isset( $valid_payment_methods[ $payment_method ] ) && ! ( isset( $payment_gateways[ $payment_method ] ) && $subscription->get_payment_method() == $payment_gateways[ $payment_method ]->id ) ) {
			throw new Exception( __( 'Please choose a valid payment gateway to change to.', 'subscriptions-recurring-payments-for-woocommerce' ) );
		}
		...
		if ( ! $subscription->is_manual() && ( '' == $payment_gateway || $subscription->get_payment_method() != $payment_gateway->id ) ) {
			...
			$gateway_status = apply_filters( 'awc_gateway_status_payment_changed', 'cancelled', $subscription, $payment_gateway );

			AWC_Subscription_Payment_Gateways::trigger_gateway_status_updated_hook( $subscription, $gateway_status );
		}
	...
	public static function get_valid_payment_methods( $subscription ) {
		...
		$valid_gateways = array( 'manual' => __( 'Manual Renewal', 'subscriptions-recurring-payments-for-woocommerce' ) );

		$available_gateways = WC()->payment_gateways->get_available_payment_gateways();

		foreach ( $available_gateways as $gateway_id => $gateway ) {

			if ( $gateway->supports( 'subscription_payment_method_change_admin' ) && false == AWC_Settings::get_option('turn_of_automatic_payment') || ( ! $subscription->is_manual() && $gateway_id == $subscription->get_payment_method() ) ) {
