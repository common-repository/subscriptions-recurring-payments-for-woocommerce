 ...
 *
 */
class AWC_Switch_Totals_Calculator {
	...
	protected function load_settings() {
		$this->apportion_recurring_price = get_option( AWC_Subscriptions_Admin::$option_prefix . '_apportion_recurring_price', 'no' );
		$this->apportion_sign_up_fee     = get_option( AWC_Subscriptions_Admin::$option_prefix . '_apportion_sign_up_fee', 'no' );
		$this->apportion_length          = get_option( AWC_Subscriptions_Admin::$option_prefix . '_apportion_length', 'no' );
	...
	public function calculate_prorated_totals() {
		foreach ( $this->get_switches_from_cart() as $cart_item_key => $switch_item ) {
			...
			if ( defined( 'AWC_DEBUG' ) && AWC_DEBUG && ! awc_doing_ajax() ) {
	...
	/**
	 * Gets all the switch items in the cart as instances of @see AWC_Switch_Cart_Item.
	 *
	 * @return AWC_Switch_Cart_Item[]
	 */
	protected function get_switches_from_cart() {
		...
		foreach ( $this->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( ! empty( $cart_item['subscription_switch']['item_id'] ) ) {
				...
				$switch_item = new AWC_Add_Cart_Item( $cart_item, $subscription );
			}
			 ...
			 *
			 *
			 * @param AWC_Switch_Cart_Item $switch_item The switch item.
			 * @param array $cart_item The item in the cart the switch item was created for.
			 * @param string $cart_item_key The cart item key.
			 */
			$switches[ $cart_item_key ] = apply_filters( 'awc_proration_switch_item_from_cart_item', $switch_item, $cart_item, $cart_item_key );

			// Ensure the filtered item is the correct object type.
			if ( ! is_a( $switches[ $cart_item_key ], 'AWC_Switch_Cart_Item' ) ) {
				unset( $switches[ $cart_item_key ] );
				WC()->cart->remove_cart_item( $cart_item_key );

				$error_notice = __( 'Your cart contained an invalid subscription switch request. It has been removed from your cart.', 'subscriptions-recurring-payments-for-woocommerce' );
	 ...
	 * 
	 * @param AWC_Switch_Cart_Item $switch_item
	 * @return bool
	 */
	...
	protected function should_reduce_prepaid_term( $switch_item ) {
		 ...
		 */
		return apply_filters( 'awc_switch_proration_reduce_pre_paid_term', $is_switch_out_of_trial || $days_in_old_cycle > $days_in_new_cycle, $switch_item->subscription, $switch_item->cart_item, $days_in_old_cycle, $days_in_new_cycle, $switch_item->get_old_price_per_day(), $switch_item->get_new_price_per_day() );
	}
	...
	protected function apportion_sign_up_fees( $switch_item ) {
		if ( 'no' === $this->apportion_sign_up_fee ) {
			...
			$sign_up_fee_due  = AWC_Subscription_Products::get_sign_up_fee( $product );
			...
			$switch_item->product->update_meta_data( '_subscription_sign_up_fee_prorated', AWC_Subscription_Products::get_sign_up_fee( $switch_item->product ) );
		}
	}
	...
	protected function calculate_upgrade_cost( $switch_item ) {
		...
		if ( ! $switch_item->is_switch_during_trial() && 1 == AWC_Subscription_Products::awc_get_length( $switch_item->product ) ) {
		...
		return apply_filters( 'awc_switch_proration_extra_to_pay', $extra_to_pay, $switch_item->subscription, $switch_item->cart_item, $switch_item->get_days_in_old_cycle() );
	}
	...
	protected function apportion_length( $switch_item ) {
		$base_length        = AWC_Subscription_Products::awc_get_length( $switch_item->canonical_product_id );
	...
	public function set_upgrade_cost( $switch_item, $extra_to_pay ) {
		...
		$existing_sign_up_fee = AWC_Subscription_Products::get_sign_up_fee( $switch_item->product );
	...
	/**
	 * Logs the switch item data to the asub-switch-cart-items file.
	 *
	...
	protected function log_switch( $switch_item ) {
		...
		if ( ! isset( $items_logged[ $key ] ) ) {
			...
			$logger->info( implode( PHP_EOL, $messages ), array( 'source' => 'asub-switch-cart-items' ) );
		}
	}
