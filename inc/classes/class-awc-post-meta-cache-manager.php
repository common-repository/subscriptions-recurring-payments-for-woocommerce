 ...
 *
 */
class AWC_Post_Meta_Cache_Manager {
	...
	protected function maybe_update_for_post_change( $update_type, $post_id ) {
		...
		if ( ! in_array( $update_type, array( 'add', 'delete' ) ) ) {
			...
			throw new InvalidArgumentException( sprintf( __( 'Invalid update type: %s. Post update types supported are "add" or "delete". Updates are done on post meta directly.', 'subscriptions-recurring-payments-for-woocommerce' ), $update_type ) );
		}
	...
	protected function trigger_update_cache_hook( $update_type, $post_id, $meta_key, $meta_value, $prev_value = '' ) {
		do_action( 'awc_update_post_meta_caches', $update_type, $post_id, $meta_key, $meta_value, $prev_value );
	}
	...
	protected function trigger_delete_all_caches_hook( $meta_key ) {
		do_action( 'awc_delete_all_post_meta_caches', $meta_key );
	}
