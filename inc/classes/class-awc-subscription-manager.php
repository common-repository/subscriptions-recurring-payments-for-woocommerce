 ...
 *
 * @package     Aco Subscriptions
 * @subpackage  AWC_Subscription_Manager
 * @category    Class
 
 */
class AWC_Subscription_Manager {
	...
	public static function prepare_renewal( $subscription_id ) {
		...
		$order_note = _x( 'Subscription renewal payment due:', 'used in order note as reason for why subscription status changed', 'subscriptions-recurring-payments-for-woocommerce' );
		...
		if ( ! empty( $subscription ) && $subscription->has_status( $required_status ) && ( 0 == $subscription->get_total() || $subscription->is_manual() || '' == $subscription->get_payment_method() || ! $subscription->payment_method_supports( 'gateway_scheduled_payments' ) ) ) {
			...
			if ( is_wp_error( $renewal_order ) ) {
				...
				if ( is_wp_error( $renewal_order ) ) {
					...
					throw new Exception( sprintf( __( 'Error: Unable to create renewal order with note "%s"', 'subscriptions-recurring-payments-for-woocommerce' ), $order_note ) );
				}
			}
			...
			if ( 0 == $renewal_order->get_total() ) {
				...
				if ( $subscription->is_manual() ) {
					do_action( 'woocommerce_generated_manual_renewal_order', awc_get_objects_property( $renewal_order, 'id' ), $subscription );
					$renewal_order->add_order_note( __( 'Manual renewal order awaiting customer payment.', 'subscriptions-recurring-payments-for-woocommerce' ) );
				} else {
	...
	public static function process_subscription_payment( $user_id, $subscription_key ) {
		_deprecated_function( __METHOD__, '2.0', 'AWC_Subscription::payment_complete()' );
		...
		if ( ! empty( $subscriptions ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				try {
					...
					$subscription->add_order_note( sprintf( __( 'Failed to activate subscription status for order #%1$s: %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), is_object( $order ) ? $order->get_order_number() : $order, $e->getMessage() ) );
				}
			}
	...
	public static function put_subscription_on_hold_for_order( $order ) {
		...
		$subscriptions = awc_get_subscriptions_for_order( $order, array( 'order_type' => 'parent' ) );
		...
		if ( ! empty( $subscriptions ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				try {
					if ( ! $subscription->has_status( awc_get_subscription_ended_statuses() ) ) {
						$subscription->update_status( 'on-hold' );
					}
				} catch ( Exception $e ) {
					// translators: $1: order number, $2: error message
					$subscription->add_order_note( sprintf( __( 'Failed to update subscription status after order #%1$s was put on-hold: %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), is_object( $order ) ? $order->get_order_number() : $order, $e->getMessage() ) );
				}
			}
		...
		if ( ! empty( $subscriptions ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				try {
					...
					// translators: $1: order number, $2: error message
					$subscription->add_order_note( sprintf( __( 'Failed to cancel subscription after order #%1$s was cancelled: %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), is_object( $order ) ? $order->get_order_number() : $order, $e->getMessage() ) );
				}
			}
		...
		if ( ! empty( $subscriptions ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				try {
					...
					// translators: $1: order number, $2: error message
					$subscription->add_order_note( sprintf( __( 'Failed to set subscription as expired for order #%1$s: %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), is_object( $order ) ? $order->get_order_number() : $order, $e->getMessage() ) );
				}
			}
	...
	public static function failed_subscription_sign_ups_for_order( $order ) {
		...
		if ( ! empty( $subscriptions ) ) {
			...
			if ( $order->has_status( 'failed' ) ) {
				$order->update_status( 'failed', __( 'Subscription sign up failed.', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}
			...
			foreach ( $subscriptions as $subscription ) {
				...
				try {
					...
					// translators: $1: order number, $2: error message
					$subscription->add_order_note( sprintf( __( 'Failed to process failed payment on subscription for order #%1$s: %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), is_object( $order ) ? $order->get_order_number() : $order, $e->getMessage() ) );
				}
			}
	...
	public static function create_pending_subscription_for_order( $order, $product_id, $args = array() ) {
		...
		$args = wp_parse_args( $args, array(
			'start_date'  => awc_get_datetime_utc_string( awc_get_objects_property( $order, 'date_created' ) ), // get_date_created() can return null, but if it does, we have an error anyway
			'expiry_date' => '',
		) );

		$billing_period   = AWC_Subscription_Products::awc_get_period( $product_id );
		$billing_interval = AWC_Subscription_Products::awc_get_interval( $product_id );
		...
		$subscriptions = awc_get_subscriptions(
			array(
		...
		if ( ! empty( $subscriptions ) ) {
			...
			if ( is_wp_error( $subscription ) ) {
				throw new Exception( __( 'Error: Unable to create subscription. Please try again.', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}
			...
			if ( ! $item_id ) {
				throw new Exception( __( 'Error: Unable to add product to created subscription. Please try again.', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}
		}

		// Make sure some of the meta is copied form the order rather than the store's defaults
		if ( awc_get_objects_property( $order, 'prices_include_tax' ) ) {
			$prices_include_tax = 'yes';
		} else {
			$prices_include_tax = 'no';
		}
		update_post_meta( $subscription->get_id(), '_order_currency', awc_get_objects_property( $order, 'currency' ) );
		...
		if ( ! empty( $args['expiry_date'] ) ) {
			...
			$expiration = AWC_Subscription_Products::get_expiration_date( $product_id, $args['start_date'] );
		}

		// Adding a new subscription so set the expiry date/time from the order date
		$trial_expiration = AWC_Subscription_Products::get_trial_expiration_date( $product_id, $args['start_date'] );
		...
		$subscription->add_order_note( __( 'Pending subscription created.', 'subscriptions-recurring-payments-for-woocommerce' ) );
	...
	public static function process_subscriptions_on_checkout( $order ) {
		...
		if ( ! empty( $_POST['_wpnonce'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'woocommerce-process_checkout' ) ) {
			AWC_Subscriptions_Checkout::process_checkout( $order, $_POST );
		}
	}
	...
	public static function update_users_subscriptions_for_order( $order, $status = 'pending' ) {
		...
		foreach ( awc_get_subscriptions_for_order( awc_get_objects_property( $order, 'id' ), array( 'order_type' => 'parent' ) ) as $subscription_id => $subscription ) {
	 ...
	 * @param array $subscriptions An array of arrays with a subscription key and corresponding 'detail' 
	 */
	 ...
	 * @param array $new_subscription_details An array like array('detail' => 'value' pair. Can alter any of these details:
	...
	public static function update_subscription( $subscription_key, $new_subscription_details ) {
		...
		return awc_get_subscription_in_deprecated_structure( $subscription );
	}
	...
	public static function clear_users_subscriptions_from_order( $order ) {
		...
		foreach ( awc_get_subscriptions_for_order( $order, array( 'order_type' => 'parent' ) ) as $subscription_id => $subscription ) {
			wp_delete_post( $subscription->get_id() );
		}
	...
	public static function maybe_trash_subscription( $post_id ) {
		...
		if ( 'shop_order' == get_post_type( $post_id ) ) {
			...
			// delete subscription
			foreach ( awc_get_subscriptions_for_order( $post_id, array( 'order_type' => 'parent' ) ) as $subscription ) {
	...
	public static function maybe_untrash_subscription( $post_id ) {
		if ( 'shop_order' == get_post_type( $post_id ) ) {
			foreach ( awc_get_subscriptions_for_order( $post_id, array( 'order_type' => 'parent', 'subscription_status' => array( 'trash' ) ) ) as $subscription ) { // phpcs:ignore WordPress.Arrays.ArrayDeclarationSpacing.AssociativeArrayFound
	...
	public static function maybe_delete_subscription( $post_id ) {
		...
		$subscriptions = awc_get_subscriptions_for_order( $post_id, array(
	...
	public static function maybe_change_users_subscription() {
		...
		AWC_User_Change_Status_Handler::maybe_change_users_subscription();
	}
	...
	public static function can_subscription_be_changed_to( $new_status_or_meta, $subscription_key, $user_id = '' ) {
		...
		if ( 'new-payment-date' == $new_status_or_meta ) {
			_deprecated_argument( __METHOD__, '2.0', 'The "new-payment-date" parameter value is deprecated. Use AWC_Subscription::can_date_be_updated( "next_payment" ) method instead.' );
	...
	public static function get_subscription( $subscription_key, $deprecated = null ) {
		...
		try {
			$subscription = awc_get_subscription_from_key( $subscription_key );
			$subscription = awc_get_subscription_in_deprecated_structure( $subscription );
		} catch ( Exception $e ) {
			$subscription = array();
		}
	...
	public static function get_status_to_display( $status, $subscription_key = '', $user_id = 0 ) {
		...
		switch ( $status ) {
			case 'active':
				$status_string = _x( 'Active', 'Subscription status', 'subscriptions-recurring-payments-for-woocommerce' );
				break;
			case 'cancelled':
				$status_string = _x( 'Cancelled', 'Subscription status', 'subscriptions-recurring-payments-for-woocommerce' );
				break;
			case 'expired':
				$status_string = _x( 'Expired', 'Subscription status', 'subscriptions-recurring-payments-for-woocommerce' );
				break;
			case 'pending':
				$status_string = _x( 'Pending', 'Subscription status', 'subscriptions-recurring-payments-for-woocommerce' );
				break;
			case 'failed':
				$status_string = _x( 'Failed', 'Subscription status', 'subscriptions-recurring-payments-for-woocommerce' );
				break;
			case 'on-hold':
			case 'suspend': // Backward compatibility
				$status_string = _x( 'On-hold', 'Subscription status', 'subscriptions-recurring-payments-for-woocommerce' );
				break;
			default:
	...
	public static function get_subscription_period_strings( $number = 1, $period = '' ) {
		return awc_get_subscription_period_strings( $number, $period );
	}
	...
	public static function get_subscription_period_interval_strings( $interval = '' ) {
		return awc_get_subscription_period_interval_strings( $interval );
	}
	...
	public static function get_subscription_ranges( $subscription_period = '' ) {
		return awc_get_subscription_ranges( $subscription_period );
	}
	...
	public static function get_subscription_trial_lengths( $subscription_period = '' ) {
		return awc_get_subscription_trial_lengths( $subscription_period );
	}
	...
	public static function get_subscription_trial_period_strings( $number = 1, $period = '' ) {
		return awc_get_subscription_trial_period_strings( $number, $period );
	}
	...
	public static function get_available_time_periods( $form = 'singular' ) {
		return awc_get_available_time_periods( $form );
	}
	...
	public static function get_subscription_key( $order_id, $product_id = '' ) {
		...
		if ( awc_order_contains_renewal( $order_id ) ) {
			$order_id = AWC_Subscription_Renewal_Order::get_parent_order_id( $order_id );
		}

		// Get the ID of the first order item in a subscription created by this order
		if ( empty( $product_id ) ) {

			$subscriptions = awc_get_subscriptions_for_order( $order_id, array( 'order_type' => 'parent' ) );
	...
	public static function get_subscriptions_failed_payment_count( $subscription_key, $user_id = '' ) {
		return apply_filters( 'woocommerce_subscription_failed_payment_count', awc_get_subscription_from_key( $subscription_key )->get_failed_payment_count(), $user_id, $subscription_key );
	}
	...
	public static function get_subscriptions_completed_payment_count( $subscription_key ) {
		return apply_filters( 'woocommerce_subscription_completed_payment_count', awc_get_subscription_from_key( $subscription_key )->get_payment_count(), $subscription_key );
	}
	 ...
	 *
	 * @param string $subscription_key (A subscription key)
	 ...
	 * product (because a AWC_Subscription object can have more than one product and syncing length with expiration date was both
	...
	public static function calculate_next_payment_date( $subscription_key, $user_id = '', $type = 'mysql', $from_date = '' ) {
		...
		return ( 'mysql' == $type ) ? $next_payment : awc_date_to_time( $next_payment );
	}
	...
	public static function calculate_trial_expiration_date( $subscription_key, $user_id = '', $type = 'mysql' ) {
		...
		$trial_end    = ( 'mysql' == $type ) ? $trial_end : awc_date_to_time( $trial_end );
	 ...
	 * @return bool | null True if the subscription exists
	 * 
	 */
	public static function requires_manual_renewal( $subscription_key, $user_id = '' ) {
		return awc_get_subscription_from_key( $subscription_key )->is_manual();
	}
	...
	public static function subscription_requires_payment( $subscription_key, $user_id ) {
		$subscription = awc_get_subscription_from_key( $subscription_key );
		return apply_filters( 'woocommerce_subscription_requires_payment', $subscription->needs_payment(), awc_get_subscription_in_deprecated_structure( $subscription ), $subscription_key, $user_id );
	}
	...
	public static function user_has_subscription( $user_id = 0, $product_id = '', $status = 'any' ) {
		return awc_user_has_subscription( $user_id, $product_id, $status );
	}
	...
	public static function get_all_users_subscriptions() {
		...
		foreach ( get_users() as $user ) {
			foreach ( awc_get_users_subscriptions( $user->ID ) as $subscription ) {
				$subscriptions_in_old_format[ awc_get_old_subscription_key( $subscription ) ] = awc_get_subscription_in_deprecated_structure( $subscription );
			}
		}
	...
	public static function get_users_subscriptions( $user_id = 0, $order_ids = array() ) {
		...
		foreach ( awc_get_users_subscriptions( $user_id ) as $subscription ) {
			$subscriptions_in_old_format[ awc_get_old_subscription_key( $subscription ) ] = awc_get_subscription_in_deprecated_structure( $subscription );
		}
	...
	public static function make_user_inactive( $user_id ) {
		awc_make_user_inactive( $user_id );
	}
	...
	public static function maybe_assign_user_cancelled_role( $user_id ) {
		awc_maybe_make_user_inactive( $user_id );
	}
	 ...
	 *
	 * @param int $user_id The id of the user whose role should be changed
	 * @param string $role_name Either a WordPress role or one of the asub keys: 'default_subscriber_role' or 'default_cancelled_role'
	 
	 * 
	 */
	public static function update_users_role( $user_id, $role_name ) {
		awc_update_users_role( $user_id, $role_name );
	}
	...
	public static function get_users_change_status_link( $subscription_key, $status ) {
		...
		return apply_filters( 'woocommerce_subscriptions_users_action_link', awc_get_users_change_status_link( $subscription_id, $status, $current_status ), $subscription_key, $status );
	}
	...
	public static function update_next_payment_date( $new_payment_date, $subscription_key, $user_id = '', $timezone = 'server' ) {
		...
		$new_payment_timestamp = ( is_numeric( $new_payment_date ) ) ? $new_payment_date : awc_date_to_time( $new_payment_date );
	 ...
	 *
	 * @param array $subscription_details A set of array like array( name => value pairs for the subscription details to include in the string. Available keys:
	 ...
	 * 
	 * @return float $proportion (e.g. 0.5 is half of the total)
	 */
	public static function get_subscription_price_string( $subscription_details ) {
		return awc_price_string( $subscription_details );
	}
	...
	public static function touch_time( $args = array() ) {
		...
		for ( $i = 1; $i < 13; $i = $i + 1 ) {
			...
			$month_input .= '>' . sprintf( _x( '%1$s-%2$s', 'used in a select box', 'subscriptions-recurring-payments-for-woocommerce' ), $month_numeral, $wp_locale->get_month_abbrev( $wp_locale->get_month( $i ) ) ) . "</option>\n";
		}
		$month_input .= '</select>';
		...
		if ( $args['include_time'] ) {
			...
			$touch_time = sprintf( __( '%1$s%2$s, %3$s @ %4$s : %5$s', 'subscriptions-recurring-payments-for-woocommerce' ), $month_input, $day_input, $year_input, $hour_input, $minute_input );

		} else {
			// translators: all fields are full html nodes: 1$: month input, 2$: day input, 3$: year input. Change the order if you'd like
			$touch_time = sprintf( __( '%1$s%2$s, %3$s', 'subscriptions-recurring-payments-for-woocommerce' ), $month_input, $day_input, $year_input );
		}

		if ( $args['include_buttons'] ) {
			$touch_time .= '<p>';
			$touch_time .= '<a href="#edit_timestamp" class="save-timestamp hide-if-no-js button">' . __( 'Change', 'subscriptions-recurring-payments-for-woocommerce' ) . '</a>';
			$touch_time .= '<a href="#edit_timestamp" class="cancel-timestamp hide-if-no-js">' . _x( 'Cancel', 'an action on a subscription', 'subscriptions-recurring-payments-for-woocommerce' ) . '</a>';
			$touch_time .= '</p>';
		}
	...
	public static function maybe_put_subscription_on_hold( $user_id, $subscription_key ) {
		...
		if ( 0 == $subscription->get_total() || $subscription->is_manual() || '' == $subscription->get_payment_method() || ! $subscription->payment_method_supports( 'gateway_scheduled_payments' ) ) {
			$subscription->update_status( 'on-hold', _x( 'Subscription renewal payment due:', 'used in order note as reason for why subscription status changed', 'subscriptions-recurring-payments-for-woocommerce' ) );
		}
	}
	 ...
	 * bug in upgrade process for 2.0.0 of Subscriptions (i.e. not 2.0.1 or newer). See awc_Repair_2_0_2::maybe_repair_status() for more details.
	 *
	 * @param int $subscription_id The ID of a 'shop_subscription' post
	 .2
	 */
	public static function maybe_process_failed_renewal_for_repair( $subscription_id ) {
		...
		if ( 'true' == get_post_meta( $subscription_id, '_awc_repaired_2_0_2_needs_failed_payment', true ) ) {
			...
			update_post_meta( $subscription_id, '_awc_repaired_2_0_2_needs_failed_payment', 'false' );

			// We've already processed the renewal
			remove_action( 'woocommerce_scheduled_subscription_payment', __CLASS__ . '::prepare_renewal' );
			remove_action( 'woocommerce_scheduled_subscription_payment', 'AWC_Subscription_Payment_Gateways::gateway_scheduled_subscription_payment', 10 );
		}
	}
	...
	public static function maybe_process_subscription_payment( $user_id, $subscription_key ) {
		self::prepare_renewal( awc_get_subscription_id_from_key( $subscription_key ) );
	}
	...
	public static function current_user_can_suspend_subscription( $subscription_key ) {
		return awc_can_user_put_subscription_on_hold( awc_get_subscription_from_key( $subscription_key ) );
	}
	...
	public static function failed_subscription_signup( $user_id, $subscription_key ) {
		...
		$subscription->get_parent()->add_order_note( sprintf( __( 'Failed sign-up for subscription %s.', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription->get_id() ) );
	...
	public static function ajax_update_next_payment_date() {
		...
		if ( ! wp_verify_nonce( $_POST['awc_nonce'], 'subscriptions-recurring-payments-for-woocommerce' ) ) {

			$response['message'] = '<div class="error">' . __( 'Invalid security token, please reload the page and try again.', 'subscriptions-recurring-payments-for-woocommerce' ) . '</div>';

		} elseif ( ! current_user_can( 'manage_woocommerce' ) ) {

			$response['message'] = '<div class="error">' . __( 'Only store managers can edit payment dates.', 'subscriptions-recurring-payments-for-woocommerce' ) . '</div>';

		} elseif ( empty( $_POST['awc_day'] ) || empty( $_POST['awc_month'] ) || empty( $_POST['awc_year'] ) ) {

			$response['message'] = '<div class="error">' . __( 'Please enter all date fields.', 'subscriptions-recurring-payments-for-woocommerce' ) . '</div>';

		} else {

			$new_payment_date      = sprintf( '%s-%s-%s %s', (int) $_POST['awc_year'], zeroise( (int) $_POST['awc_month'], 2 ), zeroise( (int) $_POST['awc_day'], 2 ), gmdate( 'H:i:s', current_time( 'timestamp' ) ) );
			$new_payment_timestamp = self::update_next_payment_date( $new_payment_date, $_POST['awc_subscription_key'], self::get_user_id_from_subscription_key( $_POST['awc_subscription_key'] ), 'user' );
			...
			if ( is_wp_error( $new_payment_timestamp ) ) {
				...
				if ( $time_diff > 0 && $time_diff < 7 * 24 * 60 * 60 ) {
					...
					$date_to_display = sprintf( __( 'In %s', 'subscriptions-recurring-payments-for-woocommerce' ), human_time_diff( gmdate( 'U' ), $new_payment_timestamp ) );
				} else {
					$date_to_display = date_i18n( wc_date_format(), $new_payment_timestamp_user_time );
				}

				$response['status']        = 'success';
				$response['message']       = '<div class="updated">' . __( 'Date Changed', 'subscriptions-recurring-payments-for-woocommerce' ) . '</div>';
