...
if(class_exists('AWC_Switch_Cart_Item')) 
	exit; // Exist if class already exist


class AWC_Switch_Cart_Item {
	 ...
	 * @param AWC_Subscription $subscription        The subscription being switched.
	 * @param WC_Order_Item_Product $existing_item The subscription line item being switched.
	 *
	 * @throws Exception If AWC_Subscription_Products::get_expiration_date() returns an invalid date.
	 */
	public function __construct( $cart_item, $subscription, $existing_item ) {
		$this->cart_item               = $cart_item;
		$this->subscription            = $subscription;
		$this->existing_item           = $existing_item;
		$this->canonical_product_id    = awc_get_canonical_product_id( $cart_item );
		$this->product                 = $cart_item['data'];
		$this->next_payment_timestamp  = $cart_item['subscription_switch']['next_payment_timestamp'];
		$this->end_timestamp           = awc_date_to_time( AWC_Subscription_Products::get_expiration_date( $this->canonical_product_id, $this->subscription->get_date( 'last_order_date_created' ) ) );
	}
	...
	public function get_old_price_per_day() {
		if ( ! isset( $this->old_price_per_day ) ) {
			...
			$this->old_price_per_day = apply_filters( 'awc_switch_proration_old_price_per_day', $old_price_per_day, $this->subscription, $this->cart_item, $total_paid_for_current_period, $days_in_old_cycle );
		}
	...
	public function get_new_price_per_day() {
		if ( ! isset( $this->new_price_per_day ) ) {
			...
			if ( $this->is_switch_during_trial() && $this->trial_periods_match() ) {
				...
				// We need to use the cart items price to ensure we include extras added by extensions like Product Add-ons, but we don't want the sign-up fee accounted for in the price, so make sure AWC_Subscription_Cart::set_subscription_prices_for_calculation() isn't adding that.
				remove_filter( 'woocommerce_product_get_price', 'AWC_Subscription_Cart::set_subscription_prices_for_calculation', 100 );
				$new_price_per_day = ( AWC_Subscription_Products::awc_get_price( $this->product ) * $this->cart_item['quantity'] ) / $days_in_new_cycle;
				add_filter( 'woocommerce_product_get_price', 'AWC_Subscription_Cart::set_subscription_prices_for_calculation', 100, 2 );
			}

			$this->new_price_per_day = apply_filters( 'awc_switch_proration_new_price_per_day', $new_price_per_day, $this->subscription, $this->cart_item, $days_in_new_cycle );
		}
	...
	public function get_last_order_paid_time() {
		if ( ! isset( $this->last_order_paid_time ) ) {
			$last_order = awc_get_last_non_early_renewal_order( $this->subscription );
	...
	public function get_total_paid_for_current_period() {
		if ( ! isset( $this->total_paid_for_current_period ) ) {
			$this->total_paid_for_current_period = AWC_Subscription_Switchers::calculate_total_paid_since_last_order( $this->subscription, $this->existing_item, 'exclude_sign_up_fees' );
		}
	...
	public function get_switch_type() {
		if ( ! isset( $this->switch_type ) ) {
			...
			$switch_type = apply_filters( 'awc_switch_proration_switch_type', $switch_type, $this->subscription, $this->cart_item, $old_price_per_day, $new_price_per_day );

			if ( ! in_array( $switch_type, array( 'upgrade', 'downgrade', 'crossgrade' ) ) ) {
				// translators: placeholder is a switch type.
				throw new UnexpectedValueException( sprintf( __( 'Invalid switch type "%s". Switch must be one of: "upgrade", "downgrade" or "crossgrade".', 'subscriptions-recurring-payments-for-woocommerce' ), $switch_type ) );
			}
	...
	public function calculate_days_in_old_cycle() {
		...
		if ( 'days_between_payments' === $method_to_use ) {
			...
			$days_in_old_cycle = awc_get_days_in_cycle( $this->subscription->get_billing_period(), $this->subscription->get_billing_interval() );
		}

		return apply_filters( 'awc_switch_proration_days_in_old_cycle', $days_in_old_cycle, $this->subscription, $this->cart_item );
	}
	...
	public function calculate_days_in_new_cycle() {
		...
		$new_billing_period   = AWC_Subscription_Products::awc_get_period( $this->product );
		$new_billing_interval = AWC_Subscription_Products::awc_get_interval( $this->product );
		...
		$days_in_new_cycle = ( awc_add_time( $new_billing_interval, $new_billing_period, $last_order_time ) - $last_order_time ) / DAY_IN_SECONDS;
		...
		return apply_filters( 'awc_switch_proration_days_in_new_cycle', $days_in_new_cycle, $this->subscription, $this->cart_item, $days_in_old_cycle );
	}
	...
	public function trial_periods_match() {
		 ...
		 * We also need to pass the new product's ID so the raw product's trial is used, not the filtered trial set by @see AWC_Subscription_Switchers::maybe_unset_free_trial() && AWC_Subscription_Switchers::maybe_set_free_trial().
		 */
		$matching_length = (int) AWC_Subscription_Products::awc_get_trial_length( $this->product->get_id() ) === (int) AWC_Subscription_Products::awc_get_trial_length( $existing_product );
		$matching_period = AWC_Subscription_Products::awc_get_trial_period( $this->product->get_id() ) === AWC_Subscription_Products::awc_get_trial_period( $existing_product );
