...
class AWC_Subscriptions_Email {
	...
	public static function init() {
		...
		add_action( 'awc_email_subscription_orders_detail', __CLASS__ . '::order_download_details', 10, 4 );
		add_action( 'awc_email_subscription_orders_detail', __CLASS__ . '::order_details', 10, 4 );
	}
	...
	public static function add_emails( $email_classes ) {
		$email_classes['AWC_Email_New_Renewal_Order']              = new AWC_Email_New_Renewal_Order();
		$email_classes['AWC_Email_New_Switch_Order']               = new AWC_Email_New_Switch_Order();
		$email_classes['AWC_Email_Processing_Renewal_Order']       = new AWC_Email_Processing_Renewal_Order();
		$email_classes['AWC_Email_Completed_Renewal_Order']        = new AWC_Email_Completed_Renewal_Order();
		$email_classes['AWC_Email_Customer_On_Hold_Renewal_Order'] = new AWC_Email_Customer_On_Hold_Renewal_Order();
		$email_classes['AWC_Email_Completed_Switch_Order']         = new AWC_Email_Completed_Switch_Order();
		$email_classes['AWC_Email_Customer_Renewal_Invoice']       = new AWC_Email_Customer_Renewal_Invoice();
		$email_classes['AWC_Email_Cancelled_Subscription']         = new AWC_Email_Cancelled_Subscription();
		$email_classes['AWC_Email_Expired_Subscription']           = new AWC_Email_Expired_Subscription();
		$email_classes['AWC_Email_On_Hold_Subscription']           = new AWC_Email_On_Hold_Subscription();
	...
	public static function hook_transactional_emails() {
		...
		if ( AWC_Subscriptions::is_duplicate_site() && ! defined( 'awc_FORCE_EMAIL' ) ) {
		...
		add_action( 'awc_subscriptions_switch_completed', __CLASS__ . '::send_switch_order_email', 10 );
	...
	public static function maybe_remove_woocommerce_email( $order_id ) {
		if ( awc_order_contains_renewal( $order_id ) || awc_order_contains_switch( $order_id ) ) {
	...
	public static function renewal_order_emails_available( $available_emails ) {
		...
		if ( awc_order_contains_renewal( awc_get_objects_property( $theorder, 'id' ) ) ) {
	...
	public static function order_details( $order, $sent_to_admin = false, $plain_text = false, $email = '' ) {
		...
		$template_path = ( $plain_text ) ? 'emails/text/email-order-details.php' : 'emails/email-order-details.php';
		$order_type    = ( awc_is_subscription( $order ) ) ? 'subscription' : 'order';
	...
	public static function attach_woocommerce_transactional_email( $hook = '', $priority = 10, $accepted_args = 10 ) {
		...
		if ( false === AWC_Subscriptions::is_woocommerce_pre( '3.0' ) && apply_filters( 'woocommerce_defer_transactional_emails', false ) ) {
