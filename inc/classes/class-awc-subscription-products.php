 ...
 *
 * @package    Aco Woo Subscriptions
 * @subpackage AWC_Subscription_Products
 * @category   Class
 * 
 */
...
class AWC_Subscription_Products{
	...
	public static function init() {
		...
		add_action( 'woocommerce_variable_product_sync_data', __CLASS__ . '::awc_variable_subscription_product_sync', 10 );
		...
		add_action( 'wp_ajax_awc_product_has_trial_or_is_synced', __CLASS__ . '::check_product_variations_for_syncd_or_trial' );
		...
		add_action( 'wp_ajax_awc_update_one_time_shipping', __CLASS__ . '::maybe_update_one_time_shipping_on_variation_edits' );
	...
	public static function get_add_to_cart_text() {
		...
		return apply_filters( 'aco_subscription_add_to_cart_text', AWC_Settings::get_option('cart_label') ? AWC_Settings::get_option('cart_label') : __('Sign up now', 'subscriptions-recurring-payments-for-woocommerce') );
	}
	...
	public static function get_parent_ids( $product ) {
		...
		if ( AWC_Subscriptions::is_woocommerce_pre( '3.0' ) && isset( $product->post->post_parent ) ) {
	 ...
	 *
	 * Unlike @see AWC_Subscription_Products::get_parent_ids(), this function will return parent products which still exist, are visible and are a grouped product.
	 *
	 * @param WC_Product The product object to get parents from.
	...
	public static function get_visible_grouped_parent_product_ids( $product ) {
		...
		foreach ( $parent_product_ids as $index => $product_id ) {
			...
			if ( ! is_a( $parent_product, 'WC_Product' ) || ! $parent_product->is_type( 'grouped' ) || 'publish' !== awc_get_objects_property( $parent_product, 'post_status' ) ) {
	...
	public static function add_to_cart_text( $button_text, $product_type = '' ) {
		...
		if ( self::is_subscription( $product ) || in_array( $product_type, array( 'subscription', 'subscription-variation' ) ) ) {
			$button_text = AWC_Settings::get_option('cart_label') ? AWC_Settings::get_option('cart_label') : __('Sign up now', 'subscriptions-recurring-payments-for-woocommerce');
		}

		return  apply_filters( 'aco_subscription_add_to_cart_text', $button_text, $product_type) ;
	}
	...
	/**
	 * Get total subscription product for set limitation to admin product edit page
	 * @return integer 
	 */
	public static function awc_subscriptions_total($product_type = 'subscription'){ // OR variable-subscription
		$term = get_term_by('name', $product_type, 'product_type' );//for example uncategorized category
        return $term->count;
	}
	...
	public static function user_can_not_delete_subscription( $allcaps, $caps, $args ) {
		...
		if ( isset( $args[0] ) && in_array( $args[0], array( 'delete_post', 'delete_product' ) ) && isset( $args[2] ) && ( ! isset( $_GET['action'] ) || 'untrash' != $_GET['action'] ) && 0 === strpos( get_post_type( $args[2] ), 'product' ) ) {
			...
			if ( false !== $product && 'trash' == awc_get_objects_property( $product, 'post_status' ) && $product->is_type( array( 'subscription', 'variable-subscription', 'subscription_variation' ) ) ) {
	...
	public static function get_trial_expiration_date( $product, $from_date = '' ) {
		...
		$trial_length = self::awc_get_trial_length( $product );
		...
		if ( $trial_length > 0 ) {
			...
			$trial_expiration_date = gmdate( 'Y-m-d H:i:s', awc_add_time( $trial_length, self::awc_get_trial_period( $product ), awc_date_to_time( $from_date ) ) );
	...
	public static function get_expiration_date( $product, $from_date = '' ) {
		...
		$subscription_length = self::awc_get_length( $product );
		...
		if ( $subscription_length > 0 ) {
			...
			if ( self::awc_get_trial_length( $product ) > 0 ) {
				$from_date = self::get_trial_expiration_date( $product, $from_date );
			}

			$expiration_date = gmdate( 'Y-m-d H:i:s', awc_add_time( $subscription_length, self::awc_get_period( $product ), awc_date_to_time( $from_date ) ) );
	...
	public static function needs_one_time_shipping( $product ) {
		$product   = self::awc_maybe_get_product_instance( $product );
		$variation = null;
		if ( $product && $product->is_type( 'variation' ) && is_callable( array( $product, 'get_parent_id' ) ) ) {
			$variation = $product;
			$product   = self::awc_maybe_get_product_instance( $product->get_parent_id() );
		}

		$one_time_shipping = AWC_Settings::get_option( 'onetime_shipping_cost') ? 'yes' : 'no';
		
		if($one_time_shipping == 'no')
			$one_time_shipping = 'yes' === self::awc_get_meta_data( $product, 'subscription_one_time_shipping', 'no' );
			

		return apply_filters( 'awc_subscriptions_product_needs_one_time_shipping', $one_time_shipping, $product, $variation );
	}
	...
	public static function get_sign_up_fee_filter( $price, $product ) {
		...
		return self::awc_get_sign_up_fee( $product );
	}
	...
	public static function get_grouped_price_html( $price, $grouped_product ) {
		...
		foreach ( $grouped_product->get_children() as $child_product_id ) {
			...
			if ( self::awc_is_subscription( $child_product_id ) ) {
				...
				$child_price      = 'incl' == $tax_display_mode ? awc_get_price_including_tax( $child_product, array( 'price' => $child_product->get_price() ) ) : awc_get_price_excluding_tax( $child_product, array( 'price' => $child_product->get_price() ) );
				$sign_up_fee      = 'incl' == $tax_display_mode ? awc_get_price_including_tax( $child_product, array( 'price' => self::awc_get_sign_up_fee( $child_product ) ) ) : awc_get_price_excluding_tax( $child_product, array( 'price' => self::awc_get_sign_up_fee( $child_product ) ) );
				$has_trial        = self::awc_get_trial_length( $child_product ) > 0;
		...
		if ( sizeof( $child_prices ) > 1 ) {
			$price .= awc_get_price_html_from_text( $grouped_product );
		}
	...
	public static function bulk_edit_variations( $bulk_action, $data, $variable_product_id, $variation_ids ) {
		if ( 'delete_all_no_subscriptions' === $bulk_action && isset( $data['allowed'] ) && 'true' == $data['allowed'] ) {
			...
			foreach ( $variation_ids as $variation_id ) {
				...
				$subscriptions = awc_get_product_subscriptions( $variation_id );
	...
	public static function subscription_row_actions( $actions, $post ) {
		...
		if ( ! empty( $the_product ) && ! isset( $actions['untrash'] ) && $the_product->is_type( array( 'subscription', 'variable-subscription', 'subscription_variation' ) ) ) {
			...
			if ( 'trash' == $post->post_status && current_user_can( $post_type_object->cap->edit_post, $post->ID ) ) {
				$actions['untrash'] = "<a
				title='" . esc_attr__( 'Restore this item from the Trash', 'subscriptions-recurring-payments-for-woocommerce' ) . "'
				href='" . wp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ) . "'>" . __( 'Restore', 'subscriptions-recurring-payments-for-woocommerce' ) . '</a>';
			}
		}
	...
	public static function add_variation_removal_flag( $loop, $variation_data, $variation ) {
		$related_subscriptions = awc_get_product_subscriptions( $variation->ID );
		$can_remove            = empty( $related_subscriptions );

		printf( '<input type="hidden" class="asub-can-remove-variation" value="%d" />', intval( $can_remove ) );

		if ( ! $can_remove ) {
			$msg = __( 'This variation can not be removed because it is associated with active subscriptions. To remove this variation, please cancel and delete the subscriptions for it.', 'subscriptions-recurring-payments-for-woocommerce' );
			printf( '<a href="#" class="tips delete asub-can-not-remove-variation-msg" data-tip="%s"></a>', wc_sanitize_tooltip( $msg ) ); // XSS ok.
		}
	}
		...
		if ( 'product_variation' === $post_type && 'variation' === $product_type ) {
			...
			if ( $post ) {
				...
				if ( 'variable-subscription' === $parent_product_type ) {
					...
					WC_Product_Subscription_Variation::test();
	...
	public static function check_product_variations_for_syncd_or_trial() {
		...
		// if ( self::awc_is_subscription( $product ) ) {
			...
			// 	if ( self::awc_get_trial_length( $variation_product ) ) {
			// 		$is_synced_or_has_trial = true;
			// 		break;
			// 	}

			// 	if ( AWC_Synchroniser::awc_is_product_synced( $variation_product ) ) {
			// 		$is_synced_or_has_trial = true;
			// 		break;
			// 	}
			// }
    ...
    /**
	 * Checks a given product is subscription product or not
	 ...
	 */
	public static function awc_is_subscription( $product ) {
		...
		if ( is_object( $product ) ) {
			...
			if ( $product->is_type( array( 'subscription', 'subscription_variation', 'variable-subscription' ) ) &&  AWC_Settings::get_option('enable_subscription')) {
				$is_subscription = true;
			}
		}

		return apply_filters( 'awc_woocommerce_is_subscription', $is_subscription, $product_id, $product );
	 ...
	 */
	public static function awc_maybe_get_product_instance( $product ) {
	 ...
	 */
	public static function awc_get_sale_price( $product, $context = 'view' ) {
	 ...
	 */
	public static function awc_get_regular_price( $product, $context = 'view' ) {
	 ...
	 * 
	 */
	public static function awc_get_price( $product ) {
		$product = self::awc_maybe_get_product_instance( $product );
		...
		$subscription_price = self::awc_get_meta_data( $product, 'subscription_price', 0 );
		$sale_price         = self::awc_get_sale_price( $product );
		$active_price       = ( $subscription_price ) ? $subscription_price : self::awc_get_regular_price( $product );
		...
		return apply_filters( 'awc_woocommerce_subscriptions_product_price', $active_price, $product );
	}

	/**
	 * @return Returns the subscription interval for a product, if it's a subscription type product.
	 *
	 * @param mixed $product A WC_Product object or product ID
	 ...
	 */
	public static function awc_get_interval( $product ) {
		return apply_filters( 'awc_woocommerce_subscriptions_product_period_interval', self::awc_get_meta_data( $product, 'subscription_period_interval', 1, 'use_default_value' ), self::awc_maybe_get_product_instance( $product ) );
	}


	/**
	 * @return Returns the subscription period for a product, if it's a subscription.
	 *
	 * @param mixed $product A WC_Product object or product ID
	 ...
	 */
	public static function awc_get_period( $product ) {
		return apply_filters( 'awc_woocommerce_subscriptions_product_period', self::awc_get_meta_data( $product, 'subscription_period', '' ), self::awc_maybe_get_product_instance( $product ) );
	}
	 ...
	 */
	public static function awc_get_length( $product ) {
		return apply_filters( 'awc_woocommerce_subscriptions_product_length', self::awc_get_meta_data( $product, 'subscription_length', 0, 'use_default_value' ), self::awc_maybe_get_product_instance( $product ) );
	}
	 ...
	 */
	public static function awc_get_sign_up_fee( $product ) {
		return apply_filters( 'awc_woocommerce_subscriptions_product_sign_up_fee', self::awc_get_meta_data( $product, 'subscription_sign_up_fee', 0, 'use_default_value' ), self::awc_maybe_get_product_instance( $product ) );
	}
	 ...
	 */
	public static function awc_variable_subscription_product_sync( $product ) {
		...
		if ( self::awc_is_subscription( $product ) ) {
			...
			if ( $child_variation_ids ) {
				...
				$min_max_data = awc_get_min_max_variation_data( $product, $child_variation_ids );
	 ...
	 */
	public static function awc_get_price_string( $product, $include = array() ) {
		global $wp_locale;
		...
		$include = apply_filters( 'awc_woocommerce_subscriptions_product_price_string_inclusions', $include, $product );

		$base_price          = self::awc_get_price( $product );
		$billing_interval    = (int) self::awc_get_interval( $product );
		$billing_period      = self::awc_get_period( $product );
		$subscription_length = (int) self::awc_get_length( $product );
		$trial_length        = (int) self::awc_get_trial_length( $product );
		$trial_period        = self::awc_get_trial_period( $product );
		$sign_up_fee         = 0;
		$include_length      = $include['subscription_length'] && 0 !== $subscription_length;

		if ( $include_length ) {
			$ranges = awc_get_subscription_ranges( $billing_period );
		}

		if ( $include['sign_up_fee'] ) {
			$sign_up_fee = is_bool( $include['sign_up_fee'] ) ? self::awc_get_sign_up_fee( $product ) : $include['sign_up_fee'];
		}

		if ( $include['tax_calculation'] ) {
			if ( in_array( $include['tax_calculation'], array( 'exclude_tax', 'excl' ), true ) ) {
				// Calculate excluding tax.
				$price = isset( $include['price'] ) ? $include['price'] : awc_get_price_excluding_tax( $product );
				if ( true === $include['sign_up_fee'] ) {
					$sign_up_fee = awc_get_price_excluding_tax( $product, array( 'price' => self::awc_get_sign_up_fee( $product ) ) );
				}
			} else {
				// Calculate including tax.
				$price = isset( $include['price'] ) ? $include['price'] : awc_get_price_including_tax( $product );
				if ( true === $include['sign_up_fee'] ) {
					$sign_up_fee = awc_get_price_including_tax( $product, array( 'price' => self::awc_get_sign_up_fee( $product ) ) );
				}
			}
		} else {
		...
		$price .= ' <span class="subscription-details omar1">';
		...
		if ( $include['subscription_price'] && $include['subscription_period'] ) { // Allow extensions to not show price or billing period e.g. Name Your Price.
			if ( $include_length && $subscription_length === $billing_interval ) {
			...
			} elseif ( AWC_Synchroniser::awc_is_product_synced( $product ) && in_array( $billing_period, array( 'week', 'month', 'year' ), true ) ) {
				$subscription_string = '';
				...
				if ( WC_Subscriptions_Synchroniser::is_payment_upfront( $product ) && ! WC_Subscriptions_Synchroniser::is_today( WC_Subscriptions_Synchroniser::calculate_first_payment_date( $product, 'timestamp' ) ) ) {
					...
					$subscription_string = sprintf( __( '%1$s now, and ', 'subscriptions-recurring-payments-for-woocommerce' ), $price );
				}
				...
				switch ( $billing_period ) {
					case 'week':
						...
						if ( 1 === $billing_interval ) {
							...
							$subscription_string .= sprintf( __( '%1$s every %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), $price, $payment_day_of_week );
						} else {
							$subscription_string .= sprintf(
								...
								__( '%1$s every %2$s on %3$s', 'subscriptions-recurring-payments-for-woocommerce' ),
								$price,
								awc_get_subscription_period_strings( $billing_interval, $billing_period ),
								$payment_day_of_week
							);
						}
						break;
					case 'month':
						if ( 1 === $billing_interval ) {
							if ( $payment_day > 27 ) {
								...
								$subscription_string .= sprintf( __( '%s on the last day of each month', 'subscriptions-recurring-payments-for-woocommerce' ), $price );
							} else {
								$subscription_string .= sprintf(
									// translators: 1$: recurring amount, 2$: day of the month (e.g. "23rd") (e.g. "$5 every 23rd of each month").
									__( '%1$s on the %2$s of each month', 'subscriptions-recurring-payments-for-woocommerce' ),
									$price,
									AWC_Subscriptions::append_numeral_suffix( $payment_day )
								);
							}
						} else {
							if ( $payment_day > 27 ) {
								$subscription_string .= sprintf(
									...
									__( '%1$s on the last day of every %2$s month', 'subscriptions-recurring-payments-for-woocommerce' ),
									$price,
									AWC_Subscriptions::append_numeral_suffix( $billing_interval )
								);
							} else {
								$subscription_string .= sprintf(
									// translators: 1$: <price> on the, 2$: <date> day of every, 3$: <interval> month (e.g. "$10 on the 23rd day of every 2nd month").
									__( '%1$s on the %2$s day of every %3$s month', 'subscriptions-recurring-payments-for-woocommerce' ),
									$price,
									AWC_Subscriptions::append_numeral_suffix( $payment_day ),
									AWC_Subscriptions::append_numeral_suffix( $billing_interval )
								);
					...
					case 'year':
						if ( 1 === $billing_interval ) {
							$subscription_string .= sprintf(
								...
								__( '%1$s on %2$s %3$s each year', 'subscriptions-recurring-payments-for-woocommerce' ),
								$price,
								$wp_locale->month[ $payment_day['month'] ],
								AWC_Subscriptions::append_numeral_suffix( $payment_day['day'] )
							);
						} else {
							$subscription_string .= sprintf(
								// translators: 1$: recurring amount, 2$: month (e.g. "March"), 3$: day of the month (e.g. "23rd").
								__( '%1$s on %2$s %3$s every %4$s year', 'subscriptions-recurring-payments-for-woocommerce' ),
								$price,
								$wp_locale->month[ $payment_day['month'] ],
								AWC_Subscriptions::append_numeral_suffix( $payment_day['day'] ),
								AWC_Subscriptions::append_numeral_suffix( $billing_interval )
				...
				$subscription_string = sprintf(
					...
					_n( '%1$s / %2$s', '%1$s every %2$s', $billing_interval, 'subscriptions-recurring-payments-for-woocommerce' ),
					$price,
					awc_get_subscription_period_strings( $billing_interval, $billing_period )
				);
			}
			...
			$subscription_string = '<span class="subscription-details omar2">' . sprintf(
				// translators: billing period (e.g. "every week").
				__( 'every %s', 'subscriptions-recurring-payments-for-woocommerce' ),
				awc_get_subscription_period_strings( $billing_interval, $billing_period )
			);
		} else {
			$subscription_string = '<span class="subscription-details omar3">';
		}
		...
		if ( $include_length ) {
			...
			// update_option( 'tests', array('range' => $ranges, 's_length' => $subscription_length) );
			$subscription_string = sprintf( __( '%1$s for %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription_string, $ranges[ $subscription_length ] );
		}

		if ( $include['trial_length'] && 0 !== $trial_length ) {
			$trial_string = awc_get_subscription_trial_period_strings( $trial_length, $trial_period );
			...
			$subscription_string = sprintf( __( '%1$s with %2$s free trial', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription_string, $trial_string );
		}

		if ( $include['sign_up_fee'] && self::awc_get_sign_up_fee( $product ) > 0 ) {
			...
			$subscription_string = sprintf( __( '%1$s and a %2$s sign-up fee', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription_string, $sign_up_fee );
		}

		$subscription_string .= '</span>';

		return apply_filters( 'awc_woocommerce_subscriptions_product_price_string', $subscription_string, $product, $include );
	}
	 ...
	 */
	public static function awc_get_trial_length( $product ) {
		return apply_filters( 'shop_subscriptions_product_trial_length', self::awc_get_meta_data( $product, '_subscription_trial_length', 0, 'use_default_value' ), self::awc_maybe_get_product_instance( $product ) );
	}
	 ...
	 */
	public static function awc_get_trial_period( $product ) {
		return apply_filters( 'shop_subscriptions_product_trial_period', self::awc_get_meta_data( $product, '_subscription_trial_period', '' ), self::awc_maybe_get_product_instance( $product ) );
	}
	...
	public static function get_first_renewal_payment_time( $product, $from_date = '', $timezone = 'gmt' ) {
		...
		$billing_interval = self::awc_get_interval( $product );
		$billing_length   = self::awc_get_length( $product );
		$trial_length     = self::awc_get_trial_length( $product );
		...
		if ( $billing_interval !== $billing_length || $trial_length > 0 ) {
			...
			if ( $trial_length > 0 ) {
				...
				$first_renewal_timestamp = awc_date_to_time( self::get_trial_expiration_date( $product, $from_date ) );

			} else {

				$site_time_offset = (int) ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS );

				// As awc_add_time() calls awc_add_months() which checks for last day of month, pass the site time
				$first_renewal_timestamp = awc_add_time( $billing_interval, self::awc_get_period( $product ), awc_date_to_time( $from_date ) + $site_time_offset );
	 ...
	 * @return mixed
	 */
	public static function awc_get_meta_data( $product, $meta_key, $default_value, $empty_handling = 'allow_empty' ) {
		...
		if ( self::awc_is_subscription( $product ) ) {
			...
			if ( is_callable( array( $product, 'meta_exists' ) ) ) { // WC 3.0
				...
				$prefixed_key = awc_maybe_prefix_key( $meta_key );
