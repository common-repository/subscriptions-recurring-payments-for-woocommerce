...
class AWC_Renewal_Cart_Stock_Manager {
	...
	public static function attach_callbacks() {
		add_action( 'awc_before_renewal_setup_cart_subscription', array( get_called_class(), 'maybe_adjust_stock_cart' ), 10, 2 );
	 ...
	 *
	 * Hooked onto 'awc_before_renewal_setup_cart_subscription'.
	 *
	...
	protected static function maybe_attach_stock_filters( $order ) {
		...
		foreach ( $order->get_items() as $line_item ) {
			...
			$required_stock = awc_get_total_line_item_product_quantity( $order, $stock_managed_product );
	...
	protected static function cart_contains_renewal_to_product( $product ) {
		...
		if ( $renewal_order && awc_order_contains_product( $renewal_order, $product ) ) {
