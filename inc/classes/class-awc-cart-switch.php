 ...
 */
class AWC_Cart_Switch extends AWC_Cart_Renewal {
	...
	public function maybe_setup_cart() {
		...
		if ( isset( $_GET['pay_for_order'] ) && isset( $_GET['key'] ) && isset( $wp->query_vars['order-pay'] ) && isset( $_GET['subscription_switch'] ) ) {
			...
			if ( awc_get_objects_property( $order, 'order_key' ) == $order_key && $order->has_status( array( 'pending', 'failed' ) ) && awc_order_contains_switch( $order ) ) {
				...
				foreach ( $order->get_items() as $item_id => $line_item ) {
					...
					$order_item = awc_get_order_item( $item_id, $order );
					$product    = wc_get_product( awc_get_canonical_product_id( $order_item ) );
