...
class AWC_Retry_Database_Store extends AWC_Retry_Store {
	 ...
	 * @var string
	 */
	const TABLE_NAME = 'awc_payment_retries';
	 ...
	 *
	 * @param AWC_Retry $retry the Retry we want to save.
	 *
	...
	public function get_retry( $retry_id ) {
		...
		$retry     = null;
		$raw_retry = $wpdb->get_row( $wpdb->prepare("SELECT * FROM {$this->get_full_table_name()} WHERE retry_id = %d LIMIT 1", $retry_id));
	...
	public function get_retries( $args = array(), $return = 'objects' ) {
		...
		foreach ( $raw_retries as $raw_retry ) {
			if ( 'ids' === $return ) {
				...
				$retries[ $raw_retry->retry_id ] = new AWC_Retry( array(
					'id'       => $raw_retry->retry_id,
					'order_id' => $raw_retry->order_id,
					'status'   => $raw_retry->status,
					'date_gmt' => $raw_retry->date_gmt,
