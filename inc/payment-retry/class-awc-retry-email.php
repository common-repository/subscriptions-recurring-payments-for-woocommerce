...
class AWC_Retry_Email {
	...
	public static function add_emails( $email_classes ) {
		$email_classes['AWC_Email_Customer_Payment_Retry'] = new AWC_Email_Customer_Payment_Retry();
		$email_classes['AWC_Email_Payment_Retry']          = new AWC_Email_Payment_Retry();
	 ...
	 *
	 * @param AWC_Retry_Rule The retry rule applied.
		...
		foreach ( array( 'customer', 'admin' ) as $recipient ) {
			if ( $retry_rule->has_email_template( $recipient ) ) {
				...
				if ( class_exists( $email_class ) ) {
					...
					$email->trigger( awc_get_objects_property( $last_order, 'id' ), $last_order );
				}
			}
		}
	}
	...
	public static function maybe_detach_email( $order_id ) {
		...
		if ( awc_order_contains_renewal( $order_id ) && AWC_Payment_Retry_Manager::rules()->has_rule( AWC_Payment_Retry_Manager::store()->get_retry_count_for_order( $order_id ), $order_id ) ) {
			...
			remove_action( 'woocommerce_order_status_failed', 'AWC_Subscriptions_Email::send_renewal_order_email', 10 );

			// Remove email sent to admin, which is sent by WooCommerce
			AWC_Subscriptions_Email::detach_woocommerce_transactional_email( 'woocommerce_order_status_pending_to_failed' );
			AWC_Subscriptions_Email::detach_woocommerce_transactional_email( 'woocommerce_order_status_on-hold_to_failed' );
	...
	public static function maybe_reattach_email( $order_id, $old_status, $new_status ) {
		...
		if ( 'failed' === $new_status && $order_id == self::$removed_emails_for_order_id ) {
			...
			add_action( 'woocommerce_order_status_failed', 'AWC_Subscriptions_Email::send_renewal_order_email' );

			// Reattach email sent to admin, which is sent by WooCommerce
			AWC_Subscriptions_Email::attach_woocommerce_transactional_email( 'woocommerce_order_status_pending_to_failed' );
			AWC_Subscriptions_Email::attach_woocommerce_transactional_email( 'woocommerce_order_status_on-hold_to_failed' );
