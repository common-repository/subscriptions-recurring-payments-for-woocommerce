...
if(!class_exists('AWC_Privacy')){
	class AWC_Privacy extends WC_Abstract_Privacy {

		/**
		 * Background updater to process personal data removal from subscriptions and related orders.
		 *
		 * @var 	AWC_Privacy_Background_Updater
		 * @access	protected
		 */
		...
		/**
		 * AWC_Privacy constructor (default callback function).
		 * @access	public
		 */
		public function __construct() {
			if ( ! self::$background_process ) {
				self::$background_process = new AWC_Privacy_Background_Updater();
			}

			parent::__construct( __( 'WooCommerce Subscriptions', 'subscriptions-recurring-payments-for-woocommerce' ) );

			// Add our exporters and erasers.
			$this->add_exporter( 'aco-woo-subscriptions-data', __( 'Subscriptions Data', 'subscriptions-recurring-payments-for-woocommerce' ), array( 'AWC_Privacy_Exporters', 'subscription_data_exporter' ) );
			$this->add_eraser( 'aco-woo-subscriptions-data', __( 'Subscriptions Data', 'subscriptions-recurring-payments-for-woocommerce' ), array( 'AWC_Privacy_Erasers', 'subscription_data_eraser' ) );
		}
		...
		public function init() {
			...
			add_action( 'woocommerce_remove_subscription_personal_data', array( 'awc_Privacy_Erasers', 'remove_subscription_personal_data' ) );
		...
		*/
		public function get_privacy_message() {
			return '' .
			'<p>' . __( 'By using WooCommerce Subscriptions, you may be storing personal data and depending on which third-party payment processors you’re using to take subscription payments, you may be sharing personal data with external sources.', 'subscriptions-recurring-payments-for-woocommerce' ) . '</p>' .
			// translators: placeholders are opening and closing link tags, linking to additional privacy policy documentation.
			'<h3>' . __( 'What we collect and store', 'subscriptions-recurring-payments-for-woocommerce' ) . '</h3>' .
			'<p>' . __( 'For the purposes of processing recurring subscription payments, we store the customer\'s name, billing address, shipping address, email address, phone number and credit card/payment details.', 'subscriptions-recurring-payments-for-woocommerce' ) . '</p>' .
			'<h3>' . __( 'What we share with others', 'subscriptions-recurring-payments-for-woocommerce' ) . '</h3>' .
			'<p>' . __( 'What personal information your store shares with external sources depends on which third-party payment processor plugins you are using to collect subscription payments. We recommend that you consult with their privacy policies to inform this section of your privacy policy.', 'subscriptions-recurring-payments-for-woocommerce' ) . '</p>' .
			// translators: placeholders are opening and closing link tags, linking to additional privacy policy documentation.
			'<p>' . sprintf( __( 'If you are using PayPal Standard or PayPal Reference transactions please see the %1$sPayPal Privacy Policy%2$s for more details.', 'subscriptions-recurring-payments-for-woocommerce' ), '<a href="https://www.paypal.com/us/webapps/mpp/ua/privacy-full">', '</a>' ) . '</p>';
		}
		...
		*/
		public static function add_remove_personal_data_bulk_action( $bulk_actions ) {
			$bulk_actions['remove_personal_data'] = __( 'Cancel and remove personal data', 'subscriptions-recurring-payments-for-woocommerce' );
			return $bulk_actions;
		}
		...
		*/
		public static function process_bulk_action() {
			...
			foreach ( $subscription_ids as $subscription_id ) {
				...
				if ( is_a( $subscription, 'AWC_Subscription' ) ) {
		...
		*/
		public static function bulk_admin_notices() {
			...
			$message = sprintf( _n( 'Removed personal data from %d subscription.', 'Removed personal data from %d subscriptions.', $changed, 'subscriptions-recurring-payments-for-woocommerce' ), number_format_i18n( $changed ) );
		...
		public static function add_caveat_to_order_data_retention_settings( $settings ) {
			...
			foreach ( $settings as &$setting ) {
				if ( isset( $setting['id'], $setting['type'] ) && 'personal_data_retention' === $setting['id'] && 'title' === $setting['type'] ) {
					...
					$note            = sprintf( __( '%1$sNote:%2$s Orders which are related to subscriptions will not be included in the orders affected by these settings.', 'subscriptions-recurring-payments-for-woocommerce' ), '<b>', '</b>' );
		...
		*/
		public static function add_subscription_data_retention_settings( $settings ) {
			...
			$erasure_text = esc_html__( 'account erasure request', 'subscriptions-recurring-payments-for-woocommerce' );
			if ( current_user_can( 'manage_privacy_options' ) ) {
				$erasure_text = sprintf( '<a href="%s">%s</a>', esc_url( admin_url( 'tools.php?page=remove_personal_data' ) ), $erasure_text );
			}

			AWC_Subscriptions_Admin::insert_setting_after( $settings, 'woocommerce_erasure_request_removes_order_data', array(
				'desc'          => __( 'Remove personal data from subscriptions', 'subscriptions-recurring-payments-for-woocommerce' ),
				/* Translators: %s URL to erasure request screen. */
				'desc_tip'      => sprintf( __( 'When handling an %s, should personal data within subscriptions be retained or removed?', 'subscriptions-recurring-payments-for-woocommerce' ), $erasure_text ),
			...
			AWC_Subscriptions_Admin::insert_setting_after( $settings, 'woocommerce_anonymize_completed_orders', array(
				'title'       => __( 'Retain ended subscriptions', 'subscriptions-recurring-payments-for-woocommerce' ),
				'desc_tip'    => __( 'Retain ended subscriptions and their related orders for a specified duration before anonymizing the personal data within them.', 'subscriptions-recurring-payments-for-woocommerce' ),
				'id'          => 'woocommerce_anonymize_ended_subscriptions',
				'type'        => 'relative_date_selector',
				'placeholder' => __( 'N/A', 'subscriptions-recurring-payments-for-woocommerce' ),
				'default'     => array(
					'number' => '',
					'unit'   => 'months',
				),
		...
		public static function add_inactive_user_retention_note( $settings ) {
			foreach ( $settings as &$setting ) {
				if ( isset( $setting['id'], $setting['desc_tip'] ) && 'woocommerce_delete_inactive_accounts' === $setting['id'] ) {
					$setting['desc_tip'] .= ' ' . __( 'Customers with a subscription are excluded from this setting.', 'subscriptions-recurring-payments-for-woocommerce' );
					break;
				}
			}
