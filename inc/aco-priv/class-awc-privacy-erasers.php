...
class AWC_Privacy_Erasers {
		...
		if ( 0 < count( $subscriptions ) ) {
			foreach ( $subscriptions as $subscription ) {
				if ( apply_filters( 'woocommerce_privacy_erase_subscription_personal_data', $erasure_enabled, $subscription ) ) {
					...
					$response['messages'][]    = sprintf( __( 'Removed personal data from subscription %s.', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription->get_order_number() );
					$response['items_removed'] = true;
				} else {
					/* Translators: %s subscription number. */
					$response['messages'][]     = sprintf( __( 'Personal data within subscription %s has been retained.', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription->get_order_number() );
	...
	public static function remove_subscription_personal_data( $subscription ) {
		...
		$subscription->add_order_note( __( 'Personal data removed.', 'subscriptions-recurring-payments-for-woocommerce' ) );
