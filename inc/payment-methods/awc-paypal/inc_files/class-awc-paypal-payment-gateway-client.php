...
// class-wc-gateway-ppec-client.php
// class-awc-paypal-payment-gateway-client.php

class AWC_Paypal_Payment_Gateway_Client {

	/**
	 * Client credential.
	 *
	 * @var AWC_Paypal_Payment_Gateway_Client_Credential
	 */
	...
	public function __construct( $credential, $environment = 'live' ) {
		...
		if ( is_a( $credential, 'AWC_Paypal_Payment_Gateway_Client_Credential' ) ) {
	 ...
	 *
	 * @param AWC_Paypal_Payment_Gateway_Client_Credential $credential Client's credential
	 */
	public function set_credential( AWC_Paypal_Payment_Gateway_Client_Credential $credential ) {
	...
	protected function _request( array $params ) {
		try {
			...
			$body = apply_filters( 'awc_paypal_payment_express_checkout_request_body', array_merge( $params, $this->_credential->get_request_params() ) );
			$args = array(
				'method'      => 'POST',
				'body'        => $body,
			...
			awc_paypal_payment_gateway_log( sprintf( '%s: remote request to %s with params: %s', __METHOD__, $this->get_endpoint(), print_r( $body, true ) ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
			...
			awc_paypal_payment_gateway_log( sprintf( '%s: returns error: %s', __METHOD__, print_r( $error, true ) ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
	...
	protected function _validate_request() {
		...
		if ( ! $this->_credential ) {
			throw new Exception( __( 'Missing credential', 'subscriptions-recurring-payments-for-woocommerce' ), self::INVALID_CREDENTIAL_ERROR );
		}

		if ( ! is_a( $this->_credential, 'AWC_Paypal_Payment_Gateway_Client_Credential' ) ) {
			throw new Exception( __( 'Invalid credential object', 'subscriptions-recurring-payments-for-woocommerce' ), self::INVALID_CREDENTIAL_ERROR );
		}

		if ( ! in_array( $this->_environment, array( 'live', 'sandbox' ), true ) ) {
			throw new Exception( __( 'Invalid environment', 'subscriptions-recurring-payments-for-woocommerce' ), self::INVALID_ENVIRONMENT_ERROR );
		}
	}
	...
	protected function _process_response( $response ) {
		if ( is_wp_error( $response ) ) {
			...
			throw new Exception( sprintf( __( 'An error occurred while trying to connect to PayPal: %s', 'subscriptions-recurring-payments-for-woocommerce' ), $response->get_error_message() ), self::REQUEST_ERROR );
		}

		parse_str( wp_remote_retrieve_body( $response ), $result );

		if ( ! array_key_exists( 'ACK', $result ) ) {
			throw new Exception( __( 'Malformed response received from PayPal', 'subscriptions-recurring-payments-for-woocommerce' ), self::REQUEST_ERROR );
		}

		awc_paypal_payment_gateway_log( sprintf( '%s: acknowleged response body: %s', __METHOD__, print_r( $result, true ) ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
	...
	public function get_set_express_checkout_params( array $args ) {
		...
		$params = array_merge(
			...
			array(
				...
				'NOSHIPPING'                   => AWC_Paypal_Payment_Methods_for_Subscription::needs_shipping() ? 0 : 1,
			)
		);
	...
	protected function _get_billing_agreement_description() {
		...
		$description = sprintf( _x( 'Orders with %s', 'data sent to PayPal', 'subscriptions-recurring-payments-for-woocommerce' ), get_bloginfo( 'name' ) );
	...
	protected function _get_details_from_order( $order_id ) {
		...
		$shipping_address = new AWC_Address_For_Paypal();
		...
		//
		// @see https://github.com/woothemes/aco-woo-subscriptions/issues/139
	...
	public function get_do_express_checkout_params( array $args ) {
		...
		$params = array(
			...
			'PAYMENTREQUEST_0_NOTIFYURL'     => WC()->api_request_url( 'AWC_Paypal_Gateway' ),
		...
		if ( AWC_Paypal_Payment_Methods_for_Subscription::needs_shipping() && ! empty( $details['shipping_address'] ) ) {
			$params = array_merge(
				$params,
				$details['shipping_address']->getAddressParams( 'PAYMENTREQUEST_0_SHIPTO' )
			);
	...
	public function get_do_reference_transaction_params( array $args ) {
		...
		$params = array(
			...
			'NOTIFYURL'     => WC()->api_request_url( 'AWC_Paypal_Gateway' ),
	...
	public function test_api_credentials( $credentials, $environment = 'sandbox' ) {
		...
		if ( 'Success' !== $result['ACK'] && 'SuccessWithWarning' !== $result['ACK'] ) {
			...
			if ( $found_10002 ) {
				...
				throw new AWC_Paypal_Payment_Gateway_Exception( $result );
			}
		}
