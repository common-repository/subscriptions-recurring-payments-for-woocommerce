...
class AWC_Paypal_Payment_Gateway_Admin_Handler {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_order_action_ppec_capture_charge', array( $this, 'awc_capture_charge_maybe' ) );
	...
	public function add_capture_charge_order_action( $actions ) {
		...
		if ( 'awc_paypal_payment' !== $payment_method || 'pending' !== $paypal_status ) {
			return $actions;
		}
		...
		$actions['ppec_capture_charge'] = esc_html__( 'Capture Charge', 'subscriptions-recurring-payments-for-woocommerce' );
	...
	public function show_decimal_warning() {
		if ( get_option( 'wc_gateway_ppce_display_decimal_msg', false ) ) {
			...
			<div class="updated fade">
				<p>
					<strong><?php esc_html_e( 'NOTE: PayPal does not accept decimal places for the currency in which you are transacting.  The "Number of Decimals" option in WooCommerce has automatically been set to 0 for you.', 'subscriptions-recurring-payments-for-woocommerce' ); ?></strong>
				</p>
			</div>
			<?php
	...
	public function filter_checkout_sections( $sections ) {
		...
		$card_sections = array(
			'awc_paypal_payment_gateway_with_credit'
		);
		...
		// If the current section is a paypal section, remove the card section, otherwise, remove the paypal section
		$sections_to_remove = in_array( $current_section, $paypal_sections, true ) ? $card_sections : $paypal_sections;

		// And, let's also remove simplify commerce from the sections if it is not enabled and it is not the current section. (Note: The option will be empty if it has never been enabled)
	...
	public function awc_capture_charge_maybe( $order ) {
		if ( ! is_object( $order ) ) {
	...
	public function capture_payment( $order_id ) {
		...
		if ( 'awc_paypal_payment' === $payment_method && $transaction_id ) {

			$trans_details = awc_paypal_ec_gateway()->client->get_transaction_details( array( 'TRANSACTIONID' => $transaction_id ) );
			...
			if ( $this->is_authorized_only( $trans_details ) ) {
				...
				$result = awc_paypal_ec_gateway()->client->do_express_checkout_capture( $params );

				if ( is_wp_error( $result ) ) {
					$order->add_order_note( __( 'Unable to capture charge!', 'subscriptions-recurring-payments-for-woocommerce' ) . ' ' . $result->get_error_message() );
				} else {
					...
					$order->add_order_note( sprintf( __( 'PayPal Checkout charge complete (Charge ID: %s)', 'subscriptions-recurring-payments-for-woocommerce' ), $transaction_id ) );
				}
			}
		}
	}
	...
	public function cancel_authorization( $order_id ) {
		...
		if ( 'awc_paypal_payment' === $payment_method ) {

			$trans_id      = get_post_meta( $order_id, '_transaction_id', true );
			$trans_details = awc_paypal_ec_gateway()->client->get_transaction_details( array( 'TRANSACTIONID' => $trans_id ) );

			if ( $trans_id && $this->is_authorized_only( $trans_details ) ) {
				$params['AUTHORIZATIONID'] = $trans_id;

				$result = awc_paypal_ec_gateway()->client->do_express_checkout_void( $params );

				if ( is_wp_error( $result ) ) {
					$order->add_order_note( __( 'Unable to void charge!', 'subscriptions-recurring-payments-for-woocommerce' ) . ' ' . $result->get_error_message() );
				} else {
					// Translators: %s is a transaction ID.
					$order->add_order_note( sprintf( __( 'PayPal Checkout charge voided (Charge ID: %s)', 'subscriptions-recurring-payments-for-woocommerce' ), $trans_id ) );
				}
			}
		}
	}
	...
	public function gateway_admin_url( $gateway_class ) {
		_deprecated_function( 'AWC_Paypal_Payment_Gateway_Admin_Handler::gateway_admin_url', '1.0.4', 'awc_paypal_ec_gateway()->get_admin_setting_link' );

		return awc_paypal_ec_gateway()->get_admin_setting_link();
	}
	...
	public function maybe_redirect_to_ppec_settings() {
		if ( ! awc_paypal_ec_gateway()->settings->enabled ) {
			return;
		}
	...
	public function maybe_reset_api_credentials() {
		...
		wp_safe_redirect( awc_paypal_ec_gateway()->get_admin_setting_link() );
	}
	...
	public function display_order_fee_and_payout( $order_id ) {
		...
		if ( 'awc_paypal_payment' !== $payment_method || ! is_numeric( $paypal_fee ) ) {
			return;
		}
		...
		<tr>
			<td class="label ppec-fee">
				<?php echo wc_help_tip( __( 'This represents the fee PayPal collects for the transaction.', 'subscriptions-recurring-payments-for-woocommerce' ) ); ?>
				<?php esc_html_e( 'PayPal Fee:', 'subscriptions-recurring-payments-for-woocommerce' ); ?>
			</td>
			<td width="1%"></td>
			<td class="total">
		...
		<tr>
			<td class="label ppec-payout">
				<?php echo wc_help_tip( __( 'This represents the net total that will be credited to your PayPal account. This may be in a different currency than is set in your PayPal account.', 'subscriptions-recurring-payments-for-woocommerce' ) ); ?>
				<?php esc_html_e( 'PayPal Payout:', 'subscriptions-recurring-payments-for-woocommerce' ); ?>
			</td>
			<td width="1%"></td>
			<td class="total">
