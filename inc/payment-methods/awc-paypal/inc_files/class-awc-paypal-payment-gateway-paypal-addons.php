...
class AWC_Paypal_Payment_Gateway_Paypal_Addons extends AWC_Paypal_Payment_Gateway {
	...
	protected function _maybe_register_callback_in_subscriptions() {
		...
		add_action( 'awc_subscription_scheduled_payment_' . $this->id, array( $this, 'scheduled_subscription_payment' ), 10, 2 );
	...
	public function is_subscription( $order_id ) {
		return ( function_exists( 'awc_order_contains_subscription' ) && ( awc_order_contains_subscription( $order_id ) || awc_is_subscription( $order_id ) || awc_order_contains_renewal( $order_id ) ) );
	}
	...
	private function is_order_changing_payment_method_for_subscription( $order_id ) {
		...
		return (
			...
			&& isset( $_POST['_asubnonce'] )
			&& wp_verify_nonce( sanitize_key( $_POST['_asubnonce'] ), 'awc_change_payment_method' )
	...
	public function process_subscription( $order_id ) {
		...
		foreach ( $subscriptions as $subscription ) {
			awc_copy_order_address( $order, $subscription );
	...
	public function scheduled_subscription_payment( $amount, $order ) {
		...
		if ( empty( $billing_agreement_id ) ) {
			awc_paypal_payment_gateway_log( sprintf( '%s: Could not found billing agreement. Skip reference transaction', __METHOD__ ) );
			return;
		}
	...
	protected function _process_reference_transaction_response( $order, $response ) {
		...
		try {
			if ( ! $client->response_has_success_status( $response ) ) {
				throw new Exception( __( 'PayPal API error', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}

			awc_paypal_gateway_save_transaction_data( $order, $response );
			...
			switch ( $status ) {
				case 'Pending':
					...
					$order_note = sprintf( __( 'PayPal transaction held: %s', 'subscriptions-recurring-payments-for-woocommerce' ), $response['PENDINGREASON'] );
				...
				case 'In-Progress':
					...
					$order->add_order_note( sprintf( __( 'PayPal payment approved (ID: %s)', 'subscriptions-recurring-payments-for-woocommerce' ), $transaction_id ) );
					$order->payment_complete( $transaction_id );
					break;
				default:
					throw new Exception( __( 'PayPal payment declined', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}
		} catch ( Exception $e ) {
	 ...
	 *
	 * AWC_Subscription_Change_Payment_Gateway::change_payment_method_via_pay_shortcode uses the
	...
	public function indicate_async_payment_method_update( $should_update, $new_payment_method ) {
		if ( 'awc_paypal_payment' === $new_payment_method ) {
			$should_update = false;
		}
	 ...
	 * When it's successful, `AWC_Subscription_Change_Payment_Gateway::change_payment_method_via_pay_shortcode` will
	 ...
	 * @param int $order_id Order ID.
	 *
	 * @return array Array used by AWC_Subscription_Change_Payment_Gateway::change_payment_method_via_pay_shortcode.
	 */
	public function change_subscription_payment_method( $order_id ) {
		try {
			return array(
				'result'   => 'success',
				'redirect' => awc_paypal_ec_gateway()->checkout->start_checkout_from_order( $order_id, false ),
			);
