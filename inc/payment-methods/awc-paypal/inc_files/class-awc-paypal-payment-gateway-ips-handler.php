...
class AWC_Paypal_Payment_Gateway_IPS_Handler {
	...
	public function get_redirect_url( $env ) {
		...
		return add_query_arg(
			array(
				'env'                     => $env,
				'awc_pg_wc_admin_nonce' => wp_create_nonce( 'awc_pg_wc' ),
			),
	...
	public function maybe_received_credentials() {
		...
		// Require the nonce.
		if ( empty( $_GET['awc_pg_wc_admin_nonce'] ) || empty( $_GET['env'] ) ) {
			return false;
		}
		$env = in_array( $_GET['env'], array( 'live', 'sandbox' ), true ) ? $_GET['env'] : 'live'; 

		// Verify the nonce.
		if ( ! wp_verify_nonce( $_GET['awc_pg_wc_admin_nonce'], 'awc_pg_wc' ) ) { 
			wp_die( __( 'Invalid connection request', 'subscriptions-recurring-payments-for-woocommerce' ) );
		}

		awc_paypal_payment_gateway_log( sprintf( '%s: returned back from IPS flow with parameters: %s', __METHOD__, print_r( $_GET, true ) ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r

		// Check if error.
		if ( ! empty( $_GET['error'] ) ) {
			$error_message = ! empty( $_GET['error_message'] ) ? $_GET['error_message'] : ''; 
			awc_paypal_payment_gateway_log( sprintf( '%s: returned back from IPS flow with error: %s', __METHOD__, $error_message ) );

			$this->_redirect_with_messages( __( 'Sorry, Easy Setup encountered an error.  Please try again.', 'subscriptions-recurring-payments-for-woocommerce' ) );
		}

		// Make sure credentials present in query string.
		foreach ( array( 'api_style', 'api_username', 'api_password', 'signature' ) as $param ) {
			if ( empty( $_GET[ $param ] ) ) {
				awc_paypal_payment_gateway_log( sprintf( '%s: returned back from IPS flow but missing parameter %s', __METHOD__, $param ) );

				$this->_redirect_with_messages( __( 'Sorry, Easy Setup encountered an error.  Please try again.', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}
		}

		$creds = new AWC_Paypal_Payment_Gateway_Credential_Signature( $_GET['api_username'], $_GET['api_password'], $_GET['signature'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash

		$error_msgs = array();
		try {
			$payer_id = awc_paypal_ec_gateway()->client->test_api_credentials( $creds, $env );

			if ( ! $payer_id ) {
				$this->_redirect_with_messages( __( 'Easy Setup was able to obtain your API credentials, but was unable to verify that they work correctly.  Please make sure your PayPal account is set up properly and try Easy Setup again.', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}
		} catch ( AWC_Paypal_Payment_Gateway_Exception $ex ) {
			$error_msgs[] = array(
				'warning' => __( 'Easy Setup was able to obtain your API credentials, but an error occurred while trying to verify that they work correctly.  Please try Easy Setup again.', 'subscriptions-recurring-payments-for-woocommerce' ),
			);
		}

		$error_msgs[] = array(
			'success' => __( 'Success!  Your PayPal account has been set up successfully.', 'subscriptions-recurring-payments-for-woocommerce' ),
		);

		if ( ! empty( $error_msgs ) ) {
			awc_paypal_payment_gateway_log( sprintf( '%s: returned back from IPS flow: %s', __METHOD__, print_r( $error_msgs, true ) ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
		}
