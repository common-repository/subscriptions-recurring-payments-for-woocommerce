...
awc_include_files('class-awc-address-for-paypal');
...
class PayPal_Checkout_Details {
	...
	public function loadFromGetECResponse( $getECResponse ) {
		...
		if ( ! $this->payer_details->loadFromGetECResponse( $getECResponse ) ) {
			awc_paypal_payment_gateway_log( sprintf( 'PayPal response did not include the payer billing details: %s', print_r( $getECResponse, true ) ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
...
class PayPal_Checkout_Payment_Details {
	...
	public function loadFromGetECResponse( $getECResponse, $bucketNum ) {
		...
		$this->shipping_address = new AWC_Address_For_Paypal();
...
class PayPal_Checkout_Payer_Details {
	...
	public function loadFromGetECResponse( $getECResponse ) {
		...
		if ( $billing_address_present ) {
			$this->billing_address = new AWC_Address_For_Paypal();
