...
class AWC_Paypal_Payment_Gateway_Settings {
	 ...
	 *
	 * @return AWC_Paypal_Payment_Gateway_Settings Instance of AWC_Paypal_Payment_Gateway_Settings
	 */
	public function load( $force_reload = false ) {
		if ( $this->_is_setting_loaded && ! $force_reload ) {
			return $this;
		}
		$this->_settings            = (array) get_option( 'woocommerce_awc_paypal_payment_settings', array() );
	...
	public function load_settings( $force_reload = false ) {
		_deprecated_function( __METHOD__, '1.2.0', 'AWC_Paypal_Payment_Gateway_Settings::load' );
	...
	public function save() {
		update_option( 'woocommerce_awc_paypal_payment_settings', $this->_settings );
	}

	/**
	 * Get API credentials for live envionment.
	 *
	 * @return AWC_Paypal_Payment_Gateway_Credential_Signature|AWC_Paypal_Payment_Gateway_Client_Credential_Certificate
	 */
	public function get_live_api_credentials() {
		if ( $this->api_certificate ) {
			return new AWC_Paypal_Payment_Gateway_Client_Credential_Certificate( $this->api_username, $this->api_password, base64_decode( $this->api_certificate ), $this->api_subject ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode
		}

		return new AWC_Paypal_Payment_Gateway_Credential_Signature( $this->api_username, $this->api_password, $this->api_signature, $this->api_subject );
	}
	...
	public function get_sandbox_api_credentials() {
		if ( $this->sandbox_api_certificate ) {
			return new AWC_Paypal_Payment_Gateway_Client_Credential_Certificate( $this->sandbox_api_username, $this->sandbox_api_password, base64_decode( $this->sandbox_api_certificate ), $this->sandbox_api_subject ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode
		}

		return new AWC_Paypal_Payment_Gateway_Credential_Signature( $this->sandbox_api_username, $this->sandbox_api_password, $this->sandbox_api_signature, $this->sandbox_api_subject );
	}
	...
	public function get_set_express_checkout_shortcut_params( $buckets = 1 ) {
		_deprecated_function( __METHOD__, '1.2.0', 'AWC_Paypal_Payment_Gateway_Client::get_set_express_checkout_params' );

		return awc_paypal_ec_gateway()->client->get_set_express_checkout_params( array( 'skip_checkout' => true ) );
	}
	...
	public function get_set_express_checkout_mark_params( $buckets = 1 ) {
		...
		return awc_paypal_ec_gateway()->client->get_set_express_checkout_params(
			array(
				'skip_checkout' => false,
			)
		);
	...
	public function get_paypal_locale() {
		...
		if ( awc_paypal_payment_gateway_is_US_based_store() ) {
