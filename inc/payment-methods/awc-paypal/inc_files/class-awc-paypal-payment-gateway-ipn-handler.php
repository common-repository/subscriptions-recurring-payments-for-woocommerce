...
class WC_Gateway_PPEC_IPN_Handler extends WC_Gateway_PPEC_PayPal_Request_Handler {
	...
	public function check_request() {
		try {
			if ( empty( $_POST ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
				throw new Exception( esc_html__( 'Empty POST data.', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}

			if ( $this->is_valid_ipn_request( $_POST ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
				awc_paypal_payment_gateway_log( 'IPN request is valid according to PayPal.' );
				...
				awc_paypal_payment_gateway_log( 'IPN request is NOT valid according to PayPal.' );
				throw new Exception( esc_html__( 'Invalid IPN request.', 'subscriptions-recurring-payments-for-woocommerce' ) );
			}
		} catch ( Exception $e ) {
			wp_die( $e->getMessage(), esc_html__( 'PayPal IPN Request Failure', 'subscriptions-recurring-payments-for-woocommerce' ), array( 'response' => 500 ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
	...
	public function is_valid_ipn_request( array $posted_data ) {
		awc_paypal_payment_gateway_log( sprintf( '%s: %s', __FUNCTION__, 'Checking IPN request validity' ) );
		...
		awc_paypal_payment_gateway_log( sprintf( '%s: %s: %s', __FUNCTION__, 'Verify IPN request', print_r( $params, true ) ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
		awc_paypal_payment_gateway_log( sprintf( '%s: %s: %s', __FUNCTION__, 'Response for the IPN request', print_r( $response, true ) ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
	...
	public function handle_valid_ipn( $posted_data ) {
		if ( ! empty( $posted_data['custom'] ) && ( $order = $this->get_paypal_order( $posted_data['custom'] ) ) ) { // phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.FoundInControlStructure,WordPress.CodeAnalysis.AssignmentInCondition.Found
			...
			awc_paypal_payment_gateway_log( 'Found order #' . $order_id );
			awc_paypal_payment_gateway_log( 'Payment status: ' . $posted_data['payment_status'] );
			...
			awc_paypal_payment_gateway_log( sprintf( '%s: %s', __FUNCTION__, 'No order data being passed' ) );
		}
	}
	...
	protected function validate_transaction_type( $txn_type ) {
		...
		if ( ! in_array( strtolower( $txn_type ), $accepted_types, true ) ) {
			awc_paypal_payment_gateway_log( 'Aborting, Invalid type:' . $txn_type );
			exit;
		}
	}
	...
	protected function validate_currency( $order, $currency ) {
		...
		if ( $order_currency !== $currency ) {
			awc_paypal_payment_gateway_log( 'Payment error: Currencies do not match (sent "' . $order_currency . '" | returned "' . $currency . '")' );
			// Put this order on-hold for manual checking.
			// Translators: placeholder is a currency code.
			$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayPal currencies do not match (code %s).', 'subscriptions-recurring-payments-for-woocommerce' ), $currency ) );
			exit;
		}
	}
	...
	protected function validate_amount( $order, $amount ) {
		if ( number_format( $order->get_total(), 2, '.', '' ) != number_format( $amount, 2, '.', '' ) ) { // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
			awc_paypal_payment_gateway_log( 'Payment error: Amounts do not match (gross ' . $amount . ')' );
			// Put this order on-hold for manual checking.
			// Translators: placeholder is an amount.
			$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayPal amounts do not match (gross %s).', 'subscriptions-recurring-payments-for-woocommerce' ), $amount ) );
			exit;
		}
	}
	...
	protected function payment_status_completed( $order, $posted_data ) {
		...
		if ( $order->has_status( array( 'completed' ) ) ) {
			awc_paypal_payment_gateway_log( 'Aborting, Order #' . $order_id . ' is already complete.' );
			exit;
		}
		...
		if ( 'completed' === $posted_data['payment_status'] ) {
			$this->payment_complete( $order, ( ! empty( $posted_data['txn_id'] ) ? wc_clean( $posted_data['txn_id'] ) : '' ), __( 'IPN payment completed', 'subscriptions-recurring-payments-for-woocommerce' ) );
			if ( ! empty( $posted_data['mc_fee'] ) ) {
				// Log paypal transaction fee.
				awc_paypal_payment_gateway_set_transaction_fee( $order, $posted_data['mc_fee'] );
			}
		} else {
			if ( 'authorization' === $posted_data['pending_reason'] ) {
				$this->payment_on_hold( $order, __( 'Payment authorized. Change payment status to processing or complete to capture funds.', 'subscriptions-recurring-payments-for-woocommerce' ) );
			} else {
				// Translators: placeholder is the reason for the payment to be in pending status.
				$this->payment_on_hold( $order, sprintf( __( 'Payment pending (%s).', 'subscriptions-recurring-payments-for-woocommerce' ), $posted_data['pending_reason'] ) );
			}
		}
	}
	...
	protected function payment_status_failed( $order, $posted_data ) {
		...
		$order->update_status( 'failed', sprintf( __( 'Payment %s via IPN.', 'subscriptions-recurring-payments-for-woocommerce' ), wc_clean( $posted_data['payment_status'] ) ) );
	}
	...
	protected function payment_status_refunded( $order, $posted_data ) {
		...
		if ( $order->get_total() == ( $posted_data['mc_gross'] * -1 ) ) { // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
			...
			// Translators: placeholder is a payment status.
			$order->update_status( 'refunded', sprintf( __( 'Payment %s via IPN.', 'subscriptions-recurring-payments-for-woocommerce' ), strtolower( $posted_data['payment_status'] ) ) );
			$this->send_ipn_email_notification(
				/* Translators: placeholder is an order number (linked to its details screen). */
				sprintf( __( 'Payment for order %s refunded', 'subscriptions-recurring-payments-for-woocommerce' ), '<a class="link" href="' . esc_url( admin_url( 'post.php?post=' . $order_id . '&action=edit' ) ) . '">' . $order->get_order_number() . '</a>' ),
				/* Translators: 1) is an order number, 2) is a PayPal reason code. */
				sprintf( __( 'Order #%1$s has been marked as refunded - PayPal reason code: %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), $order->get_order_number(), $posted_data['reason_code'] )
			);
		}
	}
	...
	protected function payment_status_reversed( $order, $posted_data ) {
		$order_id = version_compare( WC_VERSION, '3.0', '<' ) ? $order->id : $order->get_id();
		// Translators: placeholder is a payment status.
		$order->update_status( 'on-hold', sprintf( __( 'Payment %s via IPN.', 'subscriptions-recurring-payments-for-woocommerce' ), wc_clean( $posted_data['payment_status'] ) ) );
		$this->send_ipn_email_notification(
			/* Translators: placeholder is an order number (linked to its details screen). */
			sprintf( __( 'Payment for order %s reversed', 'subscriptions-recurring-payments-for-woocommerce' ), '<a class="link" href="' . esc_url( admin_url( 'post.php?post=' . $order_id . '&action=edit' ) ) . '">' . $order->get_order_number() . '</a>' ),
			/* Translators: 1) is an order number, 2) is a PayPal reason code. */
			sprintf( __( 'Order #%1$s has been marked on-hold due to a reversal - PayPal reason code: %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), $order->get_order_number(), wc_clean( $posted_data['reason_code'] ) )
		);
	}
	...
	protected function payment_status_canceled_reversal( $order, $posted_data ) {
		...
		$this->send_ipn_email_notification(
			...
			sprintf( __( 'Reversal cancelled for order #%s', 'subscriptions-recurring-payments-for-woocommerce' ), $order->get_order_number() ),
			/* Translators: 1) is an order number, 2) is the URL of the order's edit screen. */
			sprintf( __( 'Order #%1$s has had a reversal cancelled. Please check the status of payment and update the order status accordingly here: %2$s', 'subscriptions-recurring-payments-for-woocommerce' ), $order->get_order_number(), esc_url( admin_url( 'post.php?post=' . $order_id . '&action=edit' ) ) )
		);
	}
