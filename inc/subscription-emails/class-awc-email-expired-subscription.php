 ...
 *
 */
class AWC_Email_Expired_Subscription extends WC_Email {
	...
	function __construct() {
		...
		$this->title       = __( 'Expired Subscription', 'subscriptions-recurring-payments-for-woocommerce' );
		$this->description = __( 'Expired Subscription emails are sent when a customer\'s subscription expires.', 'subscriptions-recurring-payments-for-woocommerce' );

		$this->heading     = __( 'Subscription Expired', 'subscriptions-recurring-payments-for-woocommerce' );
		// translators: placeholder is {blogname}, a variable that will be substituted when email is sent out
		$this->subject     = sprintf( _x( '[%s] Subscription Expired', 'default email subject for expired emails sent to the admin', 'subscriptions-recurring-payments-for-woocommerce' ), '{blogname}' );

		$this->template_html  = 'emails/expired-subscription.php';
		$this->template_plain = 'emails/text/expired-subscription.php';
	...
	function trigger( $subscription ) {
		...
		if ( ! is_object( $subscription ) ) {
			throw new InvalidArgumentException( __( 'Subscription argument passed in is not an object.', 'subscriptions-recurring-payments-for-woocommerce' ) );
		}
	...
	function init_form_fields() {
		$this->form_fields = array(
			...
			'recipient'  => array(
				...
				'description' => sprintf( __( 'Enter recipients (comma separated) for this email. Defaults to %s.', 'subscriptions-recurring-payments-for-woocommerce' ), '<code>' . esc_html( get_option( 'admin_email' ) ) . '</code>' ),
				'placeholder' => '',
				'default'     => '',
			),
			'subject'    => array(
			...
			'heading'    => array(
				...
				'description' => sprintf( __( 'This controls the main heading contained within the email notification. Leave blank to use the default heading: <code>%s</code>.', 'subscriptions-recurring-payments-for-woocommerce' ), $this->heading ),
