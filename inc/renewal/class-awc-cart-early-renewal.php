...
if(!class_exists('AWC_Cart_Early_Renewal')){
	class AWC_Cart_Early_Renewal extends AWC_Cart_Renewal {

		public function __construct() {

			// Add the renew now button to the view subscription page.
			add_filter( 'awc_view_details_subscription_myaccount_actions', array( $this, 'add_renew_now_action' ), 10, 2 );
			...
			// After the renewal order is created on checkout, set the renewal order cart item data now that we have an order. Must be hooked on before awc_Cart_Renewal->set_order_item_id(), in order for the line item ID set by that function to be correct.
			...
			add_action( 'awc_before_early_renewal_setup_cart_subscription', 'awc_allow_protected_products_to_renew' );
			add_action( 'awc_after_early_renewal_setup_cart_subscription', 'awc_disallow_protected_product_add_to_cart_validation' );
		}
		...
		*/
		public function add_renew_now_action( $actions, $subscription ) {
			...
			if ( awc_can_user_renew_early( $subscription ) && $subscription->payment_method_supports( 'subscription_date_changes' ) && $subscription->has_status( 'active' ) ) {

				$actions['subscription_renewal_early'] = array(
					'url'  => awc_get_early_renewal_url( $subscription ),
					'name' => __( 'Renew now', 'subscriptions-recurring-payments-for-woocommerce' ),
				);
			}
		...
		public function maybe_setup_cart() {
			...
			$subscription = awc_get_subscription( absint( $_GET['subscription_renewal_early'] ) );
			...
			if ( empty( $subscription ) ) {
			...
			} elseif ( ! awc_can_user_renew_early( $subscription ) ) {

				wc_add_notice( __( 'You can not renew this subscription early. Please contact us if you need assistance.', 'subscriptions-recurring-payments-for-woocommerce' ), 'error' );

			} else {

				do_action( 'awc_before_early_renewal_setup_cart_subscription', $subscription );

				wc_add_notice( __( 'Complete checkout to renew now.', 'subscriptions-recurring-payments-for-woocommerce' ), 'success' );
				...
				do_action( 'awc_after_early_renewal_setup_cart_subscription', $subscription );
		...
		*/
		public function maybe_record_early_renewal( $order_id, $posted_data ) {
			if ( ! AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
				_deprecated_function( __METHOD__, '2.0', 'awc_Cart_Early_Renewal::add_early_renewal_metadata_to_order( $order, $posted_data )' );
			}
			...
			$subscription->update_status( 'on-hold', _x( 'Customer requested to renew early:', 'used in order note as reason for why subscription status changed', 'subscriptions-recurring-payments-for-woocommerce' ) );
		}
		...
		public function copy_subscription_meta_to_order( $order ) {
			if ( $this->cart_contains() ) {
				...
				if ( $subscription ) {
					...
					add_filter( 'awc_renewal_order_meta', array( $this, 'exclude_core_order_meta_properties' ) );
					awc_copy_order_meta( $subscription, $order, 'renewal_order' );
					remove_filter( 'awc_renewal_order_meta', array( $this, 'exclude_core_order_meta_properties' ) );
				}
			}
		}
		...
		*/
		public function maybe_update_dates( $order_id ) {
			...
			if ( ! $order || ! awc_order_contains_early_renewal( $order ) ) {
				return;
			}

			$subscription_id = awc_get_objects_property( $order, 'subscription_renewal_early' );
			...
			awc_update_dates_after_early_renewal( $subscription, $order );
		}
		...
		*/
		public function maybe_reactivate_subscription( $order_id ) {
			...
			if ( awc_order_contains_early_renewal( $order ) ) {

				// Get the subscription and make sure we have one.
				$subscription = awc_get_subscription( awc_get_objects_property( $order, 'subscription_renewal_early' ) );
		...
		*/
		public function add_note_to_record_early_renewal( $order_id ) {
			...
			if ( awc_is_order( $order ) && awc_is_subscription( $subscription ) ) {
				// translators: %s: order ID.
				$order_number = sprintf( _x( '#%s', 'hash before order number', 'subscriptions-recurring-payments-for-woocommerce' ), $order->get_order_number() );

				// translators: %s: order ID (linked to details page).
				$subscription->add_order_note( sprintf( __( 'Order %s created to record early renewal.', 'subscriptions-recurring-payments-for-woocommerce' ), sprintf( '<a href="%s">%s</a> ', esc_url( awc_get_edit_post_link( $order_id ) ), $order_number ) ) );
			}
		}
		 ...
		 * created on checkout. Required so the line item ID set by @see awc_Cart_Renewal->set_order_item_id()
		 * matches the order.
		 *
		...
		*/
		public function set_cart_item_renewal_order_data( $order_id ) {
			...
			if ( ! awc_cart_contains_early_renewal() ) {
				return;
			}
		 ...
		 *
		 * Renewal orders are usually not cancellable because @see awc_Cart_Renewal::filter_my_account_my_orders_actions() prevents it.
		...
		*/
		public static function add_cancel_order_action( $actions, $order ) {
			...
			if ( ! isset( $actions['cancel'] ) && awc_order_contains_early_renewal( $order ) && in_array( $order->get_status(), apply_filters( 'woocommerce_valid_order_statuses_for_cancel', array( 'pending', 'failed' ), $order ) ) ) {
				...
				$actions['cancel'] = array(
					...
					'name' => __( 'Cancel', 'subscriptions-recurring-payments-for-woocommerce' ),
				);
			}
		 ...
		 *
		 * Renewal orders are usually not cancellable because @see AWC_Subscription_Renewal_Order::prevent_cancelling_renewal_orders() prevents the request from being processed.
		 * In the case of early renewals, the customer has opted for early renewal and so should be able to cancel it in order to reactivate their subscription.
		 *
		...
		*/
		public static function allow_early_renewal_order_cancellation() {
			if ( isset( $_GET['cancel_order'] ) && isset( $_GET['order_id'] ) && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'woocommerce-cancel_order' ) ) {
				...
				if ( awc_order_contains_early_renewal( $order ) ) {
					remove_action( 'wp_loaded', 'AWC_Subscription_Renewal_Order::prevent_cancelling_renewal_orders', 19 );
				}
			}
		}
		 ...
		 *
		 * Attached to the dynamic hook 'awc_renewal_order_meta' which is triggered by awc_copy_order_meta
		...
		*/
		public function exclude_core_order_meta_properties( $order_meta ) {
			...
			// Additional meta keys to exclude. These are in addition to the meta keys already excluded by awc_copy_order_meta().
