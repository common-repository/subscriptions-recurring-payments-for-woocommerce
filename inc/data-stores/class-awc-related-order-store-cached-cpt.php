 ...
 *
 * Adds a persistent caching layer on top of awc_Related_Order_Store_CPT for more
 * performant queries on related orders.
 *
 */
class AWC_Related_Order_Store_Cached_CPT extends AWC_Related_Order_Store_CPT implements AWC_Cache_Updater {

	/**
	 * Keep cache up-to-date with changes to our meta data via WordPress post meta APIs
	 * by using a post meta cache manager.
	 *
	 * @var AWC_Post_Meta_Cache_Manager
	 */
	...
	public function __construct() {
		...
		$this->post_meta_cache_manager = new awc_Post_Meta_Cache_Manager( 'shop_order', $this->get_meta_keys() );
	}
	...
	protected function init() {
		...
		add_filter( 'awc_subscription_data_store_props_to_ignore', array( $this, 'add_related_order_cache_props' ), 10, 2 );
		...
		add_filter( 'awc_created_subscription', array( $this, 'set_empty_renewal_order_cache' ), -1000 );

		// When the post for a related order is deleted or untrashed, make sure the corresponding related order cache is updated
		add_action( 'awc_update_post_meta_caches', array( $this, 'maybe_update_for_post_meta_change' ), 10, 5 );
		add_action( 'awc_delete_all_post_meta_caches', array( $this, 'maybe_delete_all_for_post_meta_change' ), 10, 1 );

		// When copying meta from a subscription to a renewal order, don't copy cache related order meta keys.
		add_filter( 'awc_renewal_order_meta', array( $this, 'remove_related_order_cache_keys' ), 10, 1 );

		AWC_Debug_Tool_Factory::add_cache_tool( 'generator', __( 'Generate Related Order Cache', 'subscriptions-recurring-payments-for-woocommerce' ), __( 'This will generate the persistent cache of all renewal, switch, resubscribe and other order types for all subscriptions in your store. The caches will be generated overtime in the background (via Action Scheduler).', 'subscriptions-recurring-payments-for-woocommerce' ), self::instance() );
		AWC_Debug_Tool_Factory::add_cache_tool( 'eraser', __( 'Delete Related Order Cache', 'subscriptions-recurring-payments-for-woocommerce' ), __( 'This will clear the persistent cache of all renewal, switch, resubscribe and other order types for all subscriptions in your store. Expect slower performance of checkout, renewal and other subscription related functions after taking this action. The caches will be regenerated overtime as related order queries are run.', 'subscriptions-recurring-payments-for-woocommerce' ), self::instance() );
	}

	/* Public methods required by awc_Related_Order_Store */
	...
	public function get_related_order_ids( WC_Order $subscription, $relation_type ) {
		...
		$subscription_id   = awc_get_objects_property( $subscription, 'id' ); // We can't rely on $subscription->get_id() being available because we only require a WC_Order, not a WC_Subscription, and WC_Order does not have get_id() available with WC < 3.0
		...
		if ( '' === $related_order_ids ) {
			...
			if ( 'renewal' === $relation_type ) {
				$transient_key = "asub-related-orders-to-{$subscription_id}"; // despite the name, this transient only stores renewal orders, not all related orders, so we can only use it for finding renewal orders
	...
	public function add_relation( WC_Order $order, WC_Order $subscription, $relation_type ) {
		$this->add_related_order_id_to_cache( awc_get_objects_property( $order, 'id' ), awc_get_objects_property( $subscription, 'id' ), $relation_type );
	...
	public function delete_relation( WC_Order $order, WC_Order $subscription, $relation_type ) {
		$this->delete_related_order_id_from_cache( awc_get_objects_property( $order, 'id' ), awc_get_objects_property( $subscription, 'id' ), $relation_type );
	...
	public function delete_relations( WC_Order $order, $relation_type ) {
		$this->delete_related_order_id_from_caches( awc_get_objects_property( $order, 'id' ), $relation_type );
	...
	public function delete_related_order_id_from_caches( $order_id, $relation_type = 'any' ) {
		...
		foreach ( awc_get_subscriptions_for_order( $order_id, array( 'order_type' => $relation_types ) ) as $subscription_id => $subscription ) {
	 ...
	 * 'awc_subscription_data_store_props_to_ignore' so that cache keys are ignored.
	 *
	 * @param array $props_to_ignore A mapping of meta keys => prop names.
	 * @param awc_Subscription_Data_Store_CPT $data_store
	 * @return array A mapping of meta keys => prop names, filtered by ones that should be updated.
	 */
	public function add_related_order_cache_props( $props_to_ignore, $data_store ) {

		if ( is_a( $data_store, 'awc_Subscription_Data_Store_CPT' ) ) {
			foreach ( $this->get_meta_keys() as $relation_type => $meta_key ) {
	 ...
	 *
	 * @param AWC_Subscription $subscription A subscription to set empty renewal cache against.
	 * @return AWC_Subscription Return the instance of the subscription, required as method is attached to the 'awc_created_subscription' filter
	 */
	public function set_empty_renewal_order_cache( AWC_Subscription $subscription ) {
		$this->update_related_order_id_cache( $subscription->get_id(), array(), 'renewal' );
		return $subscription;
	}

	/* Public methods attached to awc_Post_Meta_Cache_Manager hooks for managing the cache */
	...
	/** Methods to implement awc_Cache_Updater - wrap more accurately named methods for the sake of clarity */
