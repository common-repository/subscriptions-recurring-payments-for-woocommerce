 ...
 *
 * Adds a persistent caching layer on top of awc_Customer_Store_CPT for more
 * performant queries to find a user's subscriptions.
 *
 */
class AWC_Customer_Store_Cached_CPT extends AWC_Customer_Store_CPT implements awc_Cache_Updater {

	/**
	 * Keep cache up-to-date with changes to our meta data via WordPress post meta APIs
	 * by using a post meta cache manager.
	 *
	 * @var awc_Post_Meta_Cache_Manager_Many_To_One
	 */
	 ...
	 * @var string
	 */
	private $cache_meta_key = '_awc_subscription_ids_cache';

	/**
	 * Constructor
	 */
	public function __construct() {
		$this->post_meta_cache_manager = new awc_Post_Meta_Cache_Manager_Many_To_One( 'shop_subscription', array( $this->get_meta_key() ) );
	}
	...
	protected function init() {
		...
		AWC_Debug_Tool_Factory::add_cache_tool( 'generator', __( 'Generate Customer Subscription Cache', 'subscriptions-recurring-payments-for-woocommerce' ), __( 'This will generate the persistent cache for linking users with subscriptions. The caches will be generated overtime in the background (via Action Scheduler).', 'subscriptions-recurring-payments-for-woocommerce' ), self::instance() );
		AWC_Debug_Tool_Factory::add_cache_tool( 'eraser', __( 'Delete Customer Subscription Cache', 'subscriptions-recurring-payments-for-woocommerce' ), __( 'This will clear the persistent cache of all of subscriptions stored against users in your store. Expect slower performance of checkout, renewal and other subscription related functions after taking this action. The caches will be regenerated overtime as queries to find a given user\'s subscriptions are run.', 'subscriptions-recurring-payments-for-woocommerce' ), self::instance() );
	}

	/* Public methods required by awc_Customer_Store */
	...
	public function get_users_subscription_ids( $user_id ) {
		...
		if ( '' === $subscription_ids ) {
			...
			$transient_key = "awc_user_subscriptions_{$user_id}";
	 ...
	 *
	 * Applies the 'awc_get_cached_users_subscription_ids' filter for backward compatibility with
	 * the now deprecated awc_get_cached_user_subscription_ids() method.
	 *
	 * @param int $user_id The id of the user whose subscriptions you want.
	...
	protected function get_users_subscription_ids_from_cache( $user_id ) {
		...
		return apply_filters( 'awc_get_cached_users_subscription_ids', $subscription_ids, $user_id );
	}
	...
	/* Public methods attached to awc_Post_Meta_Cache_Manager_Many_To_One hooks for managing the cache */
	...
	public function maybe_update_for_post_meta_change( $update_type, $subscription_id, $meta_key, $user_id, $old_user_id = '' ) {
		...
		switch ( $update_type ) {
			case 'add':
				...
				break;
			case 'delete':
				// If we don't have a specific user ID, the post is being deleted as awc_Post_Meta_Cache_Manager_Many_To_One doesn't pass the associated meta value for that event, so find the corresponding user ID from post meta directly
				if ( empty( $user_id ) ) {
