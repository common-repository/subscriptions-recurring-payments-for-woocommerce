 ...
 *
 */
class AWC_Subscription_Data_Store_CPT extends WC_Order_Data_Store_CPT implements WC_Object_Data_Store_Interface, WC_Order_Data_Store_Interface {
	...
	public function __construct() {
		...
		foreach ( awc_get_subscription_date_types() as $date_type => $date_name ) {
			...
			$this->subscription_meta_keys_to_props[ $meta_key ] = awc_maybe_prefix_key( $date_type, 'schedule_' );
	...
	protected function read_order_data( &$subscription, $post_object ) {
		...
		// Set all order meta data, as well as data defined by AWC_Subscription::$extra_keys which has corresponding setter methods
	...
	public function get_orders( $args = array() ) {
		...
		if ( isset( $args['paginate'] ) && $args['paginate'] ) {
			...
			if ( 'objects' === $args['return'] ) {
				$return = array_map( 'awc_get_subscription', $subscriptions->orders );
			} else {
			...
			if ( 'objects' === $args['return'] ) {
				$return = array_map( 'awc_get_subscription', $subscriptions );
			} else {
	...
	public function save_dates( $subscription ) {
		...
		foreach ( awc_get_subscription_date_types() as $date_type => $date_name ) {
			...
			$date_meta_key = awc_get_date_meta_key( $date_type );
		...
		foreach ( $this->get_props_to_update( $subscription, $date_meta_keys_to_props ) as $meta_key => $prop ) {
			...
			$saved_dates[ $prop ] = awc_get_datetime_from( $subscription->get_time( $prop ) );
		}
	...
	protected function get_props_to_ignore() {
		...
		return apply_filters( 'awc_subscription_data_store_props_to_ignore', $props_to_ignore, $this );
	}
