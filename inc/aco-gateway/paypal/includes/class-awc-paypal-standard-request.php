...
class AWC_PayPal_Standard_Request {
	...
	public static function get_paypal_args( $paypal_args, $order ) {
		...
		$is_payment_change = AWC_Subscription_Change_Payment_Gateway::$is_request_to_change_payment;
		$order_contains_failed_renewal = false;

		// Payment method changes act on the subscription not the original order
		if ( $is_payment_change ) {

			$subscription = awc_get_subscription( awc_get_objects_property( $order, 'id' ) );
			$order        = $subscription->get_parent();

			// We need the subscription's total
			if ( AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
				remove_filter( 'woocommerce_order_amount_total', 'AWC_Subscription_Change_Payment_Gateway::maybe_zero_total', 11 );
			} else {
				remove_filter( 'woocommerce_subscription_get_total', 'AWC_Subscription_Change_Payment_Gateway::maybe_zero_total', 11 );
			}
		} else {

			// Otherwise the order is the $order
			if ( $cart_item = awc_cart_contains_failed_renewal_order_payment() || false !== AWC_Subscription_Renewal_Order::get_failed_order_replaced_by( awc_get_objects_property( $order, 'id' ) ) ) {
		...
		if ( $order_contains_failed_renewal || ( ! empty( $subscription ) && $subscription->get_total() > 0 && !AWC_Settings::get_option('turn_of_automatic_payment') ) ) {
			...
			foreach ( $subscription->get_items() as $item ) {
				if ( $item['qty'] > 1 ) {
					$item_names[] = $item['qty'] . ' x ' . awc_get_paypal_item_name( $item['name'] );
				} elseif ( $item['qty'] > 0 ) {
					$item_names[] = awc_get_paypal_item_name( $item['name'] );
				}
			}
			...
			if ( empty( $order ) ) {
				...
				$paypal_args['item_name'] = awc_get_paypal_item_name( sprintf( _x( 'Subscription %1$s - %2$s', 'item name sent to paypal', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription->get_order_number(), implode( ', ', $item_names ) ) );
			} else {
				// translators: 1$: subscription ID, 2$: order ID, 3$: names of items, comma separated
				$paypal_args['item_name'] = awc_get_paypal_item_name( sprintf( _x( 'Subscription %1$s (Order %2$s) - %3$s', 'item name sent to paypal', 'subscriptions-recurring-payments-for-woocommerce' ), $subscription->get_order_number(), $order->get_order_number(), implode( ', ', $item_names ) ) );
			}
			...
			if ( $resubscribe_cart_item = awc_cart_contains_resubscribe() ) {
				$resubscribed_subscription = awc_get_subscription( $resubscribe_cart_item['subscription_resubscribe']['subscription_id'] );
				$is_early_resubscribe      = awc_is_subscription( $resubscribed_subscription ) && $resubscribed_subscription->has_status( 'pending-cancel' );
			}
			...
			$subscription_length = awc_estimate_periods_between( $length_from_timestamp, $subscription->get_time( 'end' ), $subscription->get_billing_period() );
			...
			if ( $order_contains_failed_renewal || $is_payment_change ) {
				...
				if ( $is_payment_change ) {
					...
					$suffix = '-asubcpm-' . wp_create_nonce();
				} else {
					// Failed renewal order, append a descriptor and renewal order's ID
					$suffix = '-asubfrp-' . awc_get_objects_property( $order, 'id' );
				}
				...
				if ( false === $parent_order ) {
					...
					$order_number = ltrim( $subscription->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'subscriptions-recurring-payments-for-woocommerce' ) ) . '-subscription';
					$order_id_key = array(
						'order_id'  => $subscription->get_id(),
						'order_key' => $subscription->get_order_key(),
					);
				} else {
					$order_number = ltrim( $parent_order->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'subscriptions-recurring-payments-for-woocommerce' ) );
					$order_id_key = array(
						'order_id'  => awc_get_objects_property( $parent_order, 'id' ),
						'order_key' => awc_get_objects_property( $parent_order, 'order_key' ),
					);
				}
				...
				$paypal_args['invoice'] = AWC_PayPal::get_option( 'invoice_prefix' ) . $order_number . $suffix;
				$paypal_args['custom']  = awc_json_encode(
					array_merge(
						$order_id_key,
						array(
				...
				$paypal_args['custom'] = awc_json_encode(
					array(
						...
						'order_key'        => awc_get_objects_property( $order, 'order_key' ),
						'subscription_id'  => $subscription->get_id(),
						'subscription_key' => $subscription->get_order_key(),
					)
				);
			...
			// If we're changing the payment date or switching subs, we need to set the trial period to the next payment date & installments to be the number of installments left
				...
				if ( false != $next_payment_timestamp && ! empty( $next_payment_timestamp ) ) {
					...
					$trial_until = awc_calculate_paypal_trial_periods_until( $next_payment_timestamp );
				...
				$subscription_trial_length = awc_estimate_periods_between( $start_timestamp, $trial_end_timestamp, $subscription->get_trial_period() );
			...
			if ( $is_payment_change ) {
				if ( AWC_Subscriptions::is_woocommerce_pre( '3.0' ) ) {
					add_filter( 'woocommerce_order_amount_total', 'AWC_Subscription_Change_Payment_Gateway::maybe_zero_total', 11, 2 );
				} else {
					add_filter( 'woocommerce_subscription_get_total', 'AWC_Subscription_Change_Payment_Gateway::maybe_zero_total', 11, 2 );
				}
			}
		}
