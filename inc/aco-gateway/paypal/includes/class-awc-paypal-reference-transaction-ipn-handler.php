...
class AWC_PayPal_Reference_Transaction_IPN_Handler extends AWC_PayPal_Standard_IPN_Handler {
		...
		switch ( $transaction_details['txn_type'] ) {
			...
			case 'merch_pmt':
				...
				if ( ! empty( $transaction_details['custom'] ) && ( $order = $this->get_paypal_order( $transaction_details['custom'] ) ) ) {
					...
					WC_Gateway_Paypal::log( 'Found order #' . awc_get_objects_property( $order, 'id' ) );
	...
	protected function cancel_subscriptions( $billing_agreement_id ) {
		$note = esc_html__( 'Billing agreement cancelled at PayPal.', 'subscriptions-recurring-payments-for-woocommerce' );

		foreach ( AWC_PayPal::get_subscriptions_by_paypal_id( $billing_agreement_id, 'objects' ) as $subscription ) {
			...
			if ( $is_paypal_subscription && ! $subscription->has_status( awc_get_subscription_ended_statuses() ) ) {
				try {
