...
class AWC_PayPal_Change_Payment_Method_Admin {
	...
	public static function add_payment_meta_details( $payment_meta, $subscription ) {
		...
		if ( awc_is_paypal_profile_a( $subscription_id, 'billing_agreement' ) || empty( $subscription_id ) ) {
	...
	public static function validate_payment_meta( $payment_meta, $subscription ) {
		...
		} elseif ( 0 !== strpos( $payment_meta['post_meta']['_paypal_subscription_id']['value'], 'B-' ) && awc_get_paypal_id( $subscription ) !== $payment_meta['post_meta']['_paypal_subscription_id']['value'] ) {
