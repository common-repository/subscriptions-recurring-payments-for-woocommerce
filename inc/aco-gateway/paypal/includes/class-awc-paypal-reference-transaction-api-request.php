...
class AWC_PayPal_Reference_Transaction_API_Request {
	...
	public function set_express_checkout( $args ) {
		...
		$default_description = sprintf( _x( 'Orders with %s', 'data sent to paypal', 'subscriptions-recurring-payments-for-woocommerce' ), get_bloginfo( 'name' ) );
		...
		$this->add_parameters( array(
			...
			'L_BILLINGAGREEMENTDESCRIPTION0' => awc_get_paypal_item_name( $args['billing_description'] ),
	...
	public function do_reference_transaction( $reference_id, $order, $args = array() ) {
		...
		$defaults = array(
			...
			'invoice_number'       => awc_PayPal::get_option( 'invoice_prefix' ) . awc_str_to_ascii( ltrim( $order->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'subscriptions-recurring-payments-for-woocommerce' ) ) ),
			'custom'               => awc_json_encode(
				array(
	...
	protected function add_payment_details_parameters( WC_Order $order, $type, $use_deprecated_params = false ) {
		...
		foreach ( $order->get_items() as $item ) {
			...
			$order_items[] = array(
				'NAME'    => awc_get_paypal_item_name( $product->get_title() ),
		...
		foreach ( $order->get_fees() as $fee ) {
			...
			$order_items[] = array(
				'NAME' => awc_get_paypal_item_name( $fee['name'] ),
		...
		if ( $order->get_total_discount() > 0 ) {
			...
			$order_items[] = array(
				'NAME' => __( 'Total Discount', 'subscriptions-recurring-payments-for-woocommerce' ),
				'QTY'  => 1,
		...
		if ( $this->skip_line_items( $order, $order_items ) ) {
			...
			$this->add_line_item_parameters( array(
				// translators: placeholder is blogname
				'NAME' => sprintf( __( '%s - Order', 'subscriptions-recurring-payments-for-woocommerce' ), get_option( 'blogname' ) ),
				'DESC' => awc_get_paypal_item_name( implode( ', ', $item_names ) ),
			...
			if ( $use_deprecated_params ) {
				$this->add_parameters( array(
					'AMT'              => $total_amount,
					'CURRENCYCODE'     => awc_get_objects_property( $order, 'currency' ),
					...
					'INVNUM'           => AWC_PayPal::get_option( 'invoice_prefix' ) . awc_str_to_ascii( ltrim( $order->get_order_number(), _x( '#', 'hash before the order number. Used as a character to remove from the actual order number', 'subscriptions-recurring-payments-for-woocommerce' ) ) ),
					'PAYMENTACTION'    => $type,
					'PAYMENTREQUESTID' => awc_get_objects_property( $order, 'id' ),
					'CUSTOM'           => json_encode(
						array(
							'order_id'  => awc_get_objects_property( $order, 'id' ),
							'order_key' => awc_get_objects_property( $order, 'order_key' ),
	...
	public function manage_recurring_payments_profile_status( $profile_id, $new_status, $order = null ) {
		...
		$this->add_parameters( apply_filters( 'woocommerce_subscriptions_paypal_change_status_data', array_merge( $this->get_parameters(), array(
			...
			'NOTE'      => html_entity_decode( sprintf( _x( '%1$s subscription event triggered at %2$s', 'data sent to paypal', 'subscriptions-recurring-payments-for-woocommerce' ), $new_status, get_bloginfo( 'name' ) ), ENT_NOQUOTES, 'UTF-8' ),
	...
	private function get_item_description( $item, $product ) {
		...
		if ( ! empty( WC()->cart ) && is_array( $item ) && empty( $item['item_meta'] ) ) {
			...
			if ( ! empty( $item_desc ) ) {
				...
				$item_desc = is_callable( array( $product, 'get_sku' ) ) && $product->get_sku() ? sprintf( __( 'SKU: %s', 'subscriptions-recurring-payments-for-woocommerce' ), $product->get_sku() ) : null;
			}
		}

		return awc_get_paypal_item_name( $item_desc );
	}
	...
	public function get_parameters() {
		 ...
		 */
		$this->parameters = apply_filters( 'awc_paypal_request_params', $this->parameters, $this );
	...
	private function skip_line_items( $order = null, $order_items = null ) {
		...
		$skip_line_items = awc_get_objects_property( $order, 'prices_include_tax' );
		 ...
		 */
		return apply_filters( 'awc_paypal_reference_transaction_skip_line_items', $skip_line_items, $order );
	}
